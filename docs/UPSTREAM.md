...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`8bfef4de8`** — "fix(ai): update strict mode test expectations" (advanced 2026-09-21, second advance of the day). **3 changes in `f5c946480..origin/main`; first-parent and the union agree** (no merges in range), and **no release tag** (`packages/ai/package.json` at `8bfef4de8` still reads `0.86.1`), so no regen and no port tag. **Verdicts: 0 port-shipped / 1 port-but-CATALOG-ONLY (both halves) / 2 n/a / 0 decide.** **`890f92088` is held WHOLE for the next regen, runtime half included** — see its row in the Catalog regen checklist. It flips `detectCompat`'s `supportsStrictMode` default to false and makes the generator stamp an explicit `true` on every strict-capable model. The two halves only make sense together: the port's `getOpenAICompat` falls back to `detectOpenAICompat` for any key the catalog omits, and **597 of the 688 openai-completions models in the 0.86.1 catalog omit `supportsStrictMode`** (365 of them openrouter). Porting the runtime flip alone would strip `strict` from all 597 request bodies — a state no pi build has ever been in, since npm 0.86.1 carries the old runtime and the next build carries both. **n/a (2):** `63787ee6b` adds `findExtensionStackMatches` to `core/crash-log.ts` (already entry 12 via `3c75b2747`), typed on `Extension`, whose SOLE consumer at HEAD is `modes/interactive/interactive-mode.ts` — E1 only-consumers; `8bfef4de8` is tests-only. **Reconciliation:** detector 13 paths, all under `packages/ai` + `packages/coding-agent`; no new package, no repo-root path, none of the four split-by-HUNK tripwires. **Upstream `ci.yml` per sha:** `63787ee6b` and `8bfef4de8` green; **`890f92088` FAILS** its own build (stale `cache-retention` / `fireworks-models` expectations), fixed by `8bfef4de8` one commit later — content fine, sha red. |
...
the matching npm build. **Currently 2 — drained from 10 on 2026-09-20 by the
0.86.1 regen, then `890f92088` joined.** All ten rows were ancestors of the `v0.86.1` release commit
...
|---|---|
| `890f92088` (2026-09-21) | **Runtime half rides with this row — land them in ONE commit.** Generator: `OPENAI_COMPLETIONS_DEFAULT_COMPAT.supportsStrictMode` true → false, so every model `detectOpenAICompletionsCompat` finds strict-capable (not moonshot / together / cloudflare-ai-gateway / nvidia / cerebras) gains an EXPLICIT `supportsStrictMode: true`; at 0.86.1, 597 openai-completions models carry no key. Runtime: `detectOpenAICompat`'s `SupportsStrictMode` in `ai/providers/openai_compat.go` becomes plain `false` (pi: "OpenAI compatibility alone does not imply strict JSON-schema tool support"), and the `types.ts` doc comment's "Default: true" becomes false. **Check at the regen:** the count of catalog models without the key should collapse to the non-capable remainder; any capable model still missing it would lose `strict` on its request body. Oracle: upstream `test/openai-completions-tool-choice.test.ts` (+78 at `890f92088`) and `compat_test.go`'s detected-true case, which must move with it. |
