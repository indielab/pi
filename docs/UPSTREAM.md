...
| What | Value |
|---|---|
| TS source fully reviewed/ported | `651d5d6a5` — "partial harness v2/json backend (#7611)" (2026-08-05; **5 ports → 4 Go port commits + 3 review-fix commits, 31 n/a, 1 decide raised and RESOLVED no-action** — delta `b784c8096..651d5d6a5`, **37** first-parent changes. **No release crossed** — no tag in range, every `packages/*/package.json` still **0.83.0**, no regen, no tag, no tweet. Two `ai/scripts/generate-models.ts` edits queue catalog movement for the NEXT release under the 2026-07-30 ruling: `720f0e8ee` routes Copilot `grok-4.5` through `/responses`, and `71f6c25c3` moves the Groq thinking-level override from `qwen/qwen3-32b` to `qwen/qwen3.6-27b`. All ports are **unreleased upstream**, so parity was derived from TS source at each porting sha, never from `dist/`. Ports: `686f193e5` deferred request options split (Go `24600a3`), `42a06f947`+`d3da2e968` composite OAuth refresh cancellation (Go `acd9696`), `bb6a1cddc` backend→service rename (Go `d7860e6`), `dab70b2cd` sanitize service failures (Go `aa586d1`). **One byte-golden moved — the wire**: `protocol/testdata/upstream_messages.json` gains exactly two vectors (`resp_not_implemented`, `resp_internal_error`) with every pre-existing frame byte-unchanged; derived from upstream TS (`git archive dab70b2cd`, `typebox@1.3.7` + `pi-ai@0.83.0` with lock integrity matched against `npm view … dist.integrity`, run under `node --experimental-strip-types`) and proved **non-circular at both ends** — the unmodified generator against `dab70b2cd^1` reproduces the previously committed fixture byte-for-byte, and the new vectors run against the pre-change schema are *refused* (`"Invalid server protocol message"`). The parity gate independently re-derived the whole fixture from scratch and hand-decoded the CBOR rather than trusting the generator. **`42a06f947`/`d3da2e968` are test-only in Go: the behavior was ALREADY correct** — `context.WithCancel`/`WithTimeout` do propagate the parent's cancellation cause to every descendant (only `WithCancelCause`'s own cancel func substitutes a fresh one), so all three of upstream's tightened assertions already held; proven non-vacuous by mutation against a cause-losing `WithCancelCause`+`AfterFunc` hand-roll. **`bb6a1cddc` is not the pure rename it looks like**: one renamed string in `server/sessions.go` crosses the protocol boundary to the peer verbatim (`Service returned session %s for server-assigned session %s`). Nothing in the repo asserted it, which is how it nearly slipped; it now matches pi byte-exactly and is pinned by a wire-level test — keeping `Backend` would itself have been the divergence, and a wire-visible one (a Go server and a Node server reporting different text for the same protocol error is exactly the interop failure the 2026-08-01 wire-golden ruling exists to prevent). **Two independent review gates: parity-review FAITHFUL (all four, no divergences), go-review FIX-FIRST with 2 MED + 4 LOW, all fixed** in `db2ade7`, `89a7604`, `e4ae08d`. The two MEDs both concerned code the tests could not see: (1) `InternalError.Error()` returned the sanitized constant, so the retained cause vanished on the three reporter paths that log the *wrapper* rather than `.Cause` (`snapshots.go:145`, `sessions.go:413`, `sessions.go:553`) — the exact loss the type exists to prevent; the message now names its cause while the wire formatter keeps emitting its own constant, verified wire-neutral by sweeping every site in `server/` that puts error text into a protocol field. (2) `toProtocolError`'s load-bearing branch ordering — `InternalError` must match before `*Error`, because it may wrap one — was documented at the site but **not test-locked**: swapping the blocks left `./server/...` fully green, so a future reorder would have put a wrapped `*Error`'s own code and message on the wire with no red. Now pinned by a test that fails `code = "not_found", want internal_error` when reordered. Also fixed: `FetchDeferred` rebuilt its option struct instead of copy-then-overwrite (no field dropped today, a forward hazard the next upstream field would trip silently). gofmt/build/vet green; `go test -race ./... -count=1` green; **§3 differential harness 11 PASS / 0 KNOWN / 0 FAIL** at the new sha. **Follow-up recorded, deliberately not fixed in a sync commit**: `ai/auth_resolve.go:211` returns `ctx.Err()` rather than `context.Cause(ctx)`, so a *custom* caller cause does not survive to `GetProviderAuth`'s return value where upstream rejects with `signal.reason` — pre-existing, ~12 sites, package-wide convention, not asserted by upstream's tests.) Prior pin: `b784c8096` — "fix(ai): remove OAuth refresh lock test" (2026-08-04; **15 ports → 15 Go port commits + 9 review-fix commits, 30 n/a, 0 decide open** — delta `c6eb6281a..b784c8096`, **45** first-parent changes. **No release crossed** — no tag in range, every `packages/*/package.json` still **0.83.0**, catalog + every byte-golden untouched, no regen, no tag, no tweet. `ai/scripts/generate-models.ts` moved (+105), which under the 2026-07-30 ruling signals the NEXT release will move catalog data. All 14 ports are **unreleased upstream**, so parity was derived from TS source at each porting sha, never from `dist/`. Ports: `b9d360a2c` google retry (Go `8b78853` — test-only after review, see the divergence note), `cbaca6038` Gemini 3 tool call IDs (`301637b`), `2e95584da` nullable-union coercion (`19cd850`), `0f5286d8a` `ShouldStopAfterTurn` on Agent (`55873f8`), `b0e05b442` tool-result image resize (`c740b38`), `523b5a491` find path normalization (`0cbc13b`), `18d65de62` Copilot summary endpoint (`6d4a380`), `c1019d920` Baseten (`dbc464d`), `32850ef7c` Responses incomplete stops (`ef00fe8`), `25a2c8dcf` sampling params (`fc6f777`), `305c014dc` transport-specific session auth (`8db2a2b`), `fed6009cc` model-refresh cancellation (`1592c6b`), `acbdc0d25` OAuth refresh bound (`961242b`), `382aa641c` deferred responses (`2676815`, an upstream **DRAFT** ported under the 2026-08-04 interim ruling). **Plus `a24fb9e96` null-`ProviderHeaders` (`bcd4712` + polish `8cec36a`)** — triaged `decide`, ruled `port` by the owner mid-cycle, gates go-review **SHIP** / parity-review **FAITHFUL**. **The §3 differential harness was also rebuilt durably at `~/.cache/pi-diff/`** (it had been lost twice to `/tmp`); it confirmed three of this cycle's request-body ports faithful and surfaced a **pre-existing model-visible divergence — tool-call argument key order — escalated and then FIXED in this cycle** (see the drift note). **Two independent review gates: go-review SHIP, parity-review FIX-FIRST with 4 behavioral divergences, all fixed** in `403c4b8` (D1), `27eb2d7` (D2), `1dd45be` (D3), `1a22aa7` (D4), `8310959`, `3fb2a28`, `a2aa3b0`. **None of the four was caught by any in-repo test** — the suite was green throughout. D1 (HIGH, model-visible): the find port dropped upstream's trailing-separator carry on the reasoning that "WalkDir never yields one", which inverts the mapping — upstream's carry lives on the **fd-subprocess** branch and Go's walker is what REPLACED fd; verified against the real `~/.pi/agent/bin/fd` 10.4.2, pi emits `src/` where Go emitted `src`, and the trailing slash is find's ONLY directory signal to the model. D2 (MED): `18d65de62` shipped **zero behavior** behind a "no Go analog" claim, and its test was **vacuous** (its own `streamFn` performed the rebuild, so it asserted the test double); the analog exists and exhibited pi's bug — `coding/compaction.go` passes `APIKey`, which short-circuits `auth_resolve.go` to the api-key path, leaving `auth.BaseURL` empty so the model rebuild never fires; now fixed faithfully via `summarizationRequestModel` and re-pinned against a real `ai.Models` + OAuth credential. D3 (MED): `Agent.ShouldStopAfterTurn` never saw the run's abort signal (Agent-layer only — `agent/types.go` correctly stays context-only, matching pi). D4 (MED): `CheckAuth`/`GetAvailable`/`Login` hung on non-cooperative providers where pi rejects immediately; the porter's "Go has no promise race" comment was refuted by the port's own `Refresh`, which races on `ctx.Done()`. Plus one **missed hunk** found by a Baseten sub-review: `c1019d920` added `baseten: "zai-org/GLM-5.2"` to `defaultModelPerProvider`, which IS ported to `coding/resolve.go` even though its file `model-resolver.ts` is otherwise host-only and triaged `n/a` — inert until the next catalog regen, then silently wrong; fixed in `a2aa3b0`. gofmt/build/vet green; `go test -race ./... -count=1` green; in-repo differential 40 pi-derived request-body tests pass.) Prior pin: `c6eb6281a` — "fix(coding-agent): bound post-login catalog refresh" (2026-08-03; **0 ports, 3 n/a, 0 decide** — delta `01eeafd14..c6eb6281a`, **3** first-parent changes, none in ported surface: a docs revert, a TUI model-selector fix, and a host `core/model-runtime.ts` timeout bound that is `n/a` under the 2026-07-17 model-runtime ruling. **No release crossed** — no tag in range, every `packages/*/package.json` still **0.83.0**, `models.generated.ts` untouched, catalog + every byte-golden untouched, no regen, no tag, no tweet. **Pin-advance only: no Go code changed, so the parity/review state below carries over unchanged from `01eeafd14`.**) Prior pin: `01eeafd14` — "docs(agent): add durable queue-item cancellation to harness v2 variants" (2026-08-03; **5 ports → 8 Go port commits + 3 fix commits, 39 n/a, 0 decide** — delta `ab366ebe9..01eeafd14`, **44** first-parent changes, combining the 40 triaged on 2026-08-02 (pin then deliberately HELD — see that entry) with 4 new ones. **No release crossed** — no tag in range, every `packages/*/package.json` still **0.83.0**, `models.generated.ts` untouched, catalog + every byte-golden untouched, no regen, no tag. All five ports are **unreleased upstream**, so parity was derived from TS source at each porting sha rather than from `dist/`. This cycle is the **remote-session stack**, three new Go packages (`protocol/`, `client/`, `server/`) plus `coding/remotesession.go` and `coding/transcript.go`, ~10k lines: `5a38a1c12` runtime-neutral client (Go `86a91f3`, `d9256a9`, `78b08a9`), `7d5fc9499` unix client transport (Go `a402e1d`), `73b24639f` server core + `legacy/` split (Go `1a3b46b`, `3289329`, `9afbdf2`), `03eba409c` server/protocol invariants (Go `3289329`, `9afbdf2`), `06a1ceb8d` remote client controller (Go `78b08a9`, `920f37b`). Nine independent review gates found **7 HIGH and ~23 MED**, every one fixed and test-locked in `b9b70fb`, `a1bbcfc`, `e5f820d` before the pin moved.) Prior pin: `ab366ebe9` — "fix: custom-compaction through provider via new model runtime complete (#7325)" (2026-07-31; **3 ports → 4 Go commits, 23 n/a, 0 decide** — delta `c13ffe18..ab366ebe9`, 26 first-parent changes; **no release crossed** — no tag in range, npm `pi-ai`/`pi-coding-agent` stay **0.83.0**, `models.generated.ts` untouched, catalog + every byte-golden untouched, no regen; all three ports are **unreleased upstream**, so parity was derived from TS source at each porting sha rather than from `dist/`. Ports: **streams without finish reasons** `2c3041242` (Go `3ed35dc`) — new openai-completions compat flag `supportsFinishReason` (default **true**, carried in the detect defaults so the struct's zero value is never observed; `*bool` override for `??` semantics), and when a provider waives it the stop reason is inferred as toolUse-if-any-toolCall-else-stop instead of failing; pi's guard ordering is reproduced but is **shape parity only** — `StopAborted`/`StopError` imply `hasFinishReason`, so only the `ctx.Err()` check can preempt inference. The `generate-models.ts` DATA half folds into the next regen and is confirmed inert today (`supportsFinishReason` appears zero times upstream in `models.generated.ts` and never as `false`). **Signed empty text/thinking blocks in google history** `6138f5a07` (Go `18b324e`) — Gemini attaches a `thoughtSignature` to parts whose visible text is empty and requires it echoed back; the empty-block skip now also requires an absent signature, and pi's `(!block.text || block.text.trim() === "")` correctly collapses to `strings.TrimSpace(x) == ""` since `Text` is a non-pointer string. A **live request-body change**: deriving pi from TS across 29 google models × 7 scenarios, **4 scenarios moved on 29/29 models**, new Go matching pi where old Go did not, while the two negative scenarios correctly held — no *in-repo* golden moved, and none should have. The cross-model thinking branch is **dead code in Go and in pi alike** (transformMessages runs first and google's `isSame` is strictly weaker than transform's `isSameModel`; panic-mutation leaves the package green) — ported for shape parity and now commented as such. **Anthropic initial stream block content** `59ad3dead` (Go `3fc128f`) — `content_block_start` can already carry text / thinking+signature; the builders are seeded from it before `materialize()`, so the emitted start partial carries it too, locked by a test that goes beyond upstream's. Reviews: go-review **ship** (2 MED + 2 LOW applied in `9d96ca2`, none behavioral — both MEDs were unreachable code presented as live; one LOW deliberately not taken, hoisting a shared `getOpenAICompat` through `StreamOpenAICompletions`) + independent adversarial parity review **FAITHFUL** (no fix-first divergences; mandated openai request diff **6/6 scenarios, 3324/3324 model-cases** incl. 72 byte-identical error paths; every porter claim independently checked, and the porter's "nothing moved" claim **corrected upward** — the google port did move behavior, exactly where intended). **Two pre-existing divergences recorded, not fixed**: `strings.TrimSpace` ≠ JS `.trim()` on U+FEFF/U+0085 (repo-wide question, negligible frequency), and anthropic `tool_use` blocks never seeded from `content_block.input` where pi uses `input ?? {}` — the latter sits in the very switch this fix touches and is **recommended for next cycle**. gofmt/build/vet/`-race` green. See the 2026-07-31 drift note); earlier pins — `c13ffe18` — "feat(tui): add alternate-screen renderer" (2026-07-30; **3 ports → 5 Go commits, 13 n/a, 0 decide** — delta `cced6a21..c13ffe18`, 17 first-parent changes; **release crossed — npm `pi-ai`/`pi-coding-agent` 0.82.1 → 0.83.0** (`845d6ff1`, no tag object in the fetched refs) and the **catalog DID move** — 456,576 → 477,229 B, 1116 → 1153 models (+64/−27/91 changed), regenerated in `1190695` and endpoint-pinned both ends. Ports: **qwen thinking controls** `4c1a0b92` (Go `dccdc48`) — the qwen branch emits `reasoning_effort` via `model.thinkingLevelMap` under **`??`** semantics (a present-null entry falls back to the RAW level and IS emitted — the distinction from the zai branch's `=== undefined`), so Go routes it through `effortValue` not `mappedEffortOrRaw`; a **live request-body change** for 27 models across 3 providers, since `supportsReasoningEffort` resolves as `model.compat.… ?? detected.…` and qwen-token-plan/-cn/opencode-go trip none of detection's seven negating flags. **preserve original stop reason** `d7b02636` (Go `a1cae2a`) — `AssistantMessage.RawStopReason` set by the four ported providers, anthropic's `sensitive` gains `Provider stopped with: sensitive`, google's end-of-stream error becomes `Provider stopped with: <raw>`; bedrock/vertex/mistral correctly unported (no Go stream impl, and — checked against the 2026-07-21 lesson — no shared Go resolver serves them; conversely pi's identical vertex+generative-ai edits correctly collapse into Go's single `google.go`). **empty custom payloads** `34239180` (Go `162c6fa`) — a delta carrying both `custom` and `function` is an ordinary function call, modeled by pointer nil-ness and extracted to `isGrammarCall()`. **catalog regen** `845d6ff1` (Go `1190695`) — carries the DATA half of `4c1a0b92`; without it the code half sends `reasoning_effort` to exactly the models the fix excludes, a state no published pi ever had. **One pre-existing divergence fixed** (`b293a70`, found by the parity re-gate): `coding/resolve.go` carried 35 of pi's 38 `defaultModelPerProvider` entries — the two missing qwen-token-plan ones were load-bearing (fallback cloned MiniMax-M2.5's 196608/32768 instead of qwen3.7-max's 1000000/131072, changing `max_tokens` and the context clamp). `bff5ab71` was triaged `port` and **downgraded to `n/a`** during porting (no `ResourceLoader` analog; see the 2026-07-30 ruling). Reviews: go-review **ship** (2 MED + 4 LOW applied in `6ade7a1` — the `response.failed` clear-then-set was live but unpinned; the grammar predicate was duplicated) + adversarial parity review over two passes **FAITHFUL** (first pass 6/6 but raised the catalog HIGH; the re-gate ran **8310/8310** live pi-vs-Go request bodies, endpoint-pinned the catalog at both ends, and verified the `name()` fix 17/17). gofmt/build/vet/`-race` green; in-repo differential 38/38. See the 2026-07-30 drift note); earlier pins — `cced6a21` — "fix(coding-agent): stop loading AGENTS.md twice in nested git worktrees (#7221)" (2026-07-29; **2 ports → 3 Go commits, 7 n/a, 0 decide** — delta `47ca25fc..cced6a21`, 9 first-parent changes; **no release crossed** — no tag in range, npm `pi-ai`/`pi-coding-agent` stay **0.82.1**, `models.generated.ts` + `src/data` unchanged, catalog + every byte-golden untouched, no regen. Ports: **per-request fetch injection** `027a5847` (Go `6f89b12` + review-fix `6af8469`) — pi's `StreamOptions.fetch` becomes `ai.HTTPDoer` on `StreamOptions`, threaded through `sendWithRetry` (covering anthropic-messages, openai-completions, openai-responses) and through pi-messages, which calls out directly and keeps `http.DefaultClient` as its default; google-generative-ai **rejects** an override with pi's byte-exact `Custom fetch is not supported by the Google Generative AI adapter`, guard placed before the api-key check as upstream; `http.DefaultClient` stands in for the `globalThis.fetch` pi blesses as equivalent to unset, enforced in ONE place (`customHTTPClient`) across all three call sites; **no request-building code changed** — `ai/providers/openai*.go` have a zero-byte diff. **AGENTS.md double-load in nested worktrees** `cced6a21` (Go `eae51bd`) — `LoadProjectContextFiles` skips the main repo's copy when a nested linked worktree's root holds a same-named context file; `findGitPaths` written fresh in `coding/resources.go` (pi exports it from the unported TUI footer, and `glob.go`'s `findRepoRoot` cannot be reused — it never resolves the `gitdir:`/`commondir` chain), all nine upstream scenarios ported incl. the bare-layout/sibling/submodule/dangling-gitdir negatives. Both reviews independently caught the same **HIGH** — the injected client never reached the providers from the agent SDK, because pi gets it free via `AgentLoopConfig extends SimpleStreamOptions` while Go's loop forwards `StreamOptions` field-by-field and this field was missing; fixed in `6af8469` along with a split definition of "no override" that let `http.DefaultClient` silently displace `sharedClient`, a `findGitPaths` stat-error path that aborted the walk where pi keeps climbing, and three test gaps (openai-responses injection, the `sendWithRetry` fallback, and canonicalization — which **no** worktree case pinned, since every scenario built its `gitdir:` target from the same uncanonicalized temp path). **One new deliberate divergence**: an injected client bypasses `sharedClient`'s `ResponseHeaderTimeout`, so `TimeoutMs` does not apply to it (pi keeps its timeout because its SDKs apply it outside fetch); documented on the exported field. gofmt/build/vet/`-race` green; in-repo differential request-body scenarios **38/38**. See the 2026-07-29 drift note); earlier pins — `47ca25fc` — "Revert \"fix(coding-agent): build-check-test (#7206)\"" (2026-07-28; **2 ports → 3 Go commits, 15 n/a, 0 decide** — delta `2efa728d..47ca25fc`, 17 first-parent changes; **no release crossed** — no tag in range, npm `pi-ai`/`pi-coding-agent` stay **0.82.1**, `models.generated.ts` + `src/data` unchanged, catalog + every byte-golden untouched, no regen. Ports: **minimum OAuth token validity on resolve** `99e34013` (Go `30c80c9` + review-fix `e073881`) — `AuthResolutionOverrides.MinOAuthValidityMs *int64` + a default five-minute window, so a stored OAuth credential expiring *within* the window refreshes on both the optimistic and under-lock checks; effective window is `max(default, override)` (an override can only widen), and the asymmetry is exact — the default window refreshes but imposes no post-refresh contract, while a *set* override (pi's `!== undefined`, so an explicit **zero** counts) additionally fails with pi's byte-exact `OAuth refresh returned a token that expires too soon for <provider>`; only `auth/resolve.ts` is in scope under the 2026-06-23 boundary-edge ruling, the CLI credential-print half + `core/model-runtime.ts` stay host-side, and `ModelRuntimeAuthOverrides` was checked against the 2026-07-21 lesson and carries **no Go obligation** (no `ModelRuntime` type in the port; it forwards to the already-ported `ai.AuthResolutionOverrides`). **send max_tokens for Z.AI providers** `2fe21b40` (Go `5e0b483`) — `isZai` joins the openai-completions `useMaxTokens` disjunction; **request-body surface**, and the existing differential `TestDiffDetectZai` pinned the OLD field, so its expectation moved — **re-derived from pi** (detectCompat at `2fe21b40` executed under node on that exact scenario) rather than bent to our output, with a 17-route sweep confirming only `maxTokensField` shifts and only on the four zai routes; the identical `generate-models.ts` hunk is generator-only, currently a no-op (catalog zai entries carry no `maxTokensField`, so detection governs) and folds into the next regen. **Both ports are unreleased upstream** — the shipped 0.82.1 build predates them, so the live request-diff harness cannot arbitrate; parity derived from TS source at the porting shas, executing pi's detectCompat under node across 17 routes vs Go's `detectOpenAICompat` (17 cases, 0 mismatches). Reviewed via independent go-review (**ship**; 1 MED applied — the exported `MinOAuthValidityMs` doc hid both the max() floor and the contract a set value arms — plus LOWs: phantom-caller comment corrected, `seedOAuth` folded into `seededStore`, `lockRaceStore` renamed `divergentReadStore` since it models divergent reads not a race, refresh counter made atomic) + independent adversarial parity review (**FAITHFUL**, no divergences — the `!= nil` gate, the `max(DEFAULT, override ?? 0)` truth table across nil/0/negative/equal/greater, and the error string all byte-verified programmatically; one coverage gap closed: the `!= nil` vs `> 0` boundary is now pinned by `TestResolveStoredOAuthZeroMinValidityArmsContract`, mutation-verified behavioral-red). gofmt/build/vet/`-race` green; in-repo differential request-body scenarios **38/38**. See the 2026-07-28 drift note); earlier pins — `2efa728d` — "fix(coding-agent): support concurrent user bash cancellation" (2026-07-27; **1 port → 1 Go commit, 8 n/a, 0 decide** — delta `5bc1c2c0..2efa728d`, 9 first-parent changes, all single-parent squashes; **no release crossed** — npm `pi-ai`/`pi-coding-agent` stay **0.82.1**, `models.generated.ts` unchanged, catalog + every byte-golden untouched, no regen. The one port: **pending stop reason while streaming** `f9a49869` (Go `3643c91`) — new `ai.StopPending StopReason = "pending"`; each ported provider (anthropic, openai-completions, openai-responses+shared, google, pi-messages, faux) inits its streaming partial to pending instead of stop and fails a stream that ends still-pending with pi's exact per-provider error string ("Anthropic stream ended without a stop reason", "OpenAI Responses stream ended without a stop reason", "Google stream ended without a **finish** reason", openai-completions `!hasFinishReason || pending`, "Faux response ended without a stop reason"); openai-responses additionally resolves pending→stop via `applyMessagePhaseStopReason` when a message item hits phase `final_answer`, called at both the createSlot (output_item.added) and output_item.done sites. `"pending"` is streaming-only and never persists to session JSONL, so no golden moves. The upstream `packages/agent/src/proxy.ts` streamProxy partial-init hunk is **subsumed** — Go has no streamProxy analog; the agent loop consumes provider `event.Partial` values (the only non-provider `AssistantMessage` construction, `agent/agent.go:413`, is a synthetic failure message correctly built with StopError/StopAborted). Reviewed via independent go-review (**ship**; 2 LOW applied — reattached createSlot's detached doc comment + an "unreachable today" note on the responses pending guard, which sits after the `sawTerminalResponseEvent` guard as defensive parity) + independent adversarial parity review (**FAITHFUL**, no divergences; every error string / guard-ordering / call-site byte-matched, proxy.ts subsumption independently verified, no out-of-scope provider ported; flagged four vacuous/absent test-coverage gaps — faux init line, pi-messages init, the responses createSlot call site, and the incomplete-overrides-provisional-stop interaction — **all four closed** with mutation-verified non-vacuous tests). gofmt/build/vet/`-race` green; in-repo differential request-body scenarios 14/14 (request bodies unperturbed — the change is response-side). See the 2026-07-27 drift note); earlier pins — `5bc1c2c0` — "Add [Unreleased] section for next cycle" (2026-07-26; **3 ports → 4 Go commits, 15 n/a, 0 decide** — delta `7df73a00..5bc1c2c0`, 19 first-parent changes; **release crossed — npm 0.82.0 → 0.82.1, one tag** v0.82.1 `b4f29368`, but the **catalog is byte-identical** to 0.82.0 (`models.generated.ts` unchanged across the whole range), so no regen ran and no byte-golden moved — the opus-5 `generate-models.ts` edits (`921c3543`, `af3b934f`) are generator-only, not yet regenerated. Ports: ANTHROPIC_AUTH_TOKEN bearer auth `24e5cc04` (8489218 + review-fix 7ebae86), `ModelsStoreEntry.etag` `b1c444d9` (853e88c), and ModelsError-cause **already-faithful/test-only** `4cf0a729` (18fc5e2 — Go's `ModelsError.Error()` has always composed code+message+cause). Both independent reviews caught a **precedence divergence** in the first port: `24e5cc04` replaces anthropic's generic `envApiKeyAuth` with a custom `anthropicApiKeyAuth` returning the token as an `Authorization: Bearer` header — ahead of the oauth/api-key envs but AFTER a stored/explicit credential; the initial port resolved it into the api-key slot and let the env token override an explicit key. Fixed in `7ebae86`: a custom Go resolver on the facade (`builtins_models.go`) + `withEnvAPIKey` leaves APIKey empty when the token is active + the provider re-reads it only when no key resolved (`apiKey == ""`), so credential-first precedence holds on BOTH the facade and compat paths, mutation-verified. See the 2026-07-26 drift note); earlier pins — `7df73a00` — "Add [Unreleased] section for next cycle" (2026-07-24; **4 ports → 5 Go commits, 17 n/a, 0 decide** — delta `34f3719a..7df73a00`, 21 first-parent changes; **release crossed — npm 0.81.1 → 0.82.0, one tag** v0.82.0 `083e6162`, catalog re-derived byte-identical **456,576 B** `cmp`-clean and endpoint-pinned both ends; ports: abortable provider retries `352416c` (7af8533c), constrained sampling `ddd2661`+`f9364ef`+`1758b71` (24bace27), explicit prompt-cache mode `ddd2661` (241431c6), catalog regen `c3d3fc4` (083e6162). The parity review + reconciliation sweep caught a **missed hunk** — 24bace27 threads `constrainedSampling` through `tool-definition-wrapper.ts`, which rides into `AgentTool` for free in TS via interface inheritance but had no Go home, leaving the feature implemented in `ai` and unreachable from `agent/`+`coding/`; fixed in `1758b71` alongside two more real divergences (jsNumber's exponential threshold is 1e-6 not 1e-7; `parseFloat` DOES accept the `Infinity` literal, inverting the `Retry-After: Infinity` outcome). Two silent request-body changes rode in with 24bace27 — openai-responses `supportsStrictMode` now defaults false (53 of 99 catalog responses models lose the `strict` key; the 46 `openai`-provider ones are unchanged) and function-call replay drops non-`fc_` item ids. Three deliberate divergences recorded (per-provider fail-fast scoping, `anthropicSDKErrorMessage`, and request-body key order — the last newly documented, not new). See the 2026-07-24 drift note); earlier pins — `34f3719a` — "chore: approve contributor christianbasch" (2026-07-23; **4 ports → 6 Go commits, 11 n/a, 0 decide** — delta `a5afc3f1..34f3719a`, 15 first-parent changes; **no release crossed**, npm `pi-ai`/`pi-coding-agent` stay **0.81.1** and the catalog byte-golden is untouched; ports: retry DNS transport failures `d265557` (33e40c3e), cache_control on tool-result messages `1c08770` (bc41f612), summarization isolation — cacheRetention none + fresh session id — `5c6f641` (9b3a2059), bash PI_* session metadata + bash promptGuidelines + system-prompt docs line `acb4077` (bb3d7d39); the bb3d7d39 port introduced a **data race** (live metadata read from the tool goroutine vs SetModel/Record on the main loop) caught by the independent go-review and fixed in `be304ae` + `9f7cac5` — model now read through the agent's mutex-guarded State(), recorder through recMu/recorder(), regression test mutation-verified to report DATA RACE against the pre-fix code; two new structural divergences recorded (no PI_SESSION_ID without a SessionRecorder; custom/Tools-override bash tools get no PI_* — pi's registerTool path does, via a ctx-passthrough hunk with no Go home); bb3d7d39 and bc41f612 are **unreleased upstream**, so the system-prompt golden was derived from pi's TS assembly order (independently re-derived by the parity review) rather than regenerated from the 0.81.1 build, and the live request-diff harness cannot validate this cycle's request-body changes — in-repo differential 38/38 covers regression. See the 2026-07-23 drift note); earlier pins — `a5afc3f1` — "feat(ai): add Kimi Code subscription OAuth login (#6935)" (2026-07-22; **report-only pin advance — 0 ports, 3 n/a, 0 decide**, no release / no catalog regen; delta `dd6bea41..a5afc3f1`, 3 first-parent changes: kimi-coding subscription **OAuth login** (`a5afc3f1`, #6935 — device-authorization-grant flow in `auth/oauth/kimi-coding.ts` + `load.ts`/`bun-oauth.ts` + provider `oauth: lazyOAuth` wiring in `providers/kimi-coding.ts`; OAuth **acquisition** is on the deliberate non-port list, the `OAuthAuth`/`LoginLabel`/`LazyOAuth` type surface is already latent in Go and wired to no provider, kimi-coding stays env-key auto-registered via `ai/envkeys.go` `KIMI_API_KEY`→`EnvAPIKeyAuth` — same shape as the 2026-07-17 xai OAuth login, no new boundary question), generate-models reasoning options from models.dev (`1ae06409`, #6928 — **generator** `generate-models.ts`+new `models-dev-reasoning-options.ts`+`.mjs` tooling+pi-internal tests, zero `packages/ai/src`/`agent/src`/`coding-agent/src/core` touch, folds into next regen), contributor approval mteam88 (`db647e42`); **no version bump** so npm `pi-ai`/`pi-coding-agent` stay **0.81.1** and every byte-golden (catalog, session tree, image, differential) is untouched. See the 2026-07-22 drift note); earlier pins — `dd6bea41` — "Add [Unreleased] section for next cycle" (2026-07-21; **6 ports → 8 Go commits, 33 n/a, 0 decide** — delta `8b937370..dd6bea41`, 42 first-parent changes; **release crossed — npm 0.80.10 → 0.81.1, two tags** v0.81.0 `9c480b6a` + v0.81.1 `20be4b18`, catalog re-derived byte-identical **431,732 B** `cmp`-clean; ports: tool_call_id item-level uniqueness `0e035b2`+golden `5f3eff3` (d9f7f814), credential-env passthrough on the env-key `d30903c` **and** ambient `912607a` resolvers (1942b260 — ambient half caught by the parity review), ModelsStoreEntry.lastModified `e261520` (54fad505), RetryAssistantCall retry loop `965a0ed`+nil-safe polish `24331ce` (65dd2e0e), compaction retainedTail reconstruction `3e68f4a` (9e7582aa), catalog regen + Qwen Token Plan `b4bb393` (releases + bbb91fa8); 2fd38684 usage-in-entries ruled **n/a** (no ported consumer — Go compaction in-memory + session tree read-only, usage-totals/footer/extension unported), 1235c0ec+b9e5c5d9 stream-fn decouple **n/a** (Go defaults `ai.StreamSimple` inline). See the 2026-07-21 drift note); earlier pins — `8b937370` — "kimi: add low,high to k3 and remove k2p7 references" (2026-07-20; **report-only pin advance — 0 ports, 10 n/a, 0 decide**, no release / no catalog regen; delta `3da591ab..8b937370`: pi-messages error-parse **behavior-neutral refactor** (`95607469` — Go's typed-unmarshal `parsePiMessagesErrorBody` already faithful), `uuidv7` relocated into `packages/ai/src/utils` + adopted by the unported codex provider (`d2f8dafb`, #6834 — Go already has its own `uuidv7` in `coding/session_store.go`), shared `contentText` util extraction (`94373d81`, #6840 — behavior-neutral, Go extracts text inline), opencode-go `openai-responses` capability (`75cb0b87` — Go routes by catalog `api`, no opencode model routes there yet), session-header-scan perf (`f1c587dd` — host session-discovery, no format change), kimi k3 low/high & k2p7-removal catalog-data (`8b937370`), + tui/packaging/meta; all 10 single-parent (no merge-smuggled hunks), whole-range `packages/ai/src`+`agent/src`+`coding-agent/src/core` sweep matches the per-commit sum. See the 2026-07-20 drift note); earlier pins — `3da591ab` — "feat(coding-agent): add Hugging Face llama search" (2026-07-18; **report-only pin advance — 0 ports, 5 n/a, 0 decide**, no release / no catalog regen; delta `a9f6a315..3da591ab`: copilot long-context pricing-tier **generator** fix (#6668, `ce48d9b4`), prompt-templates all-argument defaults (#6695, `64f83c85` — prompt-templates unported), a stale-model **buildfix** dropping `kimi-for-coding`/`kimi-k2-thinking`/`nvidia/llama-3.3-nemotron-super-49b-v1.5` from the catalog-data `.models.ts` + pi-internal vitest fixups (`5124c61b` — deferred to next regen), and the llama.cpp router **extension** + HF llama search (`f1a466b1`, `3da591ab` — extensions runtime unported); whole-range `packages/ai/src`+`coding-agent/src/core` sweep clean, no merge-smuggled hunks. See the 2026-07-18 drift note), `a9f6a315` — "feat(ai): separate generated model data (#6765)" (2026-07-17; **8 ports → 8 Go commits** — model-runtime facade SDK-scoped `08e0cde` (ff28097a), ModelsStore `checkedAt` entries `945696c` (bd9e09db), kimi deferred tools in openai-completions `2b07203` (f16b4e0c), xai default grok-4.5 + `OAuthAuth.LoginLabel` `ac9abd0` (a01baaae), retry Responses early EOF `6b4794c` (b0c2a90e), catalog regen → npm 0.80.10 `f1e4f31` (releases 2d16f929 + 8dc78834 + merge-carried fae7176c), xai encrypted-reasoning `include` `80ebacc` (merge-carried 5220aba6, **caught by the adversarial parity review** — first-parent triage missed it), `force` refresh flag `21a18cb` (merge-carried 97f9978f, same catch); 29 n/a; 0 decide — the one boundary question (facade scope) was ruled by the owner in-session: **SDK-scoped, recorded as the 2026-07-17 ruling**; **release crossed — npm 0.80.7 → 0.80.10, THREE tags** (v0.80.8's release commit `fae7176c` rode in via merge; v0.80.9 `2d16f929`, v0.80.10 `8dc78834`): catalog re-derived byte-identical (**420,411 B**) from the integrity-verified 0.80.10 build, endpoint-pinned both ends (0.80.7 derivation ≡ prior committed catalog, working tree ≡ 0.80.10 MODELS); no session/differential golden perturbed — differential 37/37); previous pins `dcfe36c7` (07-15; 2 ports — remove Current date from system prompt `f9b6a16`, catalog regen → npm 0.80.7 `b325abe`; 1 data-only folded into the regen — opencode `sessionAffinityFormat` `1f9e846c`; 4 n/a; release 0.80.6 → 0.80.7, catalog 416,889 B), `adfac437` (07-14; 2 ports — openai-responses reasoning `encrypted_content` backfill `592561f`, new pi-messages provider API `e46c9c1`; 1 already-faithful/no-code — anthropic skip-empty-usage-in-`message_delta`; 3 n/a; 1 decide ruled — pi-messages SDK API, Radius OAuth/host wiring out; no release, npm 0.80.6), `7303cbac` (07-13; 2 ports — forward Responses `tool_choice` `7a37fc8`, OpenRouter session-affinity format `8e258ee`; 3 n/a), `8479bd84` (07-11; 2 ports — deferred tool loading `bf103a3`, `<authenticated>` ambient-auth marker filter `4553fc1`; 1 already-faithful — cloudflare ambient account-id fallback `bdd5c53b`; 9 n/a), `81de5702` (07-10; 5 ports — ResourceExhausted retry `2bc26c1`, stale-usage-after-compaction `afcfb0e`, max thinking + input pricing tiers `6acea45`, anthropic empty-text signed thinking `5015a8e`, catalog regen → 0.80.6 `d321e2b`; release v0.80.4/5/6), `4285712b` (07-09; 1 port — retry Bun socket drops `369aca7`; 6 n/a), `312bc713` (07-08; 1 port — fail tool calls from length-truncated assistant messages `b5a73ea`; 7 n/a), `2b00dade` (07-07; 1 port — `(no tool output)` placeholder `279f53b0`; 1 already-faithful/no-code — null-content ingestion normalization `8c0ccd14`; 7 n/a), `647c5554` (07-06; 3 ports — retry Cloudflare 524 `d53b5676`, clamp openai-responses max-output-token floor to 16 `2e4ad6a0`, remove Vercel AI Gateway attribution `83cbfc65`; 21 n/a), `114bacf3` (07-02; report-only, 0 ports, 11 n/a), `8c943640` (07-01; v0.80.3 regen + bash-timeout `cbcf4e04`+`85b7c247`), `9be55bc7` (06-30), `541d11f7` (06-29), `5a073885` (06-27), `622eca76` (06-26), `1d486163` (06-25), `09f10595` (06-25), `a2e3e9d8` (06-24), `470a4736` (06-23), `3b561346` (06-22), `2417adb4` (06-21), `56b22768` (06-19), `29c1504c` (06-17). The models-runtime migration is now **complete**: the `732bb161` substrate (06-23) plus the 06-24 follow-through (catalog-data reorg landed via the 0.80.2 regen; request-scoped auth `ef231c49`; api_key/env credential `49fbe683`; OpenAI Responses terminal events `cd95c274`; anthropic compat→catalog `6184307c`; header-only client auth + vercel ungate `129eb460`). |
| npm build the byte-goldens were captured from | still `@earendil-works/pi-ai` **0.83.0** at the 2026-08-05 pin `651d5d6a5` (no release crossed `b784c8096..651d5d6a5` — no tag in range, every `packages/*/package.json` still 0.83.0, `models.generated.ts` and `image-models.generated.ts` untouched, so the catalog and every catalog-derived byte-golden are unchanged and no re-derivation ran. Two `ai/scripts/generate-models.ts` edits are queued for the next regen — see the pin row. All four ports are unreleased upstream, so the 0.83.0 build could not arbitrate any of them; the ONE golden that moved is the wire fixture `protocol/testdata/upstream_messages.json`, derived by executing upstream's own `gen-messages.ts` from the TS at `dab70b2cd` under `node --experimental-strip-types` with integrity-checked `typebox@1.3.7`, non-circular at both ends and independently re-derived by the parity gate). Prior pin: still `@earendil-works/pi-ai` **0.83.0** at the 2026-08-04 pin `b784c8096` (no release crossed `c6eb6281a..b784c8096` — no tag in range, every `packages/*/package.json` still 0.83.0; `models.generated.ts` moved ONLY to register Baseten (an import + two map entries, no data), and `ai/scripts/generate-models.ts` moved (+105), which under the 2026-07-30 ruling signals the NEXT release will move catalog data. Catalog + every derived byte-golden untouched, no re-derivation ran. All 14 ports are unreleased upstream, so the 0.83.0 build could not arbitrate any of them; the wire goldens for `305c014dc` were instead regenerated by executing upstream `codec.ts` at that sha under `node --experimental-strip-types` with integrity-checked `typebox@1.3.7`, and proved NON-CIRCULAR — the committed generator run against `305c014dc^1` reproduces the committed pre-change fixture byte-for-byte). Prior pin: still `@earendil-works/pi-ai` **0.83.0** at the 2026-08-03 pin `c6eb6281a` (no release crossed `01eeafd14..c6eb6281a` — no tag in range, no `packages/*/package.json` moved, and `models.generated.ts` / `image-models.generated.ts` / `generate-models.ts` are all untouched, so the catalog and every derived byte-golden are unchanged and no re-derivation ran). Prior pin: still `@earendil-works/pi-ai` **0.83.0** at the 2026-08-03 pin `01eeafd14` (no release crossed `ab366ebe9..01eeafd14` — no tag in range, every `packages/*/package.json` still 0.83.0, and `models.generated.ts`, `image-models.generated.ts` and `ai/scripts/generate-models.ts` are all untouched, so the catalog + every derived byte-golden are untouched and no re-derivation ran. All five ports this cycle are **unreleased upstream**, so the 0.83.0 build predates them and could not arbitrate any of them — the **wire goldens** were instead derived by executing upstream's own `gen-framing.ts` / `gen-messages.ts` / `gen-vectors.ts` from the TS at `03eba409c` under `node --experimental-strip-types` with `typebox@1.3.7`, whose lock integrity was matched against `npm view typebox@1.3.7 dist.integrity`. The two regen signals recorded on 2026-08-02 — `a688e257c` (Fireworks Kimi K3 via OpenAI compat) and `fbab971da` (z.ai `glm-5.1`→`glm-5.2`, `glm-4.5-air` dropped) — remain pending for the next publish; this delta adds none.) Prior pin: still `@earendil-works/pi-ai` **0.83.0** at the 2026-07-31 pin `ab366ebe9` (no release crossed `c13ffe18..ab366ebe9` — no tag in range; `models.generated.ts` unchanged, so the catalog + every derived byte-golden are untouched and no re-derivation ran. Note all three ports this cycle are **unreleased upstream**, so the 0.83.0 build predates them and could not arbitrate any of them — the google request-body change was instead derived by executing pi's `convertMessages` from the TS at `6138f5a07` under node. Reference-build authenticity was re-verified anyway: `package-lock` integrity for `pi-ai@0.83.0` and `pi-coding-agent@0.83.0` both match `npm view … dist.integrity`). Prior pin: `@earendil-works/pi-ai` **0.83.0** at the 2026-07-30 pin `c13ffe18` — release crossed (`845d6ff1`), and this is the **first catalog move since 0.82.0**: re-derived as `JSON.stringify(MODELS)` from the integrity-verified 0.83.0 build (**477,229 B**, 1153 models, 37 providers) and **endpoint-pinned both ends** — the same derivation against the 0.82.0 build reproduces the previous catalog byte-for-byte (456,576 B ≡ the file at `162c6fa`), and the 0.83.0 derivation is `cmp`-clean against HEAD. **No schema drift**: a full key+type enumeration of every model and nested object (`compat`, `cost`, `cost.tiers[]`, `headers`, `thinkingLevelMap`) diffs empty across the two catalogs, so `ai/types.go` is unchanged; the explicit-`null` `thinkingLevelMap` entries decode correctly through `map[level]*string`. `pi-coding-agent` **0.83.0** (no session-format / image-decision / system-prompt change crossed, so those goldens are untouched — `rawStopReason` is additive-optional and absent from every fixture). **Procedural note**: `dist/models.generated.js` is byte-identical (4,373 B) between 0.82.0 and 0.83.0 — it is an aggregator over `dist/providers/data/*.json`, so it must be **executed**, never `cmp`-ed, to decide whether a regen is needed (see the 2026-07-30 ruling). Prior pin: still **0.82.1** at the 2026-07-29 pin `cced6a21` (no release crossed `47ca25fc..cced6a21` — no tag in range; `models.generated.ts` and `src/data` unchanged, so the catalog + every derived byte-golden are untouched and no re-derivation ran; neither port touches request bodies or any persisted format). Prior pin: still **0.82.1** at the 2026-07-28 pin `47ca25fc` (no release crossed `2efa728d..47ca25fc` — no tag in range; `models.generated.ts` and `src/data` unchanged, so the catalog + every derived byte-golden are untouched and no re-derivation ran. Note both ports this cycle are **unreleased upstream**, so the 0.82.1 build predates them and could not arbitrate either change). Prior pin: still **0.82.1** at the 2026-07-27 pin `2efa728d` (no release crossed `5bc1c2c0..2efa728d`; `models.generated.ts` unchanged, so the catalog + every derived byte-golden are untouched and no re-derivation ran). Prior pin: `@earendil-works/pi-ai` **0.82.1** (catalog **byte-identical** to 0.82.0 — `models.generated.ts` is unchanged across `7df73a00..5bc1c2c0`, so the **456,576 B** catalog golden and every derived byte-golden are untouched and no re-derivation was run this cycle); `pi-coding-agent` **0.82.1** (no session-format / image-decision / system-prompt change crossed, goldens untouched). Prior pin: `@earendil-works/pi-ai` **0.82.0** (catalog re-derived byte-identical from the build's `dist/models.generated.js` MODELS — **456,576 B**, `cmp`-clean vs an independent fresh-process re-derivation, lock integrity verified against the registry — `sha512-8MvW9+zno13sXDuT2kFMnWeTNUufUhPeZDRVO+igGoBRCDWgn7Xh2FkRQI1mRuet6QhF4ENQuLYdIAOyG6BhNw==`; 1098 → 1116 models, +19/−1, the removed `nvidia/mistralai/mistral-large-3-675b-instruct-2512` confirmed unreferenced in Go source; schema drift is exactly three compat booleans — `supportsStrictTools` (14 models), `supportsOpenAIGrammarTools` (101), `supportsExplicitPromptCacheMode` (3) — all consumed; model key order churns within 6 providers from 5dc40fee's api-group assembly, provider order unchanged; note `ab55c15f` replaced `getBuiltinModelDataUrl` with a `data/.manifest.json` import but `dist/models.generated.js` still exports a flat MODELS (37 providers), so the derivation path is intact); `pi-coding-agent` **0.82.0** (integrity `sha512-Qnqgn9zhJFQ2HZ8R4iNuGhyCk93XX6+eUw9i+TjTuo47amzCy93ft3bB6yaUCleCrNO58dJDHYSGNHv/GAPWKg==`; no session-format / image-decision / system-prompt change crossed, so those goldens are untouched). Prior pin: `@earendil-works/pi-ai` **0.81.1** (catalog re-derived byte-identical from the build's `dist/models.generated.js` MODELS — **431,732 B**, `cmp`-clean vs an independent fresh-process re-derivation; adds the Qwen Token Plan providers, gpt-5.6 context window, new Gemini/Kimi models; `pi-coding-agent` **0.81.1** — the compaction retainedTail session-format addition is additive-latent, so existing session/image/differential goldens hold. Prior pin: **0.80.10** (catalog re-derived byte-identical from the build's `dist/models.generated.js` MODELS — **420,411 B**, `cmp`-clean vs an independent fresh-process re-derivation, lock integrity verified against the registry — `sha512-Moe/H8c87…F+UKUA==`; subsumes 0.80.0–0.80.9 incl. the unreleased-to-main-line 0.80.8); `pi-coding-agent` **0.80.10** (integrity `sha512-aL4apbupC…4wUcQg==`; no session-format / image-decision / system-prompt change crossed 0.80.7→0.80.10, so the session tree, image-decision, and differential goldens are untouched) |
| Parity proofs at the pin | Unchanged at `c6eb6281a` — the cycle ported nothing, so every corpus and golden still stands exactly as proved at `01eeafd14`, re-verified only by the full green gate (`go build`/`go vet`/`go test -race ./... -count=1`). Prior pin (`01eeafd14`): **The wire is a new golden class this cycle, and it was proved non-circular**: an adversarial reviewer independently regenerated all three corpora — `protocol/testdata/upstream_framing.json`, `upstream_messages.json` and `protocol/cbor/testdata/upstream_vectors.json` — from `git archive 03eba409c packages/protocol/src` plus an integrity-verified `typebox@1.3.7`, and got output `diff`-clean against the committed files, so the corpus is genuinely upstream-derived rather than pinned to our own output. **38 differential wire frames** pushed through both Node's `ServerMessageDecoder` and Go's agreed on every accept/reject verdict, covering all of `03eba409c`'s status / `stopReason` / `isError` invariants and the `item_finished` narrowing. **26/26 upstream remote-session test scenarios** map to Go counterparts, with no upstream scenario unported. **Two new fuzz targets** (`FuzzClientDecoder`, `FuzzServerDecoder`) ran **26.0M + 25.2M executions** with no crashers, asserting not just absence of panics but that anything accepted re-encodes and that re-encoding is a fixed point across a relay hop. Cross-builds green for `GOOS=windows`, `GOARCH=386` and `GOARCH=arm64` (386 was newly broken this cycle and is fixed — see the drift entry). Prior: **Live pi-vs-Go openai request diff 6/6 scenarios, 3324/3324 model-cases** — all 554 openai-completions catalog models × 6 scenarios (basic / tools / reasoning / multi-turn thinking+toolCall+toolResult / image+long-cache+sessionId / grammar tools at both strictness levels), both sides regenerated from scratch (pi through `options.onPayload`, Go through a `replace`-linked module halting pre-network), canonical key-sorted JSON, incl. **72 byte-identical error-path messages**; a first pass showed 108 diffs on 18 Cloudflare models and was traced to the *harness*, not the port (Go resolves the `CLOUDFLARE_ACCOUNT_ID` base-URL placeholder before `OnPayload`, pi after) — 3324/3324 with the env var set · the google change validated by executing pi's `convertMessages` from the TS at `6138f5a07` under node across **29 google models × 7 scenarios (203 cases)**: 4 scenarios moved on 29/29 models with new Go ≡ pi and old Go ≠ pi, the 2 negative scenarios correctly unmoved — non-vacuity by construction rather than by assertion · the cross-model thinking branch proven **unreachable on both sides** (transformMessages ordering + `isSame` strictly weaker than `isSameModel`; panic-substituted branch body leaves `ai/providers` green) · `supportsFinishReason`'s absent DATA half proven inert — zero occurrences in upstream `models.generated.ts` at `origin/main`, never `false` anywhere upstream, 0 in the Go catalog, and `getOpenAICompat` verified live to return the detected profile unchanged on empty **or malformed** `model.Compat` · the openai guard ordering probed empirically (unknown `finish_reason` → error regardless of compat; zero-choice + compat false → `stop`; tool-call + compat false → `toolUse`; `finish_reason: null` treated as absent) and shown **moot in both implementations** — the only setter of stop-reason `error` is the finish-reason mapper, so inference can never mask a real error · anthropic seeding verified to preserve delta APPEND semantics (`WriteString` prefix, `+=` on `thinkingSig`) with the seed present in the `text_start`/`thinking_start` partial · four new tests re-run against `7899c6e` and confirmed **behaviorally red**, the two negative tests correctly green · all three ported commits are merges and were re-derived with `git diff <sha>^1..<sha>` rather than the combined `git show` view — **no smuggled hunks** · goldens: none moved and none should have (`models.generated.ts` untouched upstream; no fixture feeds a signed-but-empty block; every existing `content_block_start` fixture sends `"text":""`) · gofmt clean, build/vet/`-race` green. Earlier pin: Catalog **endpoint-pinned at both ends** by executing the ESM module (not reading it): 0.82.0 `MODELS` ≡ `git show 162c6fa:ai/models_catalog.json` (456,576 B) AND 0.83.0 `MODELS` ≡ HEAD (477,229 B), both `cmp`-clean, reference-build authenticity verified via `package-lock` integrity == `npm view dist.integrity` for both packages at both versions · the `cmp`-the-aggregator trap confirmed real — `dist/models.generated.js` is byte-identical across the two versions while the underlying data differs by 20,653 B · full key+type enumeration across both catalogs **diffs empty** (no schema drift), every compat key mapped to a Go struct field across all three api families · all 27 removed ids grepped repo-wide (only synthetic-literal hits) and all 38 `defaultModelPerProvider` entries confirmed to resolve · **live pi-vs-Go request diff 8310/8310** — every openai-completions model in the catalog (554) × 15 scenarios (basic / tools / reasoning / multi-turn with thinking+toolCall+toolResult / image+long-cache+sessionId / grammar tools at both strictness levels / all 8 reasoning-effort levels / tools+xhigh+cache), both sides regenerated from scratch (pi driven through `options.onPayload`, Go through a `replace`-linked module halting pre-network), canonical key-sorted JSON, error paths included (72 byte-identical failure messages) — and **not vacuous**: 87 case-bodies genuinely move between the 0.82.0 and 0.83.0 catalogs and Go reproduces all 87 · the regen proven **behaviorally required** by crossing 0.83.0 code against 0.82.0 data, which emits `reasoning_effort` to precisely the 9 ids `4c1a0b92` excludes · the `??` vs `=== undefined` split independently re-derived from `dist/api/openai-completions.js` (qwen line 574 vs zai 564-565) and both branches proven to behave differently and each match pi · `supportsReasoningEffort` merge (`model.compat.… ?? detected.…`) confirmed identical for qwen-token-plan/-cn/opencode-go empirically, not by reading · every `Provider stopped with: …` string byte-compared against the shipped `anthropic-messages.js:1041` / `google-generative-ai.js:209`, and the shared `providerStoppedPrefix` const hex-verified (23 B, trailing space intact) · assignment **ordering** matched — pi sets `rawStopReason` before calling the mapper, so a mapper throw still leaves it set; Go does the same at all three sites · the `response.failed` clear-then-set verified against `openai-responses-shared.js:616`, an unconditional assign that CLEARS on an absent response · `name()`'s `??` reproduced exactly and verified 17/17 through the real dist `stream()` with injected SSE, incl. `{"function":{"name":""},"custom":{"name":"c"}}` → `""` (pre-fix Go returned `"c"`) and multi-chunk first-wins accumulation · `isGrammarCall()` exhaustively evaluated over its full 8-state input space at both call sites (0 mismatches, true no-op) · goldens: none moved and none should have — sessparity uses fictitious provider/model ids with no catalog lookup, imgparity is pure resize decisions, the system-prompt golden carries no model metadata, and `rawStopReason` is absent from every fixture · mutations verified **behavioral-red** on throwaway copies for every behavior change (the `RawStopReason = ""` line, both `isGrammarCall` sites, the `name()` `??`, and the two qwen-token-plan resolver entries) · gofmt clean, build/vet/`-race` green · in-repo differential 38/38. Earlier pin: The google rejection string byte-compared programmatically against pi's TS literal (66 bytes, `cmp`-clean in both the source and the test), and its guard position proven to precede the api-key check by mutation (moving it below flips the reported error) · every hunk of both upstream changes enumerated and given a Go home or a justified absence, incl. the five unported-provider hunks of `027a5847` and `ImagesOptions.fetch` (no images surface) · `simple-options.ts`'s passthrough proven to need no counterpart by confirming `SimpleStreamOptions` embeds `StreamOptions` and every consumer copies it wholesale — the one field-by-field copy, `agent/loop.go`, was the HIGH both reviews caught · pi-messages' `http.DefaultClient` default confirmed to be exactly the pre-change line · `findGitPaths` compared line-by-line against pi's incl. every error path (stat throwing vs ENOENT, readFile throwing, a `.git` file with no `gitdir: ` prefix, a missing HEAD), `resolveFrom` checked against Node `path.resolve`, and the `headPath` omission justified by its unported footer consumer · all nine upstream worktree scenarios ported with identical expected content lists AND order · ten mutations verified **behavioral-red** on isolated copies (retry-loop override, pi-messages override, google guard removed, google rejecting the default too, shadow check disabled, canonicalization at the comparison site, the separator in the prefix check, the commondir identity check, the agent forward line, and all four `canonicalizePath` call sites at once) — an earlier compile-fail mutation was redone as a real edit, since a compile-fail red does not count · goldens: none moved and none should have — no request-building file changed (`ai/providers/openai*.go` zero-byte diff), `git diff --stat` over `coding/testdata` empty, and no golden fixture uses a worktree layout · in-repo differential **38/38** · gofmt clean, build/vet/`-race` green · pi's `detectCompat` extracted from the TS at `2fe21b40` and **executed under node** across 17 provider/URL routes, compared field-by-field against Go's `detectOpenAICompat` — **17 cases, 0 mismatches**; the same before/after sweep proves only `maxTokensField` changes and only on the four zai routes, and re-derives `TestDiffDetectZai`'s moved expectation from pi rather than from our output · the OAuth error string byte-compared programmatically against pi's template literal with the provider-id sentinel substituted (EQUAL, and the Go test literal EQUAL too) · the `max(DEFAULT, override ?? 0)` truth table reproduced for nil / 0 / negative / == default / > default — identical to pi in all five, including that a zero or negative override leaves the default in force yet **still arms** the post-refresh contract (pi `!== undefined`) · every `Expires:` fixture in the Go auth tests audited to confirm none sat in the new five-minute window asserting no-refresh, so pi's `oauth-auth.test.ts` fixture bumps need no Go counterpart · `ModelRuntimeAuthOverrides` confirmed to carry no Go obligation (no `ModelRuntime` type in the port; zero non-test grep hits) · goldens: none moved and none should have — `ai/models_catalog.json` untouched and confirmed correct (no generated-model file changed upstream in the range), and no session-tree, image-decision, system-prompt or request-body golden contains `zai`/`max_completion_tokens` (repo-wide grep, zero hits) · six mutations re-run independently on a copy, all caught, no vacuous test · in-repo differential **38/38** · gofmt clean, build/vet/`-race` green · Six differentials regenerated from scratch against the real 0.82.0 build by the independent parity review — openai-responses 13/13, anthropic 3/3, google 11/11, openai-completions 7/7, plus two LIVE SSE streams (responses 11/11, completions 7/7); this cycle's request-body changes ARE released, so unlike 2026-07-23 the live pi-vs-go harness could validate them · 15/15 `jsEscape` and 15/15 grammar-buffer vectors against pi's real `dist/api/constrained-sampling.js` (astral pairs, lone surrogates, control chars, `<>&`, U+2028/9, monotonicity + re-close error paths) · 19 error bodies against the real `@anthropic-ai/sdk` 0.91.1 and `openai` 6.26.0 `APIError.generate`, confirming the two SDKs share a byte-identical `makeMessage` and diverge only in `generate` (openai unwraps `errorResponse["error"]`, anthropic passes the whole body) — `anthropicSDKErrorMessage` + `jsStringify` match byte-for-byte incl. key ORDER and JS number canonicalization (1.0→1, 1e21→1e+21, −0→0, 1e-8 not 1e-08) · the 46/53 `supportsStrictMode` census independently re-derived, and BOTH halves of the moved expectation reproduced from the real build (`strict` absent on copilot/xai, `strict:false` present on gpt-5.6-sol) — confirming it is pi-derived, not bent toward our output · `*string` for `responsesItem.Input` proven REQUIRED (real pi distinguishes absent → fallback from `""` → used-then-fails-non-monotonically), and openai-completions proven NOT to need it (pi reads that field with truthiness, so absent ≡ empty by construction) · retry fail-fast scoping ruled correct — pi's Google provider has no retry code at all · whole-range reconciliation file-by-file (26) and commit-by-commit (21), merge `ab55c15f` expanded in full · catalog endpoint-pinned both ends, `cmp`-clean, 1116 models loading in a scratch module matching the JSON inventory exactly · goldens: none moved and none should have (constrained sampling is request-side only, never persisted; `git diff --stat` over `coding/` is empty) · differential parity **38/38** · gofmt clean, build/vet/`-race` green · DNS retry patterns byte-identical to pi's and inserted at pi's source position, mutation-verified non-vacuous (no pre-existing pattern matches getaddrinfo/ENOTFOUND/EAI_AGAIN) · tool-role `cache_control`: pi's two role-check hunks proven redundant via `addCacheControlToMessage`'s single call site, so Go's collapsed loop carries both; mutation-verified (reverting puts the marker back on the user message) · summarization isolation: `createSummarizationOptions` confirmed to carry neither `sessionId` nor `cacheRetention` at `9b3a2059^1`, so pi's before→after delta equals Go's; explicit `CacheNone` confirmed to win over the `PI_CACHE_RETENTION` default path; distinctness (not just non-emptiness) asserted across two requests · bash PI_* metadata: guideline + docs-line strings byte-compared programmatically against pi; guideline placement re-derived from pi's TS assembly (`_rebuildSystemPrompt` tool-order → between read's and edit's) since the 0.81.1 build predates the commit; system-prompt golden changed in exactly the two lines upstream changed; strip-then-assign env semantics mutation-verified four ways · data race closed and independently re-probed by the reviewer (bash vs SetModel+SetThinkingLevel, and bash vs Record — both clean after the fix, DATA RACE before) · differential parity **38/38** · gofmt clean, build/vet/`-race` green · Model-runtime facade semantics matched line-by-line against pi models.ts/auth (refresh / resolveRefreshCredential / checkProviderAuth / getAvailable / login / logout / applyAuth / mergeHeaders / createProvider baseline+dynamic overlay + `checkedAt` stamping), error strings byte-matched, `??`→zero-value and `Modify` `next ?? current` post-write contracts verified; unconfigured-provider stream now errors with pi's `Provider is not configured:` message (test-locked) · kimi deferred tools: filter from untransformed context.messages, per-run insertion-ordered set, `role:system`+`tools` (no content) after the whole tool-result run (incl. after the image user message), all-deferred fall-through to the tool-history empty array, `getToolsByName` last-wins/unknown-skip — all 1:1 with pi; filter + emission mutation-verified behavioral-red; **live at this regen** (moonshotai/-cn kimi-k3 carry `deferredToolsMode:"kimi"`, pinned by TestDeferredToolsKimiCatalogLive) · xai `include:["reasoning.encrypted_content"]` on every reasoning branch of the responses builder (5220aba6 — reviewer-demonstrated live against the shipped build; grok-4.5 = new xai default routes through responses), mutation-verified · `force` refresh flag threaded to the main provider call only (cache-restore stays force-free, matching pi's error path), test-locked · retry early-EOF pattern + test message byte-identical to pi (and pairs with Go's own responses terminal-event error), mutation-verified non-vacuous · catalog regen byte-identical (**420,411 B** at npm 0.80.10, independently re-derived, `cmp`-clean, endpoint-pinned both ends; schema drift = only `compat.deferredToolsMode` + `compat.allowEmptySignature`, both consumed; all 5 removed xai ids unreferenced after the default re-point, mutation-traced through buildFallbackModel) · session tree 8/8 · image decisions 8/8 · in-repo differential parity **37/37** · gofmt clean, build/vet/`-race` green · Responses `tool_choice` option emitted verbatim iff set (nil ↔ pi's `!== undefined`), positioned after tools / before reasoning byte-identical to pi — mutation-verified non-vacuous (`tool_choice=<nil>` on revert) · OpenRouter session-affinity header shapes byte-exact on both providers (completions: openai→`session_id`+`x-client-request-id`+`x-session-affinity`, openrouter→`x-session-id`, openai-nosession drops `session_id`; responses same minus `x-session-affinity`), auto-detect (`provider==openrouter || baseURL⊇openrouter.ai`) byte-identical to pi's `detectSessionAffinityFormat`, `?? detected` ↔ pointer-nil merge, default non-openrouter path byte-unchanged — two auto-detect defaults + the openrouter branch mutation-verified non-vacuous in an isolated worktree · deferred/message-anchored tool loading (anthropic `tool_reference` + openai-responses `tool_search`) is request-body surface but a byte-level no-op for every existing golden — paths double-gated on model support + `AddedToolNames`, absent from all differential/session/image scenarios (37/37 hold); `pi_tool_load_` shortHash call-id mutation-verified byte-identical to pi's JS incl. astral surrogate pairs; `defaultSupportsToolReferences` version-gate mutation-verified (minor≥5 boundary pinned via a `4-4`→false case, haiku/4.0/4.1 false); `<authenticated>` ambient-marker filter mutation-verified (amazon-bedrock ambient path not injected as a key); `ai/models_catalog.json` unchanged (no release; `supportsToolSearch` catalog data deferred to next regen) · catalog regen byte-identical (**416,889 B** at npm 0.80.7, independently re-derived from the build's MODELS and `cmp`-clean, endpoint-pinned both ends; every model/cost/tier JSON key maps onto the Go `Model`/`ModelCost`/`ModelCostTier` structs — no unknown-key drop, no type-abort) · session tree 8/8 · image decisions 8/8 · in-repo differential parity **37/37** (the anthropic empty-text signed-thinking port is a request-body change but no differential scenario drives an empty-text signed block; the responses cache-write usage + cost-tier ports are not request-body — Usage isn't serialized in any golden and no scenario clears a tier threshold) · retry-classifier `ResourceExhausted` mutation-verified non-vacuous (NVIDIA-NIM message matches only via the new entry) · stale-usage estimate mutation-verified non-vacuous (old backward-walk fails the inserted-prefix case) · thinking-`max` opt-in + hole-between-high-and-max clamp mutation-verified · cost-tier boundary mutation-verified (272000 not `>` 272000 → base rates; 272001 → tier) · responses cache-write (input 15 = 20−2−3, cacheWrite 3) mutation-verified · anthropic empty-text signed thinking block mutation-verified (old drop-on-empty fails) · gofmt clean, build/vet/`-race` green · fireworks/cf anthropic compat coupling still 0 mismatches |
| Reviewed via | 2026-08-03 pin-advance to `c6eb6281a` — pi-triage over the 3-change drift only; no ports, so no pi-go-review / pi-parity-review gates were needed. Prior pin (`01eeafd14`): 2026-08-03 remote-session cycle — **nine independent gates**: pi-parity-review AND pi-go-review over each of `protocol/`, `client/`, `server/`, `coding/`, plus pi-triage over the drift. Every area returned **diverges** / **fix-first**: protocol (1 HIGH go, 4 MED parity, 3 MED go), client (1 HIGH found independently by BOTH gates, 1 further HIGH, 6 MED), server (1 HIGH parity, 3 HIGH go, 13 MED), coding (1 HIGH parity, 1 HIGH go, 6 MED). All fixed and test-locked before the pin moved. Prior: 2026-07-30 v0.83.0 cycle (3 ports + catalog regen, release crossed) independent go-review (**ship**; 2 MED + 4 LOW, all applied in `6ade7a1` — MED: the `response.failed` `RawStopReason = ""` clear was live and load-bearing yet **unpinned** (deleting it left the whole suite green while a failed response would retain a stale `"completed"`), and the grammar-call predicate was duplicated across two call sites with its explanatory comment living at neither; LOWs: the unreachable google fallback documented as if live, `"Provider stopped with: "` drifting as an untyped literal in two files, a non-`t.Run` loop inconsistent with every other test in the diff, and a six-line comment explaining code that isn't there. It **validated two porter judgment calls** rather than rubber-stamping them: omitting pi's `typeof effort === "string"` guard is correct because Go's type model *eliminates* it rather than hiding a gap, and the unreachable google fallback should be retained-but-recommented, not deleted or fake-tested) + independent adversarial parity review across **two passes** (**FAITHFUL**; pass 1 passed all three code ports 6/6 on the §3 request diff but raised a **HIGH release gate** — the catalog had not been regenerated, so shipping would produce a state no published pi ever had; pass 2, after the regen and the review fixes, endpoint-pinned the catalog at both ends, ran the request diff at **8310/8310** with 87 genuinely-moved case-bodies all reproduced, verified the `name()` fix 17/17 against the real dist, and proved `isGrammarCall`/`providerStoppedPrefix` true no-ops. It surfaced two **pre-existing** divergences neither commit introduced: a MEDIUM — `coding/resolve.go` missing three of pi's 38 `defaultModelPerProvider` entries, materially wrong for qwen-token-plan fallbacks — and a LOW — a comment misdescribing pi's Cloudflare `{VAR}` handling as a `createClient` throw when pi resolves it in `cloudflareStreams` and never throws. Both fixed in `b293a70`, the former with a test that locks the *consequence* (inherited context/token limits), mutation-verified behavioral-red. It also corrected a count in the porting brief — 9 models gain `thinkingLevelMap`, not 6); 2026-07-29 cycle (2 ports, no release) independent go-review (**fix-first → ship**; 1 HIGH — the injected HTTP client never reached the providers from the agent SDK, since pi gets it free from `AgentLoopConfig extends SimpleStreamOptions` while Go's loop forwards `StreamOptions` field-by-field under a comment demanding every field be listed; 1 MED — two different definitions of "no override", so `http.DefaultClient` silently displaced `sharedClient` and its `ResponseHeaderTimeout`; plus a MED on reading a whole context file to take its basename and LOWs on a redundant conversion, a non-atomic httptest flag, a near-vacuous shared-client assertion, and three near-identical provider tests missing an openai-responses row — all applied) + independent adversarial parity review (**fix-first → FAITHFUL**; found the same HIGH independently, plus the `http.DefaultClient` MED, a `findGitPaths` stat-error path that aborted the walk where pi's `existsSync` keeps climbing, and three coverage gaps. The `TimeoutMs`-under-injection divergence was raised as a MED and its disposition — record rather than reproduce — **accepted explicitly on the condition that it land in this ledger**, not the doc comment alone. Re-review of the fix commit confirmed all of it, independently re-running the mutations, and byte-verified that the `customHTTPClient` indirection left the google guard and pi-messages semantically identical; it also confirmed from `agent-session.ts` at `cced6a21` that pi's coding-agent options do **not** extend `SimpleStreamOptions`, so leaving `coding/session.go` untouched is correct rather than a second missed half. Two further LOWs surfaced by the fix commit itself and were closed: the name-only `contextFileNameInDir` probe used `fileExists`, which accepts a mode-000 candidate that `loadContextFileFromDir` and pi both skip — so a worktree with an unreadable `AGENTS.md` beside a readable `CLAUDE.md` would have shadowed the wrong file and dropped the main repo's content, fixed by keeping ONE candidate-selection rule and reverting that polish; and the `http.DefaultClient` normalization was correct but unpinned — reverting `retryFromOptions` to the pre-fix line left the whole `ai/providers` suite green. Both now mutation-verified behavioral-red); 2026-07-28 cycle (2 ports, no release) independent go-review (**ship**; one MED applied — the exported `AuthResolutionOverrides.MinOAuthValidityMs` doc understated twice, omitting that the effective window is `max(this, 5m)` (so a smaller value cannot relax when a refresh triggers) and that setting it at all arms an `ErrOAuth` failure the nil default never had; a consumer passing a small value to "relax" the check would have got a new error class instead. 5 LOWs applied — a comment citing a bearer-token-export caller that is host-side and unported, `seedOAuth` folded into the existing `seededStore` helper (killing a fifth uncommented `_, _ =`), `lockRaceStore` renamed `divergentReadStore` since it is single-goroutine and models divergent reads rather than a race, and the refresh counter made `atomic.Int64` since it is incremented inside the store's modify callback. The `*int64` modeling was challenged and **upheld** — the three states (nil / set-below-default / set-above-default) are all observable, and `time.Duration` would be wrong since the value is summed with `Credential.Expires` int64 epoch-millis in a package whose entire time vocabulary is millis-as-integer; the one-line `useMaxTokens` disjunction upheld as the right shape for the next sync's diff; hoisting a shared `expiresWithin` helper next to `models_runtime.go`'s bare expiry checks noted as a follow-up but not taken, since converging those semantics is a parity question, not a Go-polish one) + independent adversarial parity review (**FAITHFUL**, no divergences — every hunk of both upstream changes given a Go home or a justified absence; the `!= nil` gate confirmed not `> 0`/zero-value, the `Math.max` truth table and the error string byte-verified programmatically, the moved differential expectation independently re-derived by running pi's `detectCompat` under node, and a 17-route field-by-field compat comparison returning 0 mismatches; `ModelRuntimeAuthOverrides` explicitly checked against the 2026-07-21 missed-half lesson and cleared. One LOW coverage gap found and **closed**: the `!= nil` vs `> 0` boundary was unpinned — pi can reach an explicit zero via its `--min-expiry 0ms` CLI, where `!== undefined` still arms the contract — now locked by `TestResolveStoredOAuthZeroMinValidityArmsContract`, mutation-verified behavioral-red); gofmt/build/vet/`-race` green, in-repo differential 38/38; initial port + parity sweeps 1–2 (`3be3911`), registration fix (`b09cb46`); 2026-06-22 v0.79.10 cycle; 2026-06-24 v0.80.2 cycle independent go-review (ship, 3 optional LOW nits) + adversarial parity review (all 7 commits faithful, 6/6 differential, all 3 deliberate divergences confirmed observably-faithful); 2026-06-25 cycle (5 ports, no release) independent go-review (ship; one LOW `strings.Join` cleanup applied) + adversarial parity review (all 5 faithful; responses test-change mutation-verified non-vacuous; `reasoning,omitempty` confirmed acceptable-latent); 2026-06-26 cycle (1 port, no release) independent go-review (ship, no findings) + adversarial parity review (faithful; openai default-model lock mutation-verified non-vacuous); 2026-06-29 cycle (1 port, no release) independent go-review (ship, no findings) + adversarial parity review (faithful; zai `clear_thinking:false` mutation-verified non-vacuous; confirmed no 0.80.2-derived golden pins the zai request shape, so no latent divergence); 2026-06-30 cycle (1 port, no release) independent go-review (ship; 2 LOW cosmetic nits, not applied) + adversarial parity review (faithful — the 4000-char body-truncation cap is the one architecture-independent behavior; the SDK-field-probing layer is N/A since Go reads the raw `resp.Body`; truncation + metadata.raw-dedup tests mutation-verified non-vacuous; two non-blocking unpinned divergences documented — see the 2026-06-30 drift note); 2026-07-01 v0.80.3 cycle (release regen + 1 behavior port) independent go-review (ship; one LOW computed-var-vs-literal-const nit, not applied) + adversarial parity review (both changes faithful — catalog `cmp`-identical to an independent re-derivation from the 0.80.3 build; bash-timeout error strings byte-exact, boundary accepted, both new tests mutation-verified non-vacuous on a worktree copy); 2026-07-06 cycle (3 ports, no release) independent go-review (ship; one LOW stale-provenance-comment nit — applied) + adversarial parity review (all 3 faithful; caught a vacuous 524 test — the original `"error 524: origin timed out"` string also matched the pre-existing `"timed out"` pattern, tightened to `"524 status code (no body)"`; responses floor + all four inverted vercel-attribution tests mutation-verified non-vacuous on a throwaway worktree; confirmed no golden regeneration implicated — no release tag crossed); 2026-07-07 cycle (1 port, no release) independent go-review (ship, no findings) + adversarial parity review (faithful — `(no tool output)` byte-exact on both openai-completions + openai-responses; both new tests mutation-verified non-vacuous on a throwaway rsync copy (flipped literal → both fail); the image-bearing differential scenario `TestDiffToolResultImagesEmitUserMessage` still correctly expects `(see attached image)`, no empty-no-image scenario exists so 37/37 unchanged; `8c0ccd14` null-content normalization confirmed already-faithful/no-code — Go's `ContentList.UnmarshalJSON` maps null/missing → nil, nil slices range as empty (the JS "content is not iterable" crash is structurally impossible), and `ContentList.MarshalJSON` re-emits nil as `[]` byte-identically to pi's normalization; azure-openai-responses half deliberately unported); 2026-07-08 cycle (1 port, no release) independent go-review (ship; one MED test-coverage finding — the plural case was untested — applied, plus a raw-string-literal LOW; the "move the guard into `executeToolCalls`" LOW was **rejected as unfaithful**: pi puts the dispatch in `runLoop`, so the call-site placement is the parity-correct one) + adversarial parity review (faithful — error string byte-compared programmatically against pi's template literal; `%s` not `%q` confirmed correct since a tool name containing `"`/`\` must interpolate raw; event order/count, `terminate:false`→loop-continues, and guard placement all 1:1; `errorToolResult` yields `Details: map[string]any{}` == pi's `details:{}`, not nil; no golden surface touched, no pre-existing test pinned the old behavior; four mutations verified behavioral-red — revert guard, collapse loop to one result, batch the starts, perturb the string — an earlier compile-fail mutation was redone as a real edit since a compile-fail red doesn't count); 2026-07-09 cycle (1 port, no release) independent go-review (ship, no findings — one string literal added to the `strings.Join`ed pattern slice + one table row) + adversarial parity review (faithful — added string byte-identical to pi's, regex construction equivalent (`new RegExp(join("|"),"i")` ↔ `(?i)`+`strings.Join`), plain literal so it matches case-insensitively the same way, inserted at pi's source position (after `socket hang up`, before `timed? out`); test message byte-identical to pi's vitest constant; non-vacuous both by removal-mutation on a scratch copy and by confirming no pre-existing pattern matches the Bun message); 2026-07-10 v0.80.6 cycle (5 ports + catalog regen) independent go-review (ship; 3 optional LOW nits — 2 applied: truncated `#6...`→`#6464` comment + `CalculateCost` anon-struct→four plain locals; 1 skipped: promoting `Timestamp()` onto the public `Message` interface, an API-boundary change deliberately not made — the localized `messageTimestamp` helper's default-0 is unreachable today with only three Message impls) + independent adversarial parity review (all 7 changes faithful to pi 0.80.6; catalog **byte-identical** to a fresh independent `JSON.stringify(MODELS)` re-derivation from the integrity-verified build, schema-drift enumerated — `cost.tiers`/`max` and all tier keys map onto the new structs with no dropped key and integer `inputTokensAbove` across all 12 tiered models; every behavior mutation-verified behavioral-red on isolated scratch copies; cost-tier strict-`>` boundary + responses 15/2/3 + estimate 1005/0/1005/-1 & 2001/2000/1/3 re-derived by hand; differential 37/37; anchor test's `Timestamp:2` fixture-fix confirmed assertions-unchanged; `a6f720e6` compaction custom-message budget confirmed already-faithful/no-code — Go's `compactionSummaryMessage`/`branchSummaryMessage` project to `ai.UserMessage`, so custom/branch/compaction entries already count in the budget and act as cut-points/turn-starts, exactly what pi's entry-projection refactor achieves); 2026-07-11 cycle (2 ports, no release) independent go-review (**ship**; only LOW nits — the dead `*ToolResultMessage` branch in `estimate.go` collapsed to the file's value-only convention, applied; `SplitDeferredTools`' Immediate/Deferred/ByName shape confirmed the right Go decomposition of pi's one-Map-used-two-ways) + independent adversarial parity review (**both faithful**; deferred-tool loading a byte-level no-op for all existing goldens with paths double-gated on model support + `AddedToolNames`; `pi_tool_load_` shortHash call-id mutation-verified byte-identical to pi's JS incl. astral chars; sibling-content displacement + `defer_loading` shapes + version-gate all mutation-verified; `<authenticated>` ambient-marker filter faithful to pi's compat.ts guard and latent for ported providers; a `minor==4`→false case was added to pin the `≥5` gate non-vacuously; `ai/models_catalog.json` confirmed unchanged and no out-of-scope codex/extensions/generator surface ported); 2026-07-14 cycle (2 ports + 1 already-faithful + 1 decide-ruling, no release) independent go-review (**fix-first → ship**; one HIGH — an unguarded wire-controlled `contentIndex` panicked the host on a non-conformant stream — plus a MED CRLF-seam note, **both fixed**; LOWs on ctx-arg order + capitalized error strings declined as repo-consistent/parity-verbatim strings) + independent adversarial parity review (**both faithful**; openai-responses stored-sig key order proven unobservable via the parse→re-serialize replay round-trip, backfill fires only on a reasoning item carrying `encrypted_content`, 37/37 unperturbed; pi-messages request/body/converter/error-format/diagnostics byte-faithful, two real low-frequency divergences caught and **fixed** — CRLF-across-reads normalization + swallowed `OnResponse` error — both mutation-verified non-vacuous; the anthropic empty-usage change confirmed **already-faithful** — `anthropic.go` `message_delta` already guards `if ev.Usage != nil`, `Usage` being a nil-able `*anthropicUsage`)); 2026-07-17 v0.80.10 cycle (8 ports incl. the SDK-scoped facade, 3 release tags) independent go-review (**ship**; 3 LOWs, all applied — deterministic sorted-key `mergeHeaders` case-collision merge, RefreshModels joiner-semantics doc, Models register-before-concurrent-use doc; race-clean under `-race`, facade judged "real Go, not transliteration") + independent adversarial parity review (**fix-first → fixed**; TWO missed merge-carried upstream hunks caught — HIGH: 5220aba6's xai `include:["reasoning.encrypted_content"]` in the responses builder, demonstrated live against the shipped 0.80.10 build and ported+mutation-verified in `80ebacc`; MED: 97f9978f's `force` refresh flag on the just-ported facade API, ported in `21a18cb`; LOW: v0.80.8 release provenance recorded; everything else confirmed faithful — catalog independently re-derived `cmp`-clean + endpoint-pinned, kimi/xai/retry/facade semantics 1:1, all three recorded divergences still hold, with pi's new compat routing noted as converging toward Go's raw-provider+env-key path); post-review full-range reconciliation sweep of `packages/ai/src` + coding-agent core confirmed no other in-scope hunk was missed (xai.ts provider def = OAuth + catalog-`api` routing Go's generic `APIByApi` wiring already handles; bun-oauth.ts = bun packaging; sdk.ts/model-resolver.ts remainder = host ModelRegistry→ModelRuntime renames); 2026-07-15 v0.80.7 cycle (release regen + 1 behavior port, no decides) independent go-review (**ship, no findings** — the system-prompt date removal is surgical: date computation, both `Current date` lines, the dead `Now` option field + `time` import all removed together, no dead code; goldens/assembly-order assertions updated consistently, no weakened assertions) + independent adversarial parity review (**both faithful**; system-prompt removal byte-confirmed against the shipped 0.80.7 build's `dist/core/system-prompt.js` (no `Current date` string, `Current working directory` retained both branches); catalog **endpoint-pinned** — HEAD `ai/models_catalog.json` ≡ independently-derived 0.80.6 MODELS AND working tree ≡ 0.80.7 MODELS, both `cmp`-identical; schema-drift enumerated — no new model/cost/tier keys or `api` values, the only additions (`compat.sessionAffinityFormat` string, `compat.supportsToolSearch` bool) ride `ai.Model.Compat` `json.RawMessage` and cannot abort the load; every removed id confirmed unreferenced in Go source (the `mimo-v2.5-pro` grep hit is the still-present v2.5, not the removed v2); resolve re-point verified by tracing `findExactModelReferenceMatch` — `anthropic/claude-opus-4.8-fast` lands uniquely on openrouter)); 2026-07-21 v0.81.1 cycle (6 ports + catalog regen, 2 release tags) independent go-review (**ship**; 3 LOW polish — nil-safe `*RetryCallbacks` dispatcher methods, dropped a vestigial "before jitter" doc note, documented `produce`'s non-nil contract; retry-loop concurrency judged clean under `-race`, all ports "real Go, not transliterated TS") + independent adversarial parity review (**fix-first → fixed**: one CONFIRMED MED — 1942b260 was only half-ported, the `amazon-bedrock.ts` analog (Go's generic ambient-provider resolver `builtins_models.go`) still dropped `cred.Env` which the models runtime consumes; ported in `912607a` + regression-tested — everything else faithful: d9f7f814 pipe-id results independently re-derived from pi's `shortHash`+`convertMessages` incl. the hash-fallback goldens, retry-loop backoff/abort-normalization/callback-ordering 1:1, retainedTail reconstruction matches `defaultContextEntryTransform`+`sessionEntryToContextMessages`, catalog `cmp`-clean byte-identical with no schema drift, Qwen wired end-to-end; merge-reconciliation sweep clean — the ambient-resolver hunk was the only in-scope SDK miss)); 2026-07-23 cycle (4 ports, no release) independent go-review (**fix-first → ship**; one HIGH — the new live session-metadata path read `s.Model`/`s.Recorder` from the tool-execution goroutine while `SetModel`/`Record` write them, reproduced under `-race` by the reviewer with an external repro module; fixed by routing the model through the agent's mutex-guarded `State()` (also the authoritative value — `Session.Model` is a shadow copy on this path) and the recorder through `recMu`/`recorder()`, with the `var sess *Session` hoist-and-assign replaced by allocate-first so the closure captures a stable non-nil pointer and the half nil-guard could go; MED `recMu` coverage gap at `SetModel`/`SetThinkingLevel` — pre-existing and off the bash path, fixed anyway since a half-applied lock is worse than a documented absent one; 4 LOWs applied — hoisted `os.Environ()`, `slices.Contains` over the fixed key list, Windows case-dedup comment, comma-ok test tool lookup; the `sessionEnvFn`→struct LOW judged polish and skipped; the exported-`Recorder`→atomic alternative was deliberately NOT taken as a public API break, which the reviewer independently endorsed) + independent adversarial parity review (**faithful**; all 16 files of bb3d7d39 and every hunk of the other three enumerated and given a Go home or a justified absence — pi's second `addCacheControlToMessage` role check confirmed redundant via its single call site, `createSummarizationOptions` confirmed to carry neither `sessionId` nor `cacheRetention` before the change so the delta is identical, the system-prompt guideline placement independently re-derived from pi's TS assembly since the 0.81.1 build predates the commit, both new strings byte-compared programmatically; all three claimed divergences confirmed faithful, two further structural divergences surfaced and recorded; six mutations verified non-vacuous on a throwaway copy; one inaccurate rationale comment corrected in `9f7cac5`); 2026-07-26 v0.82.1 cycle (3 ports, release tag crossed but catalog byte-identical) independent go-review (**ship** with a recommended follow-up) + independent adversarial parity review (**fix-first → fixed**: one CONFIRMED MED — the ANTHROPIC_AUTH_TOKEN port inverted pi's credential-first precedence (env token overrode an explicit/stored key, and the facade `GetAuth` surfaced the token as an api key instead of a Bearer header) because it only added the env var to `apiKeyEnvVars` + a compat re-read, missing that `24e5cc04` swaps anthropic to a custom `anthropicApiKeyAuth`; fixed in `7ebae86` — a custom Go resolver returning `ModelAuth.Headers{Authorization: Bearer}` with pi's order, `withEnvAPIKey` leaving APIKey empty when the token is active, and the provider re-read gated on `apiKey == ""`; the inverted `TestAnthropicAuthTokenWinsOverApiKey` was corrected to `TestAnthropicExplicitKeyBeatsAuthToken` and facade-resolver + compat-layer precedence tests added, all mutation-verified. `b1c444d9` etag and `4cf0a729` ModelsError-cause both confirmed faithful — the latter **already-faithful**: Go's `ModelsError.Error()` composes code+message+cause, so it was ahead of pre-change pi; pi's `withCauseDetail` dedup/trim/null-guard checked and found non-observable for every real wrap site. Byte-goldens untouched (catalog byte-identical to 0.82.0); build/vet/gofmt/`-race` green); 2026-07-27 cycle (1 port, no release) independent go-review (**ship**; 2 LOW applied — the new `applyMessagePhaseStopReason` closure had been wedged between `createSlot`'s doc comment and its definition, reattached; added an "unreachable today" note on the openai-responses pending guard, which sits after the pre-existing `sawTerminalResponseEvent` guard as cheap defensive parity; const placement + closure-over-goroutine-local-`output` decomposition + guard placement all judged idiomatic) + independent adversarial parity review (**FAITHFUL**, no behavioral divergences — every per-provider error string byte-matched (incl. google's "finish reason" vs the others' "stop reason"), guard ordering 1:1 with pi in all five providers, both `applyMessagePhaseStopReason` call sites (createSlot message branch + top of output_item.done) present, terminal override correctly wins over a provisional final_answer stop, no out-of-scope provider ported, and the `proxy.ts` streamProxy subsumption independently verified (no Go analog; `agent/agent.go:413` is a synthetic failure message, not a streaming partial); "pending" proven never to reach a persisted message so no golden moves; the review flagged four vacuous/absent coverage gaps — the faux init line, the pi-messages init, the responses createSlot call site, and the incomplete-overrides-provisional-stop interaction — **all four closed** with mutation-verified non-vacuous tests). gofmt/build/vet/`-race` green; in-repo differential request-body scenarios 14/14 (response-side change, request bodies unperturbed)  |
...
agent-session-runtime (session reload + /new flow), the agent harness
(`packages/agent/src/harness/` — no `harness/` tree in Go; as of 2026-07-23 this
includes its new first-class tool suite, see that drift note's boundary note),
the Radius OAuth provider +
