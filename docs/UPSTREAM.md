...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`bbb61e34a`** — "fix(ai): send OpenRouter session affinity headers by default" (advanced 2026-09-10). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all **14** first-parent changes in `be26e3270..bbb61e34a`, and this time the two counts AGREE — 14 commits, 14 first-parent — because no merge is in range. Verdicts: **2 port (both shipped) / 2 port-but-QUEUED / 1 port-but-CATALOG-ONLY / 9 n/a / 0 decide**. The unported balance is carried by **Scope queue entries 3, 6, 7, 8, 9, 10, 11, 12, 13, 14 and 15** plus the catalog-only queue — not by the pin. **Read the queued-deltas column; never inherit the previous cycle's list.** The next cycle's drift is `bbb61e34a..origin/main` PLUS those rows. **Shipped:** (1) `c37b0e03b` — pi's agent-level retry backoff gains a 60s default cap (`DEFAULT_MAX_AGENT_RETRY_DELAY_MS`, an optional `RetryPolicy.maxAgentDelayMs`, and a new exported `retryDelayMs`). Only the `packages/ai/src/utils/retry.ts` third of that commit has a Go base; porting it exposed a shipped defect the cap alone would have masked — `ai/retry_loop.go` computed the delay as `BaseDelayMs << (attempt-1)`, which for `BaseDelayMs=1000` wraps NEGATIVE at attempt 55 and to exactly 0 from 62, and a non-positive `time.Duration` makes `time.NewTimer` fire immediately, i.e. the backoff was a hot retry loop, not a long sleep. `MaxAgentDelayMs` is a `*int` because pi's `??` makes an explicit `0` a real zero cap, the INVERSE of `ProviderRequestOptions.MaxRetryDelayMs` whose zero disables its cap. (2) `bbb61e34a` — OpenRouter session affinity becomes the default on both anthropic-messages and openai-completions, under OpenRouter's own header name. Golden surface: **366** catalog models change behaviour (351 `openai-completions` + 15 `anthropic-messages`), because ZERO openrouter entries carry an affinity compat key, so the code default decides for all of them. The anthropic adapter needed a new compat key (`sessionAffinityFormat`) it did not have; completions needed only the send default, its format having been openrouter-detected already. **Queued:** entry 8 → 13 (`c37b0e03b`'s harness half), entry 10 → 2 (`e86102f18`), entry 12 → **8** (`c37b0e03b`'s and `46bde88a1`'s `core/agent-session.ts` halves), catalog-only → 3 (`6b94ae2ec`, and `bbb61e34a`'s generator hunk). **A classification was MOVED, not merely challenged:** `46bde88a1` was drafted `n/a` on E1 and is filed instead as an entry-12 delta. Both readings agree there is zero Go work — the port has no settings reader at all — but `core/agent-session.ts` is a queued tree, and this file's own rule is that a queued tree is never `n/a`. That does not reopen 2026-09-09's `faa9863cb` ruling, which rested on a provable no-op (its whole path gated on an extension runtime the port lacks); nothing gates this one. **Ledger repairs (boy-scout, in-area):** the Scope queue table was split in two by a blank line so entries 11–15 rendered without a header, and rows 11, 12, 13 and 15 carried 5 cells against a 6-column header — both fixed, and the sentence still quoting entry 8's backlog as 11 now carries its 12 and 13. **Reconciliation at the advance:** detector 54 paths, A/D/R is 10 additions (5 `packages/agent/docs/pico*` prose, 5 test files), 2 deletions (both `packages/evals`, E1) and **zero renames** — no new top-level package, nothing moved out of `packages/`, no new repo-root directory; the one new directory is `packages/agent/docs/pico/`, `.md` only. Accounting 47 files, all accounted for. None of the four split-by-HUNK tripwire files was in range, so the standing `defaultModelPerProvider` rule did not fire. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`git tag --contains be26e3270` is empty, against 315 existing tags, and `git describe --tags be26e3270` = `v0.85.1-32-gbe26e3270`), so there was **no catalog regen and no reference-build refresh**, and no release is cut this cycle. The catalog-only queue GREW from 1 to **3**: `7d8ab31a4` (unchanged), plus `6b94ae2ec` and `bbb61e34a`'s generator hunk — see the catalog regen checklist, and note the two new rows touch the same file in a fixed order. **Carry to the next release, unchanged from 2026-09-09:** `4a6ed0194`'s MAJOR `@google/genai` bump (1.52.0 → 2.21.0) is invisible to difftest, whose google scenarios are `dist`-backed at 0.85.1, i.e. pre-bump. Re-capture the google scenarios against the first npm build that ships it. |
| Parity proofs at the pin | **2026-09-10:** both units were written red-first with a **behavioural** red; neither red was a compile failure, and getting that took deliberate test design in both cases. The retry red uses only PRE-EXISTING fields — a policy with `BaseDelayMs: 1_000_000` on a pre-cancelled context, so the loop reports the delay it would have slept and aborts on it — and failed at `scheduled delays = [1000000], want [60000]`; a test that named the new field would have failed to compile instead, which does not count. The affinity red is the empty header against pi's id, on every new cell, with the negative cases green before and after. Both of pi's own oracles are transliterated for retry (the `retryDelayMs` table and the end-to-end `[10, 15, 15, 15]` callback sequence, whose tiny 10ms/15ms policy is kept verbatim because those are real sleeps), and all four of its new affinity assertions. Two affinity cells are covered that pi's OWN suite cannot reach: the baseURL half of the detection on each adapter — upstream's capture helper rewrites `baseUrl` to the test server, so its tests only ever exercise `provider === "openrouter"` — and an explicit format off OpenRouter. **Ten mutations were run against shipped code, serially, each reverted:** drop the cap, treat a zero cap as disabled, drop overflow saturation, drop the exponent floor, revert either send default, send both header names, use the format as a second enable switch, apply detection after the compat overrides, and drop the retention gate. Every one fails the suite. Full gate green: `gofmt`/`build`/`vet`, `go test -race ./... -count=1`, `GOOS=windows` build+vet, and **difftest 51 PASS / 0 KNOWN / 0 FAIL** against 0.85.1 with an empty known-divergence baseline. **Neither unit was difftest-reachable, and that was known before the run rather than discovered by it:** the harness captures request BODIES — both arms hook the payload builder and abort before the first byte, so no HTTP request is ever issued — while this cycle shipped an HTTP header and a backoff delay. The difftest re-pin to `bbb61e34a` is bookkeeping only: all 51 scenarios are `dist`-backed and `PI_UPSTREAM_SHA` drives only `"backend": "src"` scenarios, of which there are none. |
| Reviewed via | 2026-09-10 — per-commit triage of all 14 first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Diffstat-dispatched only for the six `packages/agent/docs/pico*` prose commits, `packages/evals` and packaging; hunks read for everything touching `packages/ai/src`, `packages/agent/src` and `packages/coding-agent/src/core`. Two adversarial passes, both read-only: **8 investigators** before porting (an independent from-scratch re-triage that reproduced all 14 verdicts including both reconciliation sweeps, upstream semantics and the port's behaviour matrix for each of the two units, an attack on every draft classification, a ledger-bookkeeping audit, and a check of what difftest can actually see), then **6 review lenses** after (`pi-go-review`, `pi-parity-review`, a completeness critic re-deriving every hunk in both directions, a test-quality critic hunting vacuous assertions, a narrow adversary on header precedence, and a blast-radius pass). Every filed finding went to **3 refuters** instructed to REFUTE and to default to refuted when uncertain: **10 filed, 9 refuted 3–0, 1 survived.** The survivor is a real coverage hole with a reproduced surviving mutation — dropping the baseURL disjunct from the completions send default left the WHOLE repo green — and is closed by `TestDetectOpenAICompatSessionAffinity`. **Five refuted findings were acted on anyway**, each because the underlying FACT was re-verified independently of the classification argument the refuters won: a helper closure that captured the parent `*testing.T` (real `testing` misuse, whatever its severity); a drafted shared `isOpenRouterModel` that both duplicated this package's existing `isOpenRouterAttributionModel` AND took the name upstream binds to that OTHER predicate — dropped for pi's own per-adapter inline probe, which also shrank the diff by one file; a `1 << 40` test constant that does not compile where `int` is 32 bits; absence-only subtests over a helper that returns nil headers when no request is made, now carrying a positive control (`Header.Get` on a nil header returns `""` — this exact trap was hit in a previous cycle); and a split struct field group. **A process fault worth recording:** those fixes were applied to the working tree WHILE the reviewers were still running, so several refuters opened findings that looked already-fixed and had to export `git archive HEAD` to judge the actual cycle. They caught it; the porter should have waited. |
...
| 6 | **Mistral conversations** | 963 LOC | ~1.5 | JSON over HTTPS, header auth. | **2** — `6c87d9a02` (2026-08-29: merge indexed Mistral tool-call chunks — `consumeChatStream`'s `toolBlocksByKey` is rekeyed from the composite `` `${callId}:${index}` `` string to `toolCall.index ?? callId`, so chunks of one call that arrive with differing ids still merge. No Go base: `ai/providers/` has no mistral adapter, only registry-level references in `ai/envkeys.go`, `ai/types.go` and `coding/resolve.go`), `96617628e` (2026-09-08: `usesReasoningEffort` widens from the exact ids `mistral-medium-3.5` to the prefix `mistral-medium-`, so every reasoning-capable `mistral-medium-*` model sends `reasoning_effort` instead of prompt-mode reasoning. No Go base: `ai/providers/` still has no mistral adapter, only registry-level references in `ai/envkeys.go`, `ai/types.go` and `coding/resolve.go`) |
...
the matching npm build. **Currently 3 — grew by two on 2026-09-10.** It was
...
release commit); `7d8ab31a4` reopened it. The two new rows are ORDERED and touch
the same file — `6b94ae2ec` rewrites `applyThinkingLevelMetadata`'s fireworks
branch and `processFireworksModels`, `bbb61e34a` then rewrites line 726 of
`detectOpenAICompletionsCompat` — so replay them in that order, and check each
against the release sha with `git merge-base --is-ancestor` rather than by log
order.
...
`TestResolveModelProviderPrefixFallsBackToFullID`.
