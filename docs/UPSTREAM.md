...
the matching npm build. **Currently 6 — grew by three on 2026-09-11.** It was
drained to 0 on 2026-09-07 by the 0.85.1 regen (7 queued deltas plus
`17de82d7b`'s `gpt-6-astra` generator half, all ancestors of the `v0.85.1`
release commit); `7d8ab31a4` reopened it. Every row touches
`generate-models.ts`, and three of them form an ORDERED chain — `6b94ae2ec`
rewrites `applyThinkingLevelMetadata`'s fireworks branch and
`processFireworksModels`, `bbb61e34a` then rewrites line 729 of
`detectOpenAICompletionsCompat` (`sendSessionAffinityHeaders`), and `d92eb8d4b` then adds a key to the same
Fireworks `anthropicCompat` object `6b94ae2ec` edited — so replay the queue in
commit order, and check each row against the release sha with
`git merge-base --is-ancestor` rather than by log order.

| delta | what regenerates |
|---|---|
| `d92eb8d4b` (2026-09-10) | Fireworks Messages gets native deferred tool loading. `processFireworksModels`' shared `anthropicCompat` gains `supportsToolReferences: true`, so all **14** fireworks `anthropic-messages` models in the shipped catalog gain it (0 carry it today) and the 6 non-Messages fireworks models do not. **Request-body-affecting:** with the key on, a context whose discovery tool result carries `addedToolNames` sends the discovered tools with `defer_loading: true` and that result as `tool_reference` blocks, where today it sends plain schemas. No src-side Go change: `ai/providers/anthropic.go` reads the key through `applyCompat` and already builds both shapes for first-party Anthropic. The oracle is the new `test/fireworks-deferred-tools.test.ts` — transliterate it at the regen. It was already run once out of tree on 2026-09-11 against the shipped adapter with this compat simulated, and every body matched pi's, so it should land green. Its discovery context carries an empty-signature thinking block, so it also depends on `6b94ae2ec`'s `allowEmptySignature` and cannot go green before that row lands. |
