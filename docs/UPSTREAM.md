...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`49681e1b7`** — "fix(coding-agent): hide line range for full-file read calls with null offset/limit" (advanced 2026-09-25). **19 changes in `8676a0dcd..49681e1b7`, union = first-parent** (no merges). **No release crossed:** `git tag --contains 8676a0dcd` is empty after an explicit tag fetch, and `packages/ai/package.json` is still 0.87.1. **Verdicts: 8 port / 11 n/a / 0 decide.** Shipped: the chord delta tracker rewrite, in three parts — `d5cba1d97` the overlay tracker (`3bde1fa`: `chord/delta/{tracker,draft,emit,pieces,ordered}.go`; `Track` takes ownership in O(1), any number of changes open with a weak registry that stales the losers, ops emitted from the overlay instead of diffed), `19a0361be`'s chord half (`09f0f4c`: `chord.CopyJSON`, and `chord.IsValue` without its depth cap, both on `chord/internal/jsonvalue`, which draft placements share), `9e70c3d50` (`0abc84c`: `ApplyImmutableBatches`, and one copy per touched container). **Queued:** `a7d17e39a` → row 3 (the OpenRouter and Cloudflare Workers AI classifier APIs, `api/system-one-shared.ts`), its generator → catalog-only; `packages/durable` → row 8 (`601437d5a`, `3f0573877`, `19a0361be`'s half, `5d4de953c`, `5fd446ca1`); `d5cba1d97`'s `harness/pico3/legacy-tracker.ts` hunk → row 8; its `services/state.ts` hunk and the new `delta/revision-validator.ts` → row 11. **Catalog-only (3):** `fde38ed7c`, `a328aa89a`'s generator, `a7d17e39a`'s generator. **n/a (11):** `7c696c00f`, `481c7232f`, `cbe7cf00b` (durable design docs), `567469096` (tui colors and theme styling, E1; its ai and coding-agent hunks are tests), `d5629e204`, `7cf037c21` (tui, E1), `6966636db` (the draft Proxy's `then` probe, a JavaScript thenable check with no Go subject — D70), `b2bd111f2` (`core/export-html/**`, E1), `f444ea5ea` (`package-manager.ts` split by hunk: the per-ref temporary git checkout path, installer side, E1 — not `coding/resources.go`'s skill discovery), `92e8d4f02` (`modes/rpc/rpc-client.ts`, E1), `49681e1b7` (`core/tools/renderers/read.ts`, the tool-call renderer, E1). **Upstream `ci.yml` per sha:** `a7d17e39a`, `601437d5a`, `481c7232f`, `cbe7cf00b`, `3f0573877` FAIL two model-count tests (`images-models.test.ts`, `model-runtime-images.test.ts`: `a7d17e39a` lists OpenRouter's classifier models beside its chat and image ones; `567469096`'s test hunks fix both); `d5cba1d97` CANCELLED by a newer push (`6966636db` and `19a0361be` after it are green); `9e70c3d50` has no run (not a push head; `5fd446ca1` on it is green); the other eleven green. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.87.1** (and `pi-coding-agent` 0.87.1), unchanged — no release in range, so `ai/models_catalog.json` is untouched. `coding/testdata/defaultmodels/default-models-49681e1b7.json` re-captured from src at the pin (`capture.mjs --src`; 41 defaults, identical in value and order to 0.87.1's). `chord/delta/testdata/upstream_delta.json` re-captured from src at `9e70c3d50` (`capture.mts`; chord's src is identical from there to the pin, and needs no third-party package). **Src-only oracles of the 2026-09-24 cycle** (upstream `packages/ai/src` at `8676a0dcd`/`002fc8385`, SDKs at the lockfile versions — openai 6.40.0, @anthropic-ai/sdk 0.124.0, @google/genai 2.21.0, partial-json 0.1.7, undici 8.10.2 — integrity-identical to the 0.87.1 cache): `ai/providers/testdata/{cache-write-1h,anthropic-sse,pi-messages,response-headers,anthropic-on-response,openai-stream,streaming-json,google-stream-events,abort-before-response}` and `internal/jstext/testdata/{json-parse-errors,utf8,lower}-node.json`. **New or re-captured this cycle** (upstream src at `49681e1b7`, SDKs at its lockfile's versions): `ai/providers/testdata/{stream-abort,base-url}` (stream-abort runs every no-response mode on all five adapters since `090e6ec`) and `testdata/pi-messages/capture-pi-messages-status.mts`; `anthropic-sse`, `google-stream-events` and `response-headers` re-captured at `49681e1b7`, every earlier row byte-identical; `testdata/text-decoder` reads node alone, no pi code (D87). **Re-capture them all against the first published build that ships these commits** and re-run each capture whenever the lockfile moves one of those SDKs. |
| Parity proofs at the pin | **2026-09-25.** Every behaviour change was observed RED for a behavioural reason by mutating the shipped code in a scratch copy; each commit message carries its table (the three chord ports 24, 4 and 4 mutations; every fix its own reds). **The `chord/delta` golden** was re-captured from src at `9e70c3d50`: 284 diffs, 118 scenarios, 16 generated changes, and 100 fuzz and 20 differential runs. It replays byte-identical, with op batches compared in pi's raw order (D69). The parity reviewer regenerated it independently and drove 1,500 more random scripts through both (about 140k steps, 39k of them identity checks) plus 1,500 sort scripts with 5,522 comparator side effects, with no divergence. **difftest re-pinned** (`PI_UPSTREAM_SHA` 49681e1b7, `PI_NPM_VERSION` 0.87.1) at **61 PASS / 0 KNOWN / 0 FAIL, all `dist`-backed**. Full gate on the integrated tree: `gofmt -l` clean, `go build`/`go vet`, `go test -race ./... -count=1` green, `GOOS=windows` build + vet, `GOOS=linux GOARCH=386` build. |
| Reviewed via | 2026-09-25 — per-hunk triage of the 19 changes, then the three chord ports one after another on one tree, since each rewrites what the next builds on. One pi-go-review, one pi-parity-review and one refuter covered `3bde1fa..0abc84c`. Seven HIGH/MED findings were confirmed and fixed. `7059848`: an immutable application let the GC hand a dropped copy's address to a lazy payload, which was then written in place; a nil map was applied as an empty one. `5b696cf`: a Sort comparator that settles its own change crashed; `IsValue` allocated per member on protocol's hot path; one write to a wide object was quadratic; `Track`'s doc named the wrong representation; `CopyJSON` and `IsValue` could not read a draft. `77c50cd`: a nil root was drafted as an empty container. `51cf94a` is the behaviour-neutral half, giving the overlay's JavaScript idioms Go shapes. **Both carried open-findings lists are disposed**, and their sections deleted. 2026-09-04: S28 (`b9ad64b`, and `05d71a0` from its verification), S11 (`a23478b`), S31a (`eb5a89a`), and every surviving mutation pinned (`5a701a7`..`1909635`). 2026-09-24: every finding fixed or recorded (`67bc6f4`..`432c78f`), then one pi-parity-review over those fixes, whose findings are fixed too (`cbcf72c`..`a53f211`): request URLs get the WHATWG parser's input cleanup and the SDKs' one-slash join, a URL with credentials is refused as undici's fetch refuses it, an abort while a retryable error body is read says `Request aborted`, every SSE reader refuses a body that never progresses (D83), a custom client's rejection stays its own on pi-messages, an abort on a body that has already ended fails the next read, and one UTF-8 lead-byte table serves both decoders. Three mutants of `daed448` survive, each reachable in pi only by a race or dead there too (its commit message says which). **K21 closed** alongside: a request that gets no response fails anthropic and both openai loops with their SDKs' `Connection error.`, or `Request timed out.` (`090e6ec`), and pi-messages waits on undici's 300s headers timeout as google does (`8f17468`). K26's residuals (UTS 46 hosts, dot segments, `127.1`, percent-decoded hosts) wait on a WHATWG host parser and a new x/net dependency. **API changes (mirror ruling):** `delta.Track` returns no error; new `Tracker.Revision`, `Prepared.BaseRevision`, `Prepared.Abort`, `Draft.Snapshot` and `delta.ApplyImmutableBatches`; new `chord.CopyJSON`, whose `chord.ValueError` is the same type as `delta.ValueError`; `ErrDraftRevoked` is `ErrDraftSettled`; `ErrChangeActive` and `ErrAdoptDuringChange` are gone, since any number of changes may be open; a draft places typed slices and maps. New D87, K26, K27 and C7; D30, D69, D70 and D72–D76 rewritten for the overlay tracker; D13 and D83 extended; K21 closed and its entry deleted; D71 retired. |
...
| `core/model-runtime.ts` (2026-09-24) | the `Models` contract it forwards — the typed accessors and the chat guard on `stream`/`streamSimple`/`streamDeferred`/`cancelDeferred` → `ai.Models` (`ai/models_runtime.go`); image/classifier operations → Scope queue row 3 | auth.json credential store, models.json + extension composition (`provider-composer.ts`), radius wiring; the remote-catalog wrapping is row 19 |
...
| `core/provider-composer.ts`, `core/model-config.ts` | E1 (operator configuration) | 2026-09-24. They layer the operator's models.json and extension provider registrations over a built-in `Provider`; `ModelRuntime` bypasses the composer when neither layer exists (`model-runtime.ts:276-280` at `a328aa89a`). Previously out only by the 2026-07-17 note's only-consumers premise, which no longer holds for the cluster as a whole. |
...
- **[PARTLY SUPERSEDED 2026-08-27] 2026-07-17 — the model-runtime facade is ported SDK-scoped** (re:
  ...
  **Not ported**: the OAuth reorg (`utils/oauth`→`auth/oauth` — acquisition),
  ...
  >
  > **RE-JUDGED 2026-09-24 (standing formula — the test wins and the list follows).**
  > At `a328aa89a` the only-consumers premise is false for two of those files.
  > `core/model-runtime.ts` is imported by `core/sdk.ts` (`createAgentSession`
  > defaults `ModelRuntime.create` and streams through `modelRuntime.streamSimple`),
  > `core/agent-session.ts`, `core/cache-warmer.ts` (entry 17) and
  > `experimental/**` (entry 11), so it is split by HUNK: its `Models`-contract
  > half is `ai.Models` (the chat guard shipped 2026-09-24), its composition half
  > — auth.json, the extension registry, radius wiring — stays E1.
  > `core/remote-catalog-provider.ts` inherits the mixed consumer set and is now
  > Scope queue row 19. `core/provider-composer.ts` and `core/model-config.ts`
  > stay OUT on E1's operator-configuration ground, not the only-consumers one:
  > they layer models.json and extension registrations, and `ModelRuntime`
  > bypasses the composer entirely when neither layer exists
  > (`model-runtime.ts:276-280`). `models-store.ts` and `runtime-credentials.ts`
  > were not re-examined this cycle.
...
- **2026-06-24 — models-runtime migration completed under the "globals stay as
  ...
  3. **compat `shouldUseBuiltinModels` routing** — NOT ported. pi routes catalog
     ...
     One consequence since `a328aa89a`: pi's compat `stream`/`streamSimple`
     assert nothing, except on the cloudflare-* branch without resolved auth,
     which forwards to `compatModels` and so hits `requireChatProvider`. A
     caller-built NON-chat model there fails `Model <p>/<id> is not a chat
     model` in pi and streams in Go. Catalog data cannot reach it.
...
|---|---|---|---|---|---|
| 1 | **OAuth token acquisition** (`auth/oauth/**`, `oauth.ts`) | 2,983 LOC / 12 files — **corrected 2026-08-31**: the old parenthetical read "2,439 excluding `openai-codex.ts`, whose adapter is OUT under E2", which has been stale since the 2026-08-27 rewrite (the Codex adapter is entry 10, back in scope) and contradicted it. `openai-codex.ts` is the 544-line **OAuth** file and belongs to this entry; note it is also inside entry 10's 2,228, so the two entries **double-count** those 544 lines — 2,983 − 2,439 = 544. Do not sum the queue's sizes | ~3 | Highest value in the queue. `OAuthAuth.Refresh`/`ToAuth` and `LazyOAuth` are **ported seams with zero implementers** — Anthropic Pro/Max, Copilot, OpenRouter, Kimi and xAI subscription auth are simply unreachable from Go. Needs nothing beyond `crypto/rand`, `crypto/sha256`, `net/http`, `os/exec`. Carries `auth/oauth/radius.ts` (403), so land it with entry 2. | **+1 (2026-09-20)** — `b73412a37`: `metaOAuth` (`auth/oauth/meta.ts`, 208 LOC), Meta's Muse SUBSCRIPTION flow — `isSubscription: true` with a `Sign in with Meta` login label, the first entry-1 row that is a subscription rather than a key exchange. The provider's models, base URL and env-key auth already arrived with the 0.86.1 regen and `7cb43ec`; only the acquisition is missing, so a Meta user with `META_API_KEY` set is served today and one relying on the subscription is not. |
| 2 | **Radius provider** (`providers/radius.ts`, `providers/radius-config.ts`) | **178 LOC** | ~0.5 | Smallest entry by an order of magnitude, and it closes the **2026-08-22 tripwire**, which is loaded and will fire on the next behavioral commit: `radius-config.ts` is reachable through pi-ai's `"./providers/*"` exports wildcard, i.e. published SDK surface the 2026-07-14 ruling did not name. Only ~8 first-parent commits in 90 days touch anything Radius-named, so this is bought for the tripwire, not for churn relief. | **+1 (2026-09-20)** — `4d38031fb`: `radiusProvider` stops serving only the dynamically fetched list. It now merges a STATIC baseline (`RADIUS_MODELS`, 27 models, shipped in the 0.86.1 catalog and present in the port's embedded copy since this cycle's regen) under the fetched ones, by id, and only when the gateway is the default. So an offline or unreachable gateway still lists models. The port carries the catalog rows already; what is missing is the merge and the fetch. **Also carried, found by the 0.86.1 parity review:** the 27 Radius catalog models carry three keys no other provider has — `lab`, `enabled` and `providers` — and `ai.Model` has no field for any of them, so the port decodes those rows and silently drops the keys. Harmless while nothing reads them; the base port has to decide what they mean before it can serve Radius's routing faithfully. |
| 3 | **Non-chat model operations — image generation + classification** (widened 2026-09-24) | at `a328aa89a`: ~670 LOC of image/classifier files in `packages/ai/src` (`api/openrouter-images.ts`, `api/typesafe-system-one.ts` 187, `providers/typesafe.ts`, `image-models.ts`, `images.ts`, `images-api-registry.ts`, `providers/images/register-builtins.ts`) + ~90 in `models.ts` + ~120 of `types.ts`. The old "684 is `image-models.generated.ts`" figure is dead: `a328aa89a` deleted that file | ~2.5 | Root-export surface (2026-08-27 ruling). **Widened 2026-09-24:** `a328aa89a` unified image generation and a new `classifier` model type (TypeSafe System One, provider `typesafe`, model `typesafe/jev-latest`) on the ONE `Models`/`Provider`/`createProvider` infrastructure, and both catalogs now come out of `models.generated.ts` (`IMAGE_MODELS`, `CLASSIFIER_MODELS`), so they queue together. **The base port must build on `ai.Models`/`ai.Provider`, never an `ImagesModels` collection** — upstream deleted `images-models.ts` and the separate `openrouter-images` provider. The chat-side half of the same commit SHIPPED this cycle (`Model.Type`, `GetModelType`/`IsModelType`, `AssertChatModel`, the optional `AllModelsLister`/`AllModelsFilterer`, the typed `Models` accessors, the unknown-type store filter, `TYPESAFE_API_KEY`). Contents still owed: `Model.Output` for image models; `Models.GenerateImages`/`Classify` (never reject — `imageErrorResult`/`classifierErrorResult`: stopReason `aborted` when the signal is aborted else `error`, errorMessage = message); `Provider` optional generate/classify; `CreateProviderOptions` `Images`/`Classifiers` maps and their arms of the `Provider <id>: at least one of "api", "images", or "classifiers" is required.` validation; `assertImageModel`/`assertClassifierModel` (`Model p/id is not an image model` / `… a classifier model`); dispatch errors `Provider X does not support image generation` / `… classification` and `Provider X has no image generation implementation for "<api>"` / `… no classifier implementation for …`; the openrouter-images api + the openrouter provider's `images` wiring (OAuth via row 1); the typesafe-system-one api (POST `systemone` under the base URL, bool questions sent as `type:"noul"`, headers `providerHeadersToRecord({authorization, content-type}, model.headers, options.headers)` — so a null marker can delete the adapter's own authorization — retry maxRetries 2, `Request timed out after ${ms}ms`) + `typesafeProvider()` in `builtinProviders()`; the compat `image-models.ts`/`images.ts`/`images-api-registry.ts`/`register-builtins.ts`; embedding the `IMAGE_MODELS`/`CLASSIFIER_MODELS` dumps from the npm build. **Known gap this row closes:** pi's `builtinModels()` registers `typesafe` (classifier-only) so `Models.getProviders()`, `checkAuth` and `getAllModels("typesafe")` see it; the port's catalog-derived `BuiltinModels` cannot until the classifier base lands. `scripts/generate-image-models.ts` no longer exists (the image catalog is `generate-models.ts` + `scripts/openrouter-catalog.ts`). Oracles: `typesafe-system-one.test.ts`, `classifier-models.test.ts` (classify half), `images-models.test.ts` (generateImages cases), `openrouter-images.test.ts`, `images.test.ts`, coding-agent `model-runtime-images.test.ts` (a),(d),(e) and case (g)'s built-in half, `model-runtime-classifiers.test.ts`. | **SUBSUMED 2026-09-24:** the four image-catalog deltas this cell carried (`5ce4afbd9`, `fbc87d25a`, `bdee230f1`, `b7f4b05c7`) targeted `image-models.generated.ts`, which `a328aa89a` deleted — they are retired, not replayed; the image catalog now lands whole as the `IMAGE_MODELS` dump of whichever npm build the base port pins. **+1 (2026-09-24)** — `a328aa89a`: the unification above, including the new `typesafe` classifier provider. **+1 (2026-09-25)** — `a7d17e39a`: the classifier operation reaches two more providers. OpenRouter serves TypeSafe's System One protocol at `/api/v1/systemone`, so `providers/openrouter.ts` lists its decision models and registers `classifiers: {"typesafe-system-one": typesafeSystemOneApi()}`; Cloudflare Workers AI gets a `cloudflare-workers-ai-system-one` classifier API (`api/cloudflare-workers-ai-system-one.ts` + `.lazy.ts`: `POST <base>/run` with `{model, input}`, answers read out of Cloudflare's `{success, result: {state: "Completed", result: {answers}}}` envelope, errors joined from `errors[].message` as "Cloudflare Workers AI error: …"), wrapped in `cloudflareClassifier` (`providers/cloudflare-stream.ts`, the `{CLOUDFLARE_ACCOUNT_ID}` base-URL substitution for classifiers, with `resolveCloudflareModel` generalized to any model with a `baseUrl`). Request building and answer parsing move from `typesafe-system-one.ts` into a shared `api/system-one-shared.ts` (`classifySystemOne(transport, …)`), which the base port should mirror rather than duplicate; `KnownClassifierApi` gains `"cloudflare-workers-ai-system-one"`, and `api/cloudflare.ts` exports `CLOUDFLARE_WORKERS_AI_REST_BASE_URL`. Its generator hunks are catalog-only. Upstream CI failed on it until `567469096` fixed two model-count tests. |
| 4 | **Azure OpenAI responses** | 364 LOC | ~1 | JSON over HTTPS, header auth. | **+1 (2026-09-24)** — `002fc8385`: `stream()` forwards `options.onProviderStreamEvent` into `processResponsesStream`; landing on the Go responses loop inherits `openai_responses.go`'s callback site (shipped this cycle). Oracle: `azure-openai-base-url.test.ts` › 'forwards parsed events in order before normalizing the response' (response.created + response.completed, identity, responseId `resp_azure`). |
| 5 | **Google Vertex** | 710 LOC | ~2 | IN under E2's transparent-wrapper rider (`@google/genai`). ADC is the risk — scope it to the credential paths Go reaches with stdlib. | **+1 (2026-09-24)** — `002fc8385`: `onProviderStreamEvent` fires first in the googleStream loop with the `generateContentResponseFromVertex` shape (+ `sdkHttpResponse`); mirror `google.go`'s shipped callback (the Generative-AI half shipped this cycle). Oracle: the Vertex case of `google-raw-stop-reason.test.ts` › 'forwards each SDK chunk in order before normalizing it'. **Rider (`a328aa89a`):** route `options.headers`/`model.headers` through `headerObject.applyAsRecord(h, literals...)` (the case-folding `providerHeadersToRecord` shipped this cycle), passing the SDK's own default headers as literals in their exact spelling, never a bespoke merge. |
| 6 | **Mistral conversations** | 963 LOC | ~1.5 | JSON over HTTPS, header auth. | **3** — `6c87d9a02` (2026-08-29: merge indexed Mistral tool-call chunks — `consumeChatStream`'s `toolBlocksByKey` is rekeyed from the composite `` `${callId}:${index}` `` string to `toolCall.index ?? callId`, so chunks of one call that arrive with differing ids still merge. No Go base: `ai/providers/` has no mistral adapter, only registry-level references in `ai/envkeys.go`, `ai/types.go` and `coding/resolve.go`), `96617628e` (2026-09-08: `usesReasoningEffort` widens from the exact ids `mistral-medium-3.5` to the prefix `mistral-medium-`, so every reasoning-capable `mistral-medium-*` model sends `reasoning_effort` instead of prompt-mode reasoning. No Go base: `ai/providers/` still has no mistral adapter, only registry-level references in `ai/envkeys.go`, `ai/types.go` and `coding/resolve.go`), `4bd3f48df` (2026-09-10: `usesReasoningEffort` gains the exact id `zai-glm-5-2`, so Mistral-hosted GLM-5.2 sends `reasoning_effort` when thinking is on instead of `prompt_mode`, which Mistral ignores for that model (#9375); thinking off still sends neither. `mistral/zai-glm-5-2` is already in the shipped catalog as a `mistral-conversations` model, so the adapter must carry the id when it lands, and pi's two new `mistral-reasoning-mode.test.ts` cases are its oracle. No Go base: `ai/providers/` still has no mistral adapter) **+1 (2026-09-24)** — `002fc8385`: `consumeChatStream` takes `onProviderStreamEvent` and awaits it on `event.data` before `output.responseId ||= chunk.id`. Oracle: `mistral-http-transport.test.ts` › 'forwards each parsed SSE payload before normalizing it' (unknown `provider_metadata` round-trips). |
...
| 9 | **Bedrock adapter** | 1,459 LOC | ~3 + consult | **CONSULT ANSWERED (2026-08-31) — TAKE `aws-sdk-go-v2` (with `config`), CONFINED TO A `github.com/sky-valley/pi/providers/bedrock` SUBMODULE** with its own tag series; the root module's graph stays at its two `golang.org/x/*` requires. Back in scope 2026-08-27. The 2026-08-27 note that parity "favours hand-rolling" is **withdrawn**: it is true that there is no pi-authored byte sequence to match, but it priced only the signer. Signing is roughly a third of the work and the credential chain is the rest — and pi carries **no credential-resolution code at all**, so `AWS_PROFILE`, ECS task roles and IRSA (all advertised in `packages/coding-agent/docs/providers.md`) work only because the SDK's default chain is linked in. Hand-rolling ships a documented-feature regression. See the 2026-08-31 ruling. | **2** — `a63fb12c1` (2026-09-01: `utils/node-http-proxy.ts` rewrites `NO_PROXY` matching — entries are normalized by stripping a leading `*.`/`.`/`*` to a bare domain, then matched by exact host OR dot-suffix, with `stripBrackets` applied to BOTH the entry and the target host, and port-scoped entries parsed for bare and bracketed IPv6. `bedrock-converse-stream.ts:207` is one of the file's only two importers, so this lands with the adapter. **Do NOT assume `http.ProxyFromEnvironment` covers it** — measured non-equivalence, see the 2026-09-01 cycle note), `8a7b0c03d` (2026-09-15: `bedrock-converse-stream.ts` `handleMetadata` sets `usage.cacheWrite1h` to the sum of `event.usage.cacheDetails[].inputTokens` over entries whose `ttl === CacheTTL.ONE_HOUR`, so the shared cost path bills Bedrock's 1h cache writes at 2× input (#9457). Checked against the 2026-07-21 generic-collapse lesson and it does NOT collapse: the generic half — `Usage.CacheWrite1h` (`ai/types.go:446`) and the `rateInput*2*longWrite` term in `ai/models.go` `CalculateCost` — shipped with the Anthropic port, and upstream's `models.ts` is untouched in range; the delta is adapter-local extraction with no Go home until the adapter lands. Oracle: `packages/ai/test/bedrock-cache-write-1h-cost.test.ts` (new, 78 lines) as amended by `4c2d91339`, which derives the expected `cost.cacheWrite` from `model.cost` instead of the literal 7.75. **Rider:** the comment at `ai/types.go:444–445` says only Anthropic reports this split — true of the port today and false the day this lands; fix it in the same commit) **+1 (2026-09-24)** — `002fc8385`: `onProviderStreamEvent` fires first for every SDK union item, exception items included, BEFORE the adapter throws on them. Oracle: `bedrock-raw-stop-reason.test.ts` (errorMessage `bedrock stream failed`). **Rider (`a328aa89a`):** custom headers via `headerObject.applyAsRecord(h, literals...)`, the SDK's defaults passed as literals. |
| 10 | **Codex adapter** | 2,228 LOC | ~3 + consult | **CONSULT ANSWERED (2026-08-31) — NO DEPENDENCY REQUIRED TO SHIP.** Upstream takes no third-party package here (`node:zlib` builtin, `globalThis.WebSocket`), and the consult's premise — "zstd decompression" — does not exist: it is request-body compression only, SSE-path only, with a `return null` fallback to plain JSON. **zstd: take nothing**, mirroring upstream's own null-compression branch. **WebSocket: deferred to the WS slice**, pre-decided as `github.com/coder/websocket` in the **root** module (not a submodule — see the ruling). Ship SSE first; an SSE-only adapter is an upstream-defined runtime state, not a parity gap. **Blocked on entry 1** — Codex is OAuth-only and the port's OAuth seams have zero implementers. Back in scope 2026-08-27. | **2** — `a63fb12c1` (2026-09-01: the same `utils/node-http-proxy.ts` `NO_PROXY` rewrite; `openai-codex-responses.ts:972` is the file's other importer. Shares the delta with entry 9 — **the two entries do not sum**, it is one upstream change landing in whichever adapter ships first), `e86102f18` (2026-09-10: `buildRequestBody` stops collapsing a `null` `thinkingLevelMap.off` into `"none"` — `reasoningEffort: "none"` now sends the mapped value only when the key is DEFINED, so a null mapping suppresses the `reasoning` block instead of forcing `effort: "none"` — and a request that passes NO `reasoningEffort` at all now sends `reasoning: {effort: thinkingLevelMap.off ?? "none"}` whenever the model reasons and `off` is not null. No Go base: `grep -rin codex --include='*.go'` finds only the `APIOpenAICodexResponses` api id, a tool-call-id provider-set entry and a default-model row — `ai/types.go:583` states the adapter is deliberately not ported. Checked against the 2026-07-21 generic-collapse lesson and it does NOT collapse: `openai_responses.go` is a different adapter with its own effort mapping) **+1 (2026-09-24)** — `002fc8385`: `mapCodexEvents` fires the callback for every parsed SSE and WebSocket event before the type check; a throwing callback is wrapped as `ProviderStreamEventCallbackError` (message `formatThrownValue(cause)`), which `isCodexNonTransportError` accepts, so it skips the WebSocket retry and the SSE fallback. Oracle: `openai-codex-stream.test.ts` (5 events, `response.done` in the WS fixture). |
| 8 | **Agent harness + search** | 10,273 LOC src (+5,733 LOC upstream test) | see below | **FUNDED 2026-08-27** — the owner ruled it in. Active drain, not a parked item. **Shape not yet fixed:** the harness is a parallel implementation of surface `coding/` already has, so the estimate depends on the shape chosen — see "Harness shape" below. Backlog: **12** against its own tree (13 minus `e7fb8eb2a`, reassigned to entry 7) — of which 3 are already satisfied in `coding/` and 3 are upstream dead code, leaving **5** load-bearing (`3e4bc2680` is the fifth, added 2026-09-09, and it is the first entry here to arrive already part-shipped). See "Harness delta" below. **Slice 8b SHIPPED 2026-08-28** (`b677517`, `ce76e94`, `cd0e3b9`, `1f49233`), and the re-measure split it into **8b-i** (`ExecutionEnv`, harness source, 7 symbols closed) and **8b-ii** (the seven `*Operations` seams, coding-agent source, invisible to this entry's counter) — 8b-ii ships with a named remainder. The entry stays open on 8c and 8d, and the backlog count is unaffected because 8b was a base-port slice rather than one of the deferred commits. | **15** — `cf8d5fac3` (2026-09-18: `agent/src/pico/{index,memory-storage,types}.ts`, 494 new lines — the Pico storage foundation lands OUTSIDE `harness/`, a sibling of it, so it is this entry's tree by feature rather than by path; filed here with `46b66c59a` so the Pico line stays in one place. The commit's OTHER hunks are `n/a`: `ai/src/types.ts`'s `JsonRepresentation`/`IsJsonCompatible`/conditional `ToolResultMessage<TDetails>` and the `ToolResultMessage<unknown>` → `ToolResultMessage` ripples through `harness/execution/tools.ts` and `harness/runtime/drive/**` are type-level only — **E3** — and Go's `any`/`json.RawMessage` already carries them), `46b66c59a` (2026-09-14: `agent/src/harness/pico3/**`, ~7,500 src lines across 15 files plus 24 test files. The hardened Pico3 kernel — scheduler, membrane, view, memory, session, and the `kinds/**` generation/tool/job/plugin surface. **It survives at upstream HEAD**: `56cd5989e` deleted only the earlier `harness/pico/**` prototype and `729d5cb74` deleted only the design docs, so reading either as "Pico was reverted" is wrong — check the tree at the sha, not the subject lines. No Go base anywhere: `grep -rn --include='*.go' -e Pico -e pico` is empty), `c37b0e03b` (2026-09-09: the retry-cap change's harness half. `DEFAULT_RETRY_POLICY` gains `maxAgentDelayMs`, `validateRetryPolicy` validates it, `NormalizedRetryPolicy` carries it, and `runtime/drive/retry.ts`'s local `retryDelay` is DELETED for pi-ai's new shared `retryDelayMs` — so `retryNotBefore` now takes the whole policy rather than a bare `baseDelayMs`. No Go base: `grep -rn --include='*.go' -e NormalizedRetryPolicy -e retryNotBefore` exits 1 and there is no drive/lane runtime at all. The pi-ai half of the same commit SHIPPED this cycle as `ai/retry_loop.go`'s `RetryDelayMs` + `MaxAgentDelayMs`, so this row carries only the harness wiring), `3e4bc2680` (2026-09-08, a 12-commit merge of `dev-named-forks-streaming`; 23 files, every source file under `packages/agent/src/harness/**`). Named-branch streaming forks. `fork-policy.ts` is re-cut from a classifier into a projector: `classifyForkAddress`/`ForkDisposition` are DELETED for `selectBranchFork` + `projectForkCurrentStateWrite`, which rewrites `pi.branch.tip` to the destination tip and emits a fresh idle `pi.lane.state` in place, instead of tagging those rows "reconstruct" for a tail append. JSONL parse/publish splits into a new `session/jsonl/io.ts`, and the whole-file snapshot fork becomes a two-scan streaming fork (`session/jsonl/fork.ts`, `runJsonlFork`) that never writes its source, stops at the `nextSeq` captured on the source commit queue, and throws on a transaction straddling that boundary. `legacy-v3.ts`'s eager `normalizeLegacyV3Records` becomes a streaming `LegacyV3Source`: forking an **open** v3 session now rejects, a **closed** one forks through format 4. `MemoryStorage.fork()` builds the destination state directly via `InMemoryStorageState.createFork`. The same merge also RELAXED WP08 — seq preservation left the fork contract and "bounded memory" became "streaming, index may be resident" — so port the relaxed contract, not the work package as first written. **The `FileSystem` half is NOT queued: it SHIPPED 2026-09-09** (`7c3ea77`). `harness/types.ts`'s `FileSystem` is `coding/execenv.go` (slice 8b-i), so `openTextLineReader` + `TextLine`/`TextLineReader` landed as `OpenTextLineReader`/`TextLine`/`TextLineReader`, `ReadTextLines` was rebuilt on the reader exactly as upstream rebuilt `readTextLines`, and `splitLines` died with its last caller. **The general lesson, worth more than the delta:** "this tree is queued" is a statement about a tree, and a commit that touches a queued tree can still touch shipped Go surface inside it. Split the merge by hunk before filing it, or a portable delta gets parked under a heading that says the Go code does not exist. **Still queued (the rest):** no `SessionRepo`, `ForkOptions`, JSONL v4 storage, lane or label surface exists anywhere in `agent/` or `coding/` — `coding/session_store.go` is an append-only v3 recorder and `coding/session_tree.go` a whole-file parser. That half lands with slice **8c**, sequenced inside entry 11. **Path correction, 2026-09-20 — the detector earned its keep.** `080160162` MOVED the Pico kernel out of `packages/agent/src/pico` into a NEW published top-level package, `packages/durable` (`@earendil-works/pi-durable`, versioned with the rest at 0.86.1, `exports` mapping `.` to `dist/index.js`). Nothing in a pathspec written before that day would have printed it; the no-pathspec detector sweep did. `a16ccd9be` and `0db565924` then reworked it in place — record and query contracts tightened, memory-storage queries indexed. The tree is IN scope on reachability (a published SDK package, no exclusion fires) and QUEUED as before; only its path changed. **Do not read `packages/agent/src/pico` as evidence of anything again** — it is gone, and a grep for it at HEAD returns nothing. **+2 (2026-09-22)** — `47a18e37b`: `harness/tools/image.ts`'s `detectSupportedImageMimeType` requires the full `GIF87a`/`GIF89a` signature, the same fix `coding/tools.go` shipped this cycle for `utils/mime.ts` (pi #9755); land it with the harness image tool. `10d1ad621`: `harness/pico3/chord.ts` moves from `state` + `publish()` to the transactional `change(ctx, draft => …)` / `replace(ctx, value)` API (see entry 11's +1 the same day). **+6 (2026-09-23)** — `packages/durable` (`@earendil-works/pi-durable`): `8158b0321` versioned document storage (memory storage + types); `5901c9b9e` a SQLite storage backend on Node's builtin `node:sqlite` (`storage/sqlite/**`, new `./storage/sqlite` and `./storage/sqlite/node` exports) — the driver question is already answered by the 2026-08-31 ruling: `modernc.org/sqlite` in a `backends/sqlite` submodule, never the root module; `898ab8040` a JSONL storage backend plus `env/**` (node environment, output capture, truncation, adaptive publisher); `b313731b8` JSONL sidecar reclamation. `9a139c62b`'s agent half: `harness/pico3/legacy-tracker.ts` (new, a flush-shaped adapter over chord's immutable tracker), `session.ts`, `view.ts` — build them on `chord/delta`'s `Tracker`/`Change`/`Prepared`, which shipped this cycle. And a trap for the harness port: `d192bd6dc` did NOT touch `harness/compaction/compaction.ts`, which at `b313731b8` keeps the old `TURN_PREFIX_SUMMARIZATION_PROMPT` and `<conversation>` wrapper, so the harness must carry its own constants, never reuse `coding`'s (the same name now holds a different value in each place). **+1 (2026-09-24)** — `b45597504`: `packages/durable`'s storage conformance suite moves from `test/` into a published `./testing` export (`createStorageConformance`, `registerStorageConformance(runner, name, withStorage)`, `createExpectAssertions`, the `StorageConformance*` types) and the memory/JSONL/SQLite backend tests are rewired onto it. Land it with the durable base port as a Go testing helper package, the way `server/src/testing` became `server/internal/servertest`. **+6 (2026-09-25)** — `packages/durable`: `601437d5a` `env/utils/output-capture.ts` reports `truncatedBy` from the retained window's own decision and a trailing newline's `lastLineBytes`, keeps a four-byte allowance past twice the byte limit, and fixes `truncateHead`/`truncateTail`'s `truncatedBy` (only omitted lines prove the line limit) — durable's fork only: `coding-agent/src/core/tools/truncate.ts`, which `coding/truncate.go` mirrors, keeps the old rule at `49681e1b7`, so the port must not carry the fix across; `3f0573877` exports the storage benchmark workloads from `./testing`; `19a0361be`'s durable half, transactional sessions and documents (`session/{session,transaction,publications}.ts`, `documents.ts`, the storage backends and `types.ts`), which copies with chord's `copyJson` — build it on `chord.CopyJSON`, shipped this cycle; `5d4de953c` checkpoints and document migrations; `5fd446ca1` document replay (conformance cases added). Also `d5cba1d97`'s `harness/pico3/legacy-tracker.ts`: it drops its Proxy wrapper over chord's drafts, since the overlay itself deletes an object member written `undefined` — build it on `chord/delta`'s overlay `Tracker`. |
| 11 | **The 2026-09-03 architecture migration** — `packages/chord` (new published package, 39 files), `packages/protocol`/`client`/`server` rebuilt on service-addressed routing, `packages/agent` grown into the durable-drive harness (214 files), `coding-agent/src/experimental/**` (~8k LOC) | see map | ~77 slices | **OPENED 2026-09-03** under the mirror ruling. Carries the ~50 in-scope commits of the dev-branch fast-forward that are not Phase 0 fixes, **and the eleven deletions**: `protocol/schemas.go`, `server/{protocol,sessions,snapshots}.go`, `server/internal/servertest/service.go`, `client/{session_handle,state}.go`, `coding/{remotesession,transcript}.go`, `ai/providers/cloudflare_gateway_binding.go` die with their upstream sources. Zero new root-module dependencies — every candidate (esbuild, json-patch, go-diff, uuid, sqlx, migrate, fxamacker/cbor, x/sync) was argued down to stdlib; `modernc.org/sqlite` goes in a `backends/sqlite` submodule. Order: chord → protocol → (server+client ∥ agent/harness) → experimental → sqlite. **Chord's `bundler.ts` + `node/bundle*.ts` are NOT mirrored** (JS facet bundling has no Go subject — owner, 2026-09-03). **Strict-decoder boundary, settled 2026-09-03 (standing formula, not an owner call):** the any-tree strict decoder in `protocol/decode.go`+`unions.go` moves to `internal/jsonstrict`, imported by BOTH `protocol/` and `chord/`; upstream's own `boundary.test.ts` permits `node:` builtins and an `internal/` package is the Go analogue. Layering stays upstream's — `protocol` → `chord`, never the reverse — and `chord.IsValue` does not accept `cbor.OrderedObject`. **The pin sha was advanced to `64eeb82a4` on 2026-09-04 ahead of this row draining (owner's call), so THIS ROW is now the only record of the unported balance — a drift check that reads the pin alone will miss it.** | **+1 (2026-09-20)** — `b7f788194`: `experimental/client.ts` awaits the terminal remote-prompt event before resolving, 15 added lines. Small, but it is the class of bug that only shows up under a real peer: without the await the client can resolve a prompt before the peer's terminal event arrives, so a caller sees an incomplete turn. **+1 (2026-09-22)** — `10d1ad621` "transactional replicated state": `MutableReplicatedState` drops the `state` proxy + `publish(context)` for `change(context, mutate(draft))` (synchronous copy-on-write, handles revoked on return, async callbacks and non-JSON values throw) and `replace(context, value)`; new `chord/src/state/{draft,diff,value}.ts` (~1k LOC) computes the op delta from a draft; `services/{state,provider}.ts` rebuilt on it; `experimental/services/{connection,models-provider,server,transcript-provider}.ts` migrated. **Its `delta/index.ts` half shipped** (`4fd7875`, the `m` permute op) — the rest waits on this row's runtime slice, since the port mirrors only chord's value/wire types (D52). **Drained 2026-09-23:** its `chord/src/state/{draft,diff,value}.ts` half shipped with the `9a139c62b` port (`0dcd440`, `chord/delta/{draft,diff,value,tracker}.go`) — `9a139c62b` moved those files into `delta/` (`state/diff.ts` replaced by `delta/diff.ts`), so the `state/` paths no longer exist upstream. Still on this row from `10d1ad621`: `services/{state,provider}.ts` and `experimental/services/**`. **+1 (2026-09-23)** — `9a139c62b`'s services half: `services/state.ts` rebuilds `MutableReplicatedStateImpl` on `Tracker.beginChange`/`prepare`/`adopt`, adds `ReplicatedStatePublisher`, `AttachedReplicatedStateImpl` and `attachReplicatedStateSource`, and `ReplicatedStateReplica` adopts through `JsonRevisionStore.adopt`; `api.ts` gains the `replicatedState(source, options)` overload; `types.ts` adds `ReplicatedStateSource`/`SourceAttachment`/`SourceFrame`/`SourceOptions` and `AttachedReplicatedState`; `services/provider.ts` takes a member's initial snapshot in one step (`member.state.snapshot()`), and `services/state-internals.ts` replaces `sequence`/`value` with that atomic `snapshot()` — keep the atomic-snapshot contract when this lands; `src/index.ts` re-exports `Draft` from delta and the new types. No Go home (D52). Build it on `chord/delta`'s shipped `Tracker`, and port the replica's revision validator with it (D76; `JsonRevisionStore` is gone — see the +1 of 2026-09-25). **+1 (2026-09-25)** — `d5cba1d97`'s services half: `ReplicatedStateReplica` validates each revision with the new `delta/revision-validator.ts` (`JsonRevisionValidator`, "Replicated state …" texts, skipping containers validated in earlier revisions) where it adopted through `JsonRevisionStore.adopt`, which is gone; `api.ts`/`types.ts` document `replicatedState`/`replace` as taking ownership of an alias-free strict-JSON root, and `change` as an overlay mutation that copies placements and deletes an object member assigned `undefined`. Port the validator with the replica (D76). |
| 12 | **`SessionManager` / `AgentSession` base port** — `core/session-manager.ts` (1,746 LOC), `core/agent-session.ts` (3,524 LOC) | 5,270 LOC src | ~6 as hunk-ports, ~16 as base ports | **OPENED 2026-09-03.** The port has no `SessionManager` class: `SessionRecorder` is an append-only writer keyed to a file, `SessionTree` a read-only parser; there are **no label entries** (`grep -c label coding/session.go coding/session_store.go coding/session_tree.go` = 0; the only `coding/session*` hits are a fixture and comments in `session_tree_test.go`), no `createBranchedSession` fork writer, no `inMemory` constructor. Read-side version migration is NOT a gap: `coding/session_store.go` `readSessionEntries` → `migrateSessionEntries` ports `migrateToCurrentVersion` V1→V2→V3 on both read paths. `coding/session.go` has `WaitForIdle` but no `IsIdle` and no compaction/branch-summary abort controllers. **`core/session-manager.ts` is PARTLY ported — split it by HUNK (2026-09-16):** its static discovery surface is `coding/session_store.go` — `getDefaultSessionDir` → `DefaultSessionDir`, `list` → `ListSessions`, `LatestSession`, `findById` → `FindSessionByID` (`57a3304`), `readSessionHeader`/`readSessionHeaderForDiscovery`, the `list`/`findById` cwd-filter prologue (`sessionListingDir`, `sessionCwdMatches`), plus `generateId`, the safe-path encoding and the migrations — so a hunk on that half is an ordinary `port` (the row-8 lesson: this cycle's draft parked `findById` here first, and the adversarial pass caught it); only hunks on the class itself (labels, fork, `inMemory`, tree navigation, compaction/abort state, `SessionInfo`'s full shape) queue here. Known remainder inside the ported half, carried by this row: `SessionInfo` is a five-field shape, and `ListSessions` orders by header timestamp where pi's `list()` orders by `modified` (last message activity → header → mtime). | **8** — `2631b25c3` (preserve compaction boundary when forking: `firstKeptEntryId` re-chained through removed labels), `2b768ba42` (`inMemory(cwd, options, entries)` + `_loadEntries`), `bea67d90d` (abort cancels compaction + branch summary; `isIdle` includes compacting), `e266507b6` (drop duplicate `auto_retry_end` event type), `e44d75c20` (2026-09-04: branch-summary output cap `maxTokens: 2048` → `Math.min(4096, model.maxTokens > 0 ? model.maxTokens : Infinity)`, plus the `reserveTokens` doc correction — it reserves for *history selection*, not for prompt+response. No Go base: `coding/compaction.go` ports the COMPACTION summarizer (`summaryMaxTokens(frac, reserveTokens)`), a different function; `generateBranchSummary` itself is unported and its only `src` consumer is `core/agent-session.ts`. The port renders branch summaries (`coding/messages.go`, `coding/session_tree.go`) but never generates one), `e687434a6` (2026-09-07: `navigateTree` gains an `isCompacting` guard beside its existing `isStreaming` one, throwing "Wait for the current compaction or tree navigation to finish before navigating the session tree." No Go base: the port has no tree-navigation API at all — no `Navigate`/`SetLeaf`/`ResetLeaf`/`Branch` surface and no compaction-state flag in `coding/`). `c37b0e03b` (2026-09-09: `_scheduleAutoRetry`'s delay becomes `retryDelayMs(settings, this._retryAttempt)` instead of the inline `settings.baseDelayMs * 2 ** (this._retryAttempt - 1)`, so the AgentSession auto-retry inherits the 60s cap. No Go base: `grep -rni --include='*.go' -e auto_retry -e retryPolicy -e retryCallbacks coding/ agent/` returns nothing — the port has no AgentSession auto-retry loop. The shared helper it now calls DID ship this cycle as `ai.RetryDelayMs`, so this delta is the call site only), `46bde88a1` (2026-09-07: the four `settingsManager.getCompactionSettings()` call sites become `getCompactionSettings(model)`, so compaction budgets resolve against the CURRENT model at each call rather than once. The resolution itself is host surface — `core/settings-manager.ts`'s new `compaction.modelOverrides` table, E1, and the port has no settings reader (`grep -rn SettingsManager --include='*.go'` finds three comments and no type) — but the CALL SITES are `core/agent-session.ts`, this entry's tree, and the portable semantic is that pi RE-RESOLVES per call where `coding/compaction.go` freezes `CompactionSettings` at `EnableCompaction` while reading `s.Model.ContextWindow` live. Filed here rather than `n/a` on this file's own rule that a queued tree is never `n/a`; that is not in tension with 2026-09-09's `faa9863cb` ruling, which turned on a provable no-op — the whole path there was gated on an extension runtime the port lacks, and nothing gates this one). **Counter corrected 2026-09-08: this cell read `**4**` while enumerating FIVE deltas** — `e44d75c20` was appended by `cb254e1` without bumping the number — so appending `e687434a6` took it to 6, not 5; the 2026-09-10 cycle's two took it to 8. **+2 (2026-09-20), taking it to 10.** `de2de549b`: compaction cancellation races. `abort()` now sets `_agentRunAbortRequested` while a run is active, and every continuation point in `_runAgentPrompt`/`_handlePostAgentRun` consults it, so an abort during a retry or an auto-compaction stops the loop instead of being overtaken by the next continue; `_willRetryAfterAgentEnd` returns false outright; the summarization auth call takes the compaction `AbortSignal`; and `aborted` is decided by `signal.aborted || cancelledByExtension` rather than by STRING-MATCHING the error message, which is the part worth carrying — the port would inherit the same fragility if it copied the old shape. `3c75b2747`: bug reporting adds `AgentSession.summarizeForBugReport` (asks the current model to describe what went wrong, for a user who declines to share the transcript) and splits `serializeSessionBranch` out of `exportSessionToJsonl`. The rest of that commit is E1 — `modes/interactive/bug-report.ts`, the `/bug` slash command, `utils/zip.ts`, `core/crash-log.ts` and the upload — and the port has no session export at all. **+1 (2026-09-22)** — `466db0fec` "canonical session context boundaries", the SessionManager/AgentSession half. **Shipped this cycle on the static half:** `context_edit` entries, retain-none compaction and the newest-compaction-only rule in `SessionTree.BuildContext`/`BuildProjection` (`fb1f9df`), and `estimateTokens`' system case. **Queued here:** `SessionManager.appendContextEdit` (validation: target on the active branch, editable role, string→text-block normalization for assistant/toolResult), `appendCompaction(summary, null, …)` storing its own id, the fork-path re-chaining of a self-referencing `firstKeptEntryId`, `ReadonlySessionManager.buildSessionProjection`; `compaction.ts`'s projection-based `prepareCompaction` (`findProjectedCutPoint` with the recovery-omission suffix rule, `estimateProjectedContextTokens` invalidating usage after a later `context_edit`/compaction); and `agent-session.ts` making the SessionManager projection canonical for every provider request via `agent.prepareRequest` (assigning `agent.state.messages` no longer changes history), turn-end boundary drafts committed through `agent.finishTurn`, and post-run recovery omitting the failed attempt with a persisted `context_edit` instead of slicing agent state. The agent hooks it builds on (`finishTurn`, `prepareRequest`, `peekQueuedMessages`) SHIPPED (`5c6af5a`). The extension boundary events (`turn_end` result, `agent_before_settle`) are E1. **Two estimate gaps this entry must close when the cut-point rework lands** (measured 2026-09-22 against pi 0.87.0): (a) `SessionTree` converts `bashExecution`, `branchSummary` and `compactionSummary` to user messages at load, so `EstimateMessageTokens` weighs the wrapped text where pi's `estimateTokens` weighs the raw fields (6/30/32 vs 3/5/5 tokens on the reviewer's fixtures) — `BuildProjection`'s per-entry messages carry the converted form, and `findProjectedCutPoint` will consume them, so carry the raw role's weight through the projection first; (b) without trusted usage the port counts every system patch, where pi's `estimateProjectedContextTokens` counts only `getCurrentSystemMessage(projection.messages)` once. **+ (2026-09-23)** — **Shipped ahead of the base port, from this cycle's reviews:** a compaction aborted mid-summary is no longer checkpointed (pi's `throwIfAborted()` before `appendCompaction`; `224fd46`); resuming a session file resumes its compaction (`Session.LoadBranch`; `edc0727`, `11ebece`); and the invalidation half of `estimateProjectedContextTokens` — usage recorded before the latest compaction or `context_edit` is not trusted, and without trusted usage the current system message counts once (`9ad9374`), which closes estimate gap (b) on that path; gap (a) stays open. **Queued here — compaction trigger timing** (measured 2026-09-23 against 0.87.1 by the round-4 review): pi checks at three sites — before the prompt lands, on the last assistant's usage alone; at `agent_end`, on the final assistant's usage (`_checkCompaction`); and between turns, in `_compactBeforeNextAssistantResponse` via `prepareNextTurn` — while the port checks only in the per-request transform, after the new prompt and its system declaration are in the transcript. Measured effects: a large prompt that tips the estimate compacts one turn early and moves the cut; a threshold crossed at the end of a run compacts at the next request with the new prompt already inside, so the cut can land on it; a switch to a model with a larger window right after the threshold skips a compaction pi performs (32 messages sent where pi sends 10); a thinking-level change after the threshold makes the summarization requests carry the new level. With small prompts the bytes are identical. Fix shape: mirror the three sites and keep the transform only to re-apply the checkpoint, locked by a live pi-vs-Go capture whose prompts are big enough to move the cut; lock two estimate mutants with it (the current system message dropped from the estimate, and usage distrusted exactly at the boundary). Drains with the `agent.prepareRequest` item above. **+1 (2026-09-24)** — measured at `a328aa89a` `agent-session.ts:1677-1690`: `AgentSession.prompt()` throws before any streaming when neither `hasConfiguredAuth(provider)` nor `checkAuth(provider)` passes — `Authentication failed for "<p>". Credentials may have expired or network is unavailable. Run '/login <p>' to re-authenticate.` for OAuth, `formatNoApiKeyFoundMessage(provider)` otherwise. `coding/` holds neither text. So pi reaches `a328aa89a`'s chat assertion from `prompt()` only once auth is configured, while the port's session chat guard (`6e78ccc`, `sessionStreamFn`) runs for any session. |
| 13 | **Edit-tool result `details`** — `core/tools/edit-diff.ts` (structured diff + unified patch + `firstChangedLine`) | 556 LOC | ~1 | **OPENED 2026-09-03.** pi's edit returns `details: {diff, patch, firstChangedLine}` (`core/tools/edit.ts:211`); the Go edit tool returns a bare `textResult("Successfully replaced %d block(s) in %s.")` with no details (`coding/tools.go`). Surfaced while auditing every `Details` site during the write-tool port. Not a one-liner: it lands a new structured payload on the session entry and the server bridge, so it carries its own goldens. Slice: port `edit-diff.ts` as `coding/editdiff.go` with parity tests against pi's diff/patch text, then wire it into the edit result. The ~500-line matching/apply engine is byte-identical between upstream's two forks, so it is shared (`internal/`), not duplicated. | — |
| 14 | **`assistant-message-frame.ts`** — `packages/ai/src/utils/assistant-message-frame.ts` (`AssistantMessageFrameEncoder` + the frame reducer) | ~330 LOC | ~1 | **OPENED 2026-09-04.** IN scope and unported: it is root-exported pi-ai SDK surface (`packages/ai/src/index.ts:39`) and no exclusion test fires, but the port has no frame encoder at all (`grep -rl 'FrameEncoder' --include='*.go'` is empty) and no row named it. Added upstream by `a829de0a7` (2026-08-17) and it has taken **four** commits since with nothing landing port-side — the 2026-09-03 pin advance recorded only the streamSimple half of `5c6655e76`, not its framing half. Same class as **K15** (`utils/overflow.ts`): published SDK surface whose only consumer is the unported durable-drive harness, so it drains with entry 11 rather than ahead of it. | **2** — `8b5899dce` (2026-09-03: `isJsonPrefix` relaxes the catch-up check — a legacy grammar call's delta stream starts at an empty input, so its parsed arguments may EXTEND the start snapshot rather than reproduce it exactly), `0fdec07ba` (2026-09-03: `cloneStartMessage` carries `providerThinkingLevel` through the frame) |
| 15 | **Mirror pi's attribution placement** — `core/provider-attribution.ts` is homed in `ai/providers/attribution.go`, and pi's new `providers/opencode-headers.ts` has no Go counterpart | — | ~1 | **OPENED 2026-09-09.** Deliberately a queue row and **not** a Divergences row: a recorded divergence makes every future finding against it refuted-by-default, and this is fixable work with a known shape. pi splits this surface in two, and `561a2e066` is what made the split load-bearing: the **ai** package injects `x-opencode-session` from `options.sessionId` for the opencode/opencode-go providers alone (`providers/opencode-headers.ts`), while `x-opencode-client`, the `opencode.ai` host gate and the whole default-attribution bundle stay in **coding-agent** and reach the request through `transformHeaders` (`core/sdk.ts:331` → `core/provider-attribution.ts`). The port collapses both halves into the ai layer. That is precisely why `561a2e066` needed no Go change — the port had already closed the gap upstream was closing — and it is why three cells still differ on the **bare-SDK** path, all in the same direction (the port sends MORE than pi's ai package): it also sends `x-opencode-client`; it also honours the `opencode.ai` host gate; and it puts the session header BELOW `model.Headers`, where post-commit pi's bare path puts it above. A fourth, narrower cell — a caller's case-variant deletion marker suppressed the header on the three `defaultHeaders` adapters but not on google — **CLOSED 2026-09-24** (`a328aa89a`'s case-folding `providerHeadersToRecord`, ported in `44bf5a7`; `TestOpenCodeSessionHeaderCallerDeletionMarker` now asserts google too). **None of the three is reachable from `coding.Session`**, which matches pi's coding-agent path exactly. With one Go function standing in for two pi paths that now disagree, the port matches whichever path its own consumers take — and that is the right one to match, which is why this is a ~1 and not urgent. The slice: move the attribution bundle into `coding/` behind the `TransformHeaders` seam the port already has (`ai/models_runtime.go`), leave the ai layer holding only the opencode session header keyed on provider identity, and keep `ai/providers/opencode_provider_headers_test.go` green throughout. **Landing it changes a recorded Go-home mapping** — the `pi-triage` skill's table sends `core/provider-attribution.ts` → `ai/providers/attribution.go` — so fix that table in the same commit, per this file's own rule that the test wins and the list follows. Deferred out of the 2026-09-09 cycle under the 2026-09-02 boy-scout **boundary**, not against the rule: the repair changes a subsystem's shape and carries every provider's header order as its parity surface. | — |
| 16 | **Context-overflow detection** — `packages/ai/src/utils/overflow.ts` | 180 LOC / 1 file (at `16292398a`) | ~1 | **OPENED 2026-09-18.** `isContextOverflow` and its `OVERFLOW_PATTERNS` / `NON_OVERFLOW_PATTERNS` decide whether a failed assistant turn was a context overflow — the signal that drives compaction rather than a retry. The port has **no Go base**: `grep -rn --include='*.go' -e context_length_exceeded -e 'too many tokens' -e 'token limit exceeded' ai coding agent` is empty, and `coding/compaction.go` triggers on its own estimate alone, never on a provider's refusal. So the port cannot react to a provider that says "too long" when the estimate said otherwise. Root-exported (`export * from "./utils/overflow.ts"`), no dependency question, and the patterns are the same JS-RegExp-under-RE2 translation `ai/retry_classify.go` already solves with `buildProviderErrorPattern` — so this is a small, well-shaped slice, not a research problem. Land it beside `retry_classify.go`, not inside `coding/`. | **1** — `661619e87` (2026-09-18: the Cerebras bodyless-400/413 pattern moves OUT of `OVERFLOW_PATTERNS` into its own `CEREBRAS_BODYLESS_OVERFLOW_PATTERN`, applied only when `message.provider === "cerebras"` (#9482). A bodyless 400/413 from any other provider stops counting as an overflow. Port it WITH the base, not after: shipping the base without it would build in the bug upstream just fixed.) **+1 (2026-09-20)** — `0e283203c`: `OVERFLOW_PATTERNS`' first entry widens from `/prompt is too long/i` to `/prompt (?:is )?too long/i`, so z.ai's `{"code":"1261","message":"Prompt too long"}` is recognised as an overflow instead of a hard failure. z.ai also overflows SILENTLY on other paths (`usage.input > contextWindow`), which the same file handles; the doc comment is updated to stop claiming z.ai never errors. Land the widened pattern with the base port — it is one character class, but it is the whole value of the row for z.ai users. **+1 (2026-09-22)** — `466db0fec`: `_checkCompaction` now treats an explicit `stopReason: "error"` overflow as recoverable only while the assistant is still projected (no later `context_edit` hid it), and the recovery omits the attempt with a persisted `context_edit` (entry 12) rather than dropping it from agent state. Land together with entry 12's recovery half. |
| 17 | **Prompt cache warming** — `core/cache-warmer.ts` (437 LOC) plus its wiring: `core/cache-stats.ts`, `core/usage-totals.ts`, and the `createAgentSession` streamFn refactor in `core/sdk.ts` (`provider-composer.ts`'s `promptCache` merge was listed here until 2026-09-24; it is the models.json layer, E1 — see the out-table) | ~500 LOC src | ~1.5 | **OPENED 2026-09-20** (`c596d09d9`, rider `3390bd936`). pi issues SPECULATIVE provider requests to keep a prompt cache alive between turns, driven by the new `Model.promptCache` lifetimes. The data half SHIPPED this cycle (`120e5d9`: `ai.ModelPromptCache` + `Model.PromptCache`, and the 0.86.1 catalog carries it for 14 anthropic models), so the port reads the lifetimes and does nothing with them. The warmer itself has no Go base: `grep -rn 'CacheWarmer\|cache_warm' --include='*.go'` returns nothing, and its collaborators are absent too — no `UsageTotals`, no `CacheStats`. **Not E1**, and that is the call worth recording: its consumers are mixed (`core/sdk.ts` and `core/agent-session.ts` alongside `settings-manager`, `model-registry` and the interactive footer), so the only-consumers clause does not fire and the default is IN. It changes what requests reach the provider, which is SDK behaviour, not presentation. `3390bd936` is already a bug fix on it — a late refresh must be skipped once the context it was warming has moved on — so expect the upstream shape to keep moving; take it after entry 16, which is smaller and unblocks compaction. | **1** — `3390bd936` (skip late cache-warming refreshes) |
| 18 | **A model on the tool execution context** — pi's `AgentTool.execute` receives a ctx carrying the request's `model`; the port's `agent.AgentTool.Execute(ctx, toolCallID, params, onUpdate)` does not | ~1 | ~1 | **OPENED 2026-09-20** (`f5c946480`). The read tool reads `ctx?.model?.inputLimits?.images?.resize` to pick an image resize profile, and pi's own `ReadToolOptions.resizeOptions` exists as the fallback for a context with no model. The port shipped that fallback this cycle (`c87df16`) as an `imageResizeFn` getter installed by `NewSession`, read per call so a mid-session `SetModel` still reaches the tool — which covers this case completely. The row is open because the SEAM is missing, not the behaviour: every future upstream change that reads something else off `ctx.model` inside a tool will need the same bespoke getter, and at the second one that is a worse trade than widening `Execute`. Cheap, and an API break the mirror ruling already permits. | — |
| 19 | **pi.dev remote model-catalog overlay** — `core/remote-catalog-provider.ts` (`withRemoteCatalog`, the stored-overlay restore, the ETag/Last-Modified refresh protocol, `?types=` negotiation, `minimumPiVersion`) | ~250 LOC src at `a328aa89a` | ~1.5 | **OPENED 2026-09-24.** Filed `n/a` since 2026-07-17 under E1's only-consumers clause, whose premise is false at `a328aa89a`: its sole importer is `core/model-runtime.ts`, which `createAgentSession` (`core/sdk.ts`, SDK) defaults via `ModelRuntime.create`, and `ModelRuntime.create` wraps every built-in except radius and runs `refresh({allowNetwork:false})`, which publishes the restored stored overlay; the networked path is reached from `experimental/services/models-provider.ts` (entry 11). Mixed consumers, so the clause does not fire and no other E1 category fits a catalog fetch. No Go base (`grep -rn --include='*.go' -e 'api/models/providers' -e withRemoteCatalog -e parseCatalog ai coding agent cmd internal` is empty apart from the attribution HTTP-Referer); the `ai.ModelsStoreEntry.{Etag,LastModified,CheckedAt}` seams are already ported and latent. Wire note: `url.searchParams.set("types", "chat,image,classifier")` percent-encodes the commas (`types=chat%2Cimage%2Cclassifier`), which `url.Values.Encode` reproduces and a hand-built query does not. Oracles: `remote-catalog-provider.test.ts`, `model-catalog-protocol.test.ts` (`7fd564cbb`: the `?types=` request, the 307 to `&pi-version=<VERSION>`, per-`minimumPiVersion` selection). | **1** — `a328aa89a`: type-aware overlay — `REMOTE_CATALOG_MODEL_TYPES = ["chat","image","classifier"]`, `parseCatalog` keeps entries whose `type` is undefined or a listed string (drops `"video"`; drops `type: null`, unlike pi-ai's `getModelType`), `mergeModels` replaces by `(getModelType, id)`. |
...
already shipped ahead of it. Entries 16, 17 and 18 are off that spine and
depend on nothing in it — take them in any cycle with room, smallest first:
18 is an afternoon, 16 is one slice, 17 is about one and a half.
...
the matching npm build. **Currently 3 (2026-09-24: `fde38ed7c`, `a328aa89a`; 2026-09-25: `a7d17e39a`);
drained 1 → 0 on 2026-09-23 by the 0.87.1 regen (`e953886`).** Every drained row (`b4588f26a`, `db91e0331`,
`27c072e98`, `b7f4b05c7` and the carried `1a584a7a5`) was checked as an
ancestor of the `v0.87.1` release commit (`git merge-base --is-ancestor <delta>
f07218c4d`, per row, never by log order) and landed as its generator diff
predicted; `1a584a7a5`'s rider moved `defaultModelPerProvider.xai` to `grok-4.7`
in the same commit, once `xai/grok-4.7` was in the catalog. The measurements are
in `e953886`'s message.

**How the regen is decided, and it is not by reading git.** Catalog data is
generated at publish time and never committed upstream, so a `git diff` over
the delta can never see it (2026-07-30). Execute `JSON.stringify(MODELS)`
against `dist/models.generated.js` in both npm builds and compare. **Verify the
method before trusting the output:** dumping the OLD version must reproduce the
committed catalog byte for byte. On 2026-09-20 that check passed at 592,846
bytes, which is what licensed the new file.

**A queued row can be killed by a later upstream change, and this queue is
where that gets noticed.** `d92eb8d4b` queued `supportsToolReferences: true`
onto the Fireworks compat; `9e05370b2` then deleted that key from
`AnthropicMessagesCompat` outright, along with the row's own oracle. Replaying
it would have emitted a key the type no longer has. It was RETIRED, not
drained. **So: verify every queued row against the generator at HEAD before
replaying it** (`git -C <clone> grep -n "<the key>" origin/main -- packages/ai`)
— a row is a description of code that may no longer be there.

| delta | what regenerates |
|---|---|
| `fde38ed7c` | `generate-models.ts`: the "models.dev may list Opus 5.5 before its effort metadata is complete" override widens from `anthropic/claude-opus-5-5` to also cover `github-copilot/claude-opus-5.5` (`{off:null, minimal:null, low…max}` spread over models.dev's map). The shipped catalog already carries exactly that map (0.87.1's hard-coded entry), so the regen should leave it unchanged even though models.dev now lists the model — this is what turned upstream CI green after `b313731b8`/`4c2dfd936` failed `github-copilot-anthropic.test.ts`. |
| `a328aa89a` | `generate-models.ts`, `model-data.ts` (schema 3 → 6, per-type validation, keys `${type}:${id}`), new `openrouter-catalog.ts`, `generate-image-models.ts` deleted: every chat entry gains `type:"chat"`, image and classifier groups come out of the same run, a `typesafe` provider appears (classifier-only, from models.dev `?type=decision`), per-provider order switches to `localeCompare`. See the model-type tripwire below. |
| `a7d17e39a` | `generate-models.ts`, `openrouter-catalog.ts`: OpenRouter's decision models come from a third listing, `?output_modalities=decisions` (`buildOpenRouterCatalog(listed, imageListed, decisionListed)` gains a `classifiers` group), and Cloudflare Workers AI's System One model `typesafe/jev` is hard-coded (`CLOUDFLARE_WORKERS_AI_CLASSIFIER_MODELS`: no unauthenticated catalog, zero cost, `contextWindow` 32000). Classifier groups only — `MODELS`' chat entries are unaffected; dump `CLASSIFIER_MODELS` for row 3's record. |
...
touching `applyAnthropicAllowedFallbackModelMetadata`. Checked and clean at
0.86.1 (2/2 entries carry it).

**Regen tripwire — thinkingLevelMap is request-body-affecting.** Since
`265a33393` most of the tree derives it from models.dev rather than a
hand-maintained map, so diff it PER MODEL after a regen, not just the model
list. 0.86.1 moved **57** of them, concentrated in fireworks (`6b94ae2ec`) and
google/vertex (`aa50fe778`), and the direction matched each row's prediction.

**Regen tripwire — a hard-coded generator entry can be displaced by models.dev.**
`27c072e98` hard-codes `github-copilot/claude-opus-5.5` (thinking map with `off`/`minimal`
null) "until models.dev includes it", and `b7f4b05c7`'s effort-map override is
anthropic-only. By `b313731b8` models.dev lists the copilot model, so upstream's CI —
which regenerates from live models.dev — ships `[off minimal …]` levels for it and fails
`github-copilot-anthropic.test.ts`. The published 0.87.1 build still carries the
hard-coded entry (`[low medium high xhigh max]`), and the port matches it on all 1,495
models. **At the next regen the build may flip; if it does, the build wins** and
`TestCatalogClaudeOpus55`, which mirrors upstream's expectation, fails by design —
follow the build and move the test with it. **Update 2026-09-24:** `fde38ed7c` extends the
effort-map override to the copilot model, so the next build should keep the
`[low medium high xhigh max]` levels (upstream CI is green again from that sha);
the entry's other fields still come from models.dev.

**Regen tripwire — the model-type catalog shape (`a328aa89a`).** (i) Every
`MODELS` entry gains `"type":"chat"` — the LAST key for models.dev, AI Gateway and
Radius entries, the FIRST for `buildOpenRouterCatalog` ones. It decodes into
`ai.Model.Type` (shipped 2026-09-24); any test that deep-compares a catalog model
against a literal needs `Type: "chat"`. (ii) `MODELS` gains `"typesafe": {}`.
`ai/catalog.go` `LoadBuiltinModels` registers per MODEL, so a provider key with
zero models vanishes from `ai.GetProviders()`, where pi's
`getBuiltinProviders()` is `Object.keys(MODELS)` and lists it — and
`coding/resolve_test.go` `TestDefaultModelsExistInCatalog` then goes VACUOUS for
it rather than red. Record zero-model provider keys when the loader is touched and
mirror upstream's `model-resolver.test.ts` "no chat models ⇒ no default" branch in
the same commit; do NOT paper over it with a `BuiltinModels` skip (pi's
`builtinModels()` registers `typesafe` — Scope queue row 3). (iii) Model key order
churns in six providers under `localeCompare` (baseten, huggingface, openrouter,
qwen-token-plan, qwen-token-plan-cn, together): compare dumps parsed, and still
reproduce the outgoing catalog byte for byte first. (iv) Dump `IMAGE_MODELS` and
`CLASSIFIER_MODELS` for row 3's record only; do not embed them until it lands.

**Regen tripwire — a copilot model moving onto Responses changes its code
path.** `ai/providers/openai_responses.go` carries `github-copilot`-specific
branches, so a copilot model whose `api` flips from `openai-completions` starts
executing code it never reached. `7d8ab31a4` did exactly that to
`github-copilot/gpt-6-astra` at the 0.86.1 regen. Its request body was read by
hand and is correct. **The hole this exposed is now closed:**
`TestCopilotGPTModelsUseResponsesApi` derives every `github-copilot` `gpt-*`
model from the catalog and asserts the Responses api, with a precondition check
so the loop cannot go vacuous — before it, nothing pinned any copilot model's
api and the flip would have happened in silence.

**Regen tripwire — model ids are load-bearing in Go tests.** A regen that
removes a model can turn a green test red or, worse, leave it green while it
silently stops testing anything. **Prefer fixtures that DERIVE the expectation
from the catalog and assert the precondition that makes the behaviour
deterministic.** 0.86.1 removed 56 models, including `deepseek/deepseek-v4-flash`
(`12f59336a`), which two tests in `ai/providers/openai_compat_test.go` named
outright. Both now iterate `ai.GetModels("deepseek")` and fail loudly on an
empty result, so the next rename can leave them neither red nor vacuous.

**Regen tripwire — the difftest harness can be wrong about which side is
ahead.** `difftest/pi/capture.mjs` used to early-return the raw context for
`dist` scenarios because "the published build predates `9e05370b2`". 0.86.1
does not, and that one stale assumption turned **46 of 58** scenarios red at
once, each reading as a port bug — pi apparently missing the system message and
the tools. It now PROBES the backend for `utils/transcript.js` instead. The
general rule: when a bulk of scenarios fails the same way right after an npm
bump, suspect the harness's model of the build before suspecting the port.
...
| — | **`reducer.ts`, JSONL v4, `agent-harness.ts`, `events.ts`** | ~6-8 | **DEFERRED — tripwire FIRED 2026-09-09** | **2,134 LOC src + 2,836 LOC test** — 21% of entry 8's source but **49% of its test mass**, which is itself an argument for deferring. All four are dead or stub code upstream: `events.ts`'s only importer at the pin is its own test, and `agent-harness.ts` is a scaffold. Re-open on the first upstream commit that makes them real. |

**The tripwire — FIRED, and re-pointed 2026-09-09.** Checked because this cycle
touched the harness tree, which is the rule that saved it.

The recorded command
`git show <sha>:packages/agent/src/harness/agent-harness.ts | grep -cE "HarnessNotImplemented|this\.unavailable\("`
was **27** at `ccfe79ed2` and at `56f3f33a9` — not merely equal but the same blob
(`3802900d`, 508 lines), split 5 `HarnessNotImplemented` + 22 `this.unavailable(`.

It returns **0** at `96617628e` and at `be26e3270`, and **that 0 is not the signal
it looks like.** The file was repurposed into a types/contract module (blob
`55b853e1`, 622 lines, last touched `e26afb63a`, 2026-09-02) and the marker was
renamed `HarnessNotImplemented` → `SliceNotImplemented`, which now lives in
`runtime/types.ts`. A command that greps a renamed symbol inside a repurposed
file reports "every stub is gone" whether or not a single line was implemented —
so a tripwire pinned to a path and a spelling reads as PASSED at exactly the
moment it should be firing. **Re-point one the moment its subject moves.**

Re-measured where the harness actually lives now, **the condition is met**:
upstream grew `packages/agent/src/harness/runtime/**` (10 entries, including
`drive/`, `reducer.ts`, `restore.ts`, `transcript.ts`) starting at `5c6655e76`
(2026-08-25); `runtime/harness.ts` is 408 lines of real method bodies; and
exactly **one** unimplemented site survives anywhere in the tree —
`runtime/harness.ts:306`, `throw new SliceNotImplemented("watchSession")`. So
**27 stub sites → 1**. The writer side landed on 2026-08-25 and went unnoticed
for two cycles; this cycle's own delta (`3e4bc2680`, 12 commits of real JSONL v4
fork work) is the second confirmation.

**Consequence.** The DEFERRED row above no longer holds as written: `reducer.ts`,
JSONL v4 and `agent-harness.ts` are not "dead or stub code upstream" any more,
and that row's re-open condition has been satisfied. The deferral itself stands —
that work is sequenced inside entry 11 — but it now stands on COST, not on the
absence of a real consumer, and it may not be defended on the old ground again.

**The re-pointed tripwire, so it stays checkable:**
`git grep -c "SliceNotImplemented" <sha> -- packages/agent/src/harness` is **5**
at `be26e3270`: one `throw`, plus the class, its `name`, its import and its
re-export. **Watch the `throw` count, not the symbol count** — and re-derive both
...
entry 8's backlog is 11 commits against its own tree (12 from 2026-09-09, 13 from 2026-09-10), of which 3 are already
...
— pi omits `max_output_tokens`, the port emits it); a **truthy** mistyped value
on openai-completions' `supportsStrictMode`, default-FALSE since `890f92088`
and tested `!== false` at the tool site (`{"supportsStrictMode":"yes"}` — pi
0.87.0 sends `strict:false`, the port omits the key; unreachable through
`models.json`, whose schema types it boolean); and nested mistyping inside
...
appears nowhere in `ai/`; the six body-marshal sites are in `anthropic.go`,
`google.go`, `openai.go`, `openai_responses.go` (two) and `pi_messages.go`. Value-preserving after parse, which is why nothing catches
...
**D10 — Google: a record entry spelled differently from one of genai's own
defaults arrives comma-joined with it; the port sends the entry alone.** Since
`a328aa89a` pi's `providerHeadersToRecord` folds names case-insensitively (last
slot wins, last spelling kept), so pi's own sources no longer collide. What is
left is `@google/genai` 2.21.0 (the lockfile version at `a328aa89a`): it seeds
`{"User-Agent": "google-genai-sdk/2.21.0 gl-node/<v>", "x-goog-api-client":
<same>, "Content-Type": "application/json"}`, spreads the record over them by
exact key, and fills the request with `Headers.append`, which comma-joins case
variants. Measured: record `{user-agent: X}` → `google-genai-sdk/2.21.0
gl-node/v26.4.0, X` (port: `X`); `{X-Goog-Api-Client: m}` → the genai value
`, m`. `{content-type: text/plain}` → `application/json, text/plain`, which the
port DOES reproduce: Content-Type is passed to `applyAsRecord` as an adapter
literal (`f9c0b54`); `x-goog-api-key` in any spelling is replaced by the record
value, since genai appends the key only when absent (port matches). The gap is
therefore only genai's User-Agent and x-goog-api-client defaults, which the port
never sends (D12). Pinned by `TestGoogleCaseCollidingModelUserAgentWins`.

**D11 — an empty or whitespace-only User-Agent vanishes in Go; pi sends it
present-and-empty.** pi sends `User-Agent: ` for `""`, `" "`, `"\t"` on all five
ported adapters (measured at `8676a0dcd` with @anthropic-ai/sdk 0.124.0, openai
6.40.0, genai 2.21.0). Since `8c4f08f` every header value is trimmed as fetch's
`Headers` trims it, so the whitespace-only values become `""` too, and both
net/http's HTTP/1 writer and its HTTP/2 encoder drop an empty User-Agent. NOT
fixable through `http.Header`, and RFC 9113 forbids a raw `" "` over HTTP/2
anyway. Pinned by `TestEmptyUserAgentIsDroppedEntirely` (its two `""` rows; the
whitespace widening is documented in its comment, not pinned).
...
`google-genai-sdk/2.21.0 gl-node/<v>`). Separately, the port sends no
`x-goog-api-client` header at all on google requests. Since `a328aa89a` this row
also owns google's case-variant marker: opts `{user-agent: null}` folds pi's
record to `undefined`, so pi's wire carries genai's own agent and the port's
`Go-http-client/1.1` (`TestUserAgentMarkerBeatsPiDefault`, weak assertion). The
request headers undici's fetch adds by default are the sibling class, K22. **This is why the marker tests assert only
...
cap applies to the three providers that honour `TimeoutMs` through
`sendWithRetry` (anthropic, openai-completions, openai-responses); google also
routes through it but, like pi's, ignores `TimeoutMs` and waits on undici's 300s
headers timeout instead (`bb33378`), and so does pi-messages, which calls fetch
itself (`undiciHeadersTimeoutMs`). This is
...
parity assertion**, though its own comment reads like one. The Responses path is
not under this row: since `b32a55b` it follows pi (oracle in
`ai/providers/testdata/httperror`); google and anthropic are K18. Nor is the
STREAM error-chunk path (an SSE `data: {"error":…}` chunk): since the
`002fc8385` port it is exact — the status-less makeMessage text plus
`"\n"+String(metadata.raw)` unless already contained
(`ai/providers/testdata/openai-stream`).
...
**D61 — the anthropic adapter accepts one credential pi's adapter does not:
`ANTHROPIC_AUTH_TOKEN`.** `8b5899dce` re-attached pi's `assertRequestAuth` to
`streamSimple`, and the port's gate was widened to match its acceptance set —
an api key, or a non-empty `authorization` / `x-api-key` /
`cf-aig-authorization` request header (`hasAnthropicAuthHeader`,
`ai/providers/anthropic.go`). The port keeps a third arm pi's *adapter* has no
counterpart for: it reads `ANTHROPIC_AUTH_TOKEN` from the env inside the
adapter (upstream `24e5cc04`). pi resolves that env var one layer **up**, in the
provider resolver, into exactly the `Authorization: Bearer` header the first arm
already accepts (`8b5899dce:packages/ai/src/providers/anthropic.ts:24-31`), so
**end to end the two accept the same set** — the port's compat path
(`ai.StreamSimple` → `withEnvAPIKeySimple`, which deliberately leaves `APIKey`
empty for this var) simply reaches it later. The sets differ only for a caller
who invokes `StreamSimpleAnthropic`/`StreamAnthropic` directly with the env var
set and no header, which pi's adapter would reject and this one serves. Pinned
by `TestAnthropicStreamSimpleAuthTokenStillAuthenticates`. Do NOT "fix" this by
deleting the arm — that would break the port's own env-key path, which has no
resolver layer to fall back on.

**D62 — no `cf-aig-authorization` is synthesized without a gateway credential.**
Widening D61's gate made `cloudflare-ai-gateway` reachable with an empty api
key, and `applyAnthropicHeaders` built its gateway bundle unconditionally, so a
header-owned request went out carrying `cf-aig-authorization: "Bearer "` — an
empty credential pi never sends (pi's adapter never mentions Cloudflare; on the
"api key or header-owned auth" branch it passes `apiKey ?? null` and sends the
caller's headers alone). The bundle is now built only from a non-empty key,
which is what pi's resolver does — it produces the bundle, markers included,
only when it HAS a gateway credential. Recorded because the bundle's deletion
markers (`Authorization: nil`, `x-api-key: nil`) are load-bearing: skipping the
bundle also skips the suppression, which is correct precisely because there is
no synthesized header left to suppress. Pinned by
`TestAnthropicStreamSimpleGatewayHeaderOwnedAuthSendsNoEmptyGatewayBearer`.

**D63 — a terminal event releases the consumers it did not reach; pi leaves them
pending.** pi's `push` hands the terminal event to one waiter and leaves every
other registered waiter parked until `end()` resolves them with `{done: true}`
(event-stream.ts `push`/`end` @ `96617628e`). `ai/eventstream.go` closes the
remaining waiters as soon as a terminal event sets `done`, so their `Next`
returns `ok=false` immediately. Observationally identical whenever `end()`
follows — which pi's own producers always do — and it diverges only when a
producer terminates via a `done`/`error` event and never calls `End`, where pi
would park those consumers forever. A pending JS promise costs nothing; a parked
goroutine is a leak. This preserves the behaviour the port has had since the
`sync.Cond` implementation, where `Broadcast` woke every consumer and the
`done` check returned them. Deliberate.
...
`chord/` imports nothing from this module except `internal/jsonstrict`, its
`chord/delta`, and `chord/internal/jsonvalue` (the strict-JSON rules both
share); `chord.IsValue` does not accept `cbor.OrderedObject`. Not mirrored:
`bundler.ts` and `node/bundle*.ts` (JS facet bundling; no Go subject).
...
**D30 — chord core.** IsValue and CopyJSON (`chord/internal/jsonvalue`, which draft placements share) are one Go spelling of strict JSON. Every Go numeric kind is a number, and so is a json.Number (JS has one number type); only finite ones pass; []byte is rejected (upstream rejects Uint8Array; encoding/json would base64 it); structs are rejected as class instances; pointers/funcs/chans/complex have no JSON form; any string-kind map key is an object key, and a typed slice, array or map copies to the plain one. No depth limit since `19a0361be`: a cycle is refused by an ancestor set keyed by a map's header, or a slice's backing array plus its length — a descendant equal to an ancestor in both is the same array, so the key is sound (the claim this row used to make, that no visited set could be, was wrong). Go has no undefined, so upstream's `{omitted: undefined}` case is pinned as an object holding a non-JSON member (a func), and copyJson's `omitUndefinedProperties` has no Go form. A `*delta.Draft` is not a value to CopyJSON, where pi's copyJson reads a proxy through its traps; placed in another draft, a draft is copied as it holds its content now.
...
**D41 — chord/delta.** `chord/delta` — `Overlap(a, b, scan)` is exported as upstream's `overlap` is; its optional `probe` and `maxCandidates`, which no caller in pi passes (diff.ts and delta.test.ts give three arguments), are fixed at their defaults, 64 and 8. It counts scan, probe and result in UTF-16 code units, pi's unit and the unit a "t" count carries; the search runs on bytes with the head/tail boundaries translated, and the pair-splitting cuts pi's units produce (a head ending in a lone high surrogate, a tail opening with a lone low one) are reproduced by a same-high-surrogate check on the following rune and by starting the tail after the split pair. 37 inputs pinned against pi under node (TestOverlapMatchesUpstream, re-checked at `9a139c62b`), including those cuts. The two sides can only differ on strings one of them cannot hold: a Go string with invalid UTF-8 (each bad byte counts one unit) or a pi string with a lone surrogate — neither is a JSON value.
...
**D52 — chord/types+wire.** types.go mirrors only the value/wire types of src/types.ts (ServiceMode, catalogue entry, address, snapshots, updates, ServiceCall). The runtime interfaces there — ReplicatedState/MutableReplicatedState, ServiceSpawner, RemoteServices, ServiceSubscription, RemoteServiceTransport, RemoteServiceBinding(+Options), FacetEnvironment/Facet/FacetHost/FacetLoader/LoadedFacets/FacetOptions/RemoteServiceSource, ReplicatedStateDelivery, and (since `9a139c62b`) the source-backed ReplicatedStateSource/ReplicatedStateSourceAttachment/ReplicatedStateSourceFrame/ReplicatedStateSourceOptions/AttachedReplicatedState behind `replicatedState(source, options)` — hinge on generic methods (use<T>) and the Promise/undefined-vs-null result mapping, which are design decisions of the consumer/provider/state/facet slices; left to them rather than pre-committed here. No Context/JsonRepresentation types per D26/D31.
...
### chord/delta immutable tracking (ported 2026-09-23 at `9a139c62b`; the overlay tracker 2026-09-25 at `9e70c3d50`)

Upstream replaced the diff-based tracker with an overlay (`d5cba1d97`): `track(initial)` takes ownership of the root in O(1), `beginChange()` opens an overlay whose nodes record writes, deletions and structural edits, `prepare()` emits the ops those records mean — without diffing — and materializes the revision, `adopt()` swaps the root and stales every competing change. `diffRevisions` stays, unused by the tracker. The oracle is `chord/delta/testdata/upstream_delta.json`, captured by `testdata/capture.mts` from pi's own `packages/chord/src/delta` at `9e70c3d50` (identical at `49681e1b7`; no npm build carries either): 284 diffRevisions pairs (upstream's state-diff suite, 120 random identity-sharing arrays, the op cap, the anchor and semantic-table budgets, and the snapshot threshold and each cost term at pi's flip point), 118 tracker/draft scenarios, 16 generated cases (dense regions, the op cap, a 20,000-op queue, fragmentation, 100,000-item inserts, a whole-root replacement), state-fuzz's 100 seeds × 100 steps, and 20 random scripts over every draft operation. Every batch is compared byte for byte: a tracker's in pi's own order, a diff's modulo D69.

**D69 — chord/delta.** Object member order, in the diff. JavaScript enumerates an object's keys integer-like first (ascending), then in insertion order; a Go map keeps none, so `DiffRevisions` visits a revision's members — and `Draft.Keys` lists them — integer-like first, then by UTF-16 code unit: the order of an object whose keys were inserted sorted (`Draft.Keys` lists the string keys a change adds after them, in the order it added them, as pi does). Where pi's insertion order differs, a diff lists the same member ops in a different order (sets, then deletes, on both sides; nothing else moves) — upstream's own state-diff case "emits sets and deletes" is one (`change` was inserted before `add`). A tracker's batches are not affected since `d5cba1d97`: the overlay emits in the order the change made its edits, which the Go overlay keeps (`chord/delta/ordered.go`), so they match pi's op for op, in pi's order. encoding/json marshals a payload's members in a third order, byte order (`{"10","9","a","｡","😀"}` in that order, where `Draft.Keys` gives `9 10 a 😀 ｡`), so a pi replica decoding a Go payload inserts them in byte order. The replica is the same value either way. Not fixable without an ordered object in the port's value model, which the port declines for the whole wire (Rulings, "Do NOT make the CBOR decoder order-preserving"); the golden records pi's own diff order as `raw` beside Go's.

**D70 — chord/delta.** The draft is methods, not a Proxy; doc.go maps each trap to its method. Consequences: a write through a settled draft returns `ErrDraftSettled` and a read panics with it (pi throws a TypeError from both); `Has` on an array sees its indices and `"length"`, not `Array.prototype`'s members (`"push" in draft` is true in pi), and `Get` of a named key on an array reads nothing where pi reads the prototype's; `Pop`/`Shift` return `(value, present, err)` where pi returns undefined; an array method on an object draft fails with upstream's "Array mutator called on incompatible receiver" (pi's text for a borrowed mutator), and `SetLen` on an object draft is refused (pi's `obj.length = n` writes a member, as `Set("length", n)` does here); `Splice` always takes a start and a count, so upstream's zero- and one-argument forms have no Go spelling; Go has no undefined, so `Set(k, nil)` writes null where pi's `draft.k = undefined` deletes (that is `Delete(k)` here); a numeric Go key is the key its integer spells, and an integral float past int64 saturates (`Set(1e21, v)` writes `"9223372036854775807"` where pi's `draft[1e21]` writes `"1e+21"`); `Change.State` still returns the draft after the change settles, where pi's getter throws, and fails on first use instead; the traps with no Go spelling (defineProperty, getPrototypeOf, setPrototypeOf, preventExtensions, symbol keys, `6966636db`'s `then` probe) have no port. `Get("length")` and `Set("length", v)` on an array are kept, since they are the property they spell; `Set` converts `v` as ArraySetLength does (ToNumber: null 0, booleans 0/1, strings by StringToNumber, arrays through their string form, an object with a "toString" member a TypeError), pinned by 64 assignments captured from pi.

**D71 — retired 2026-09-25.** Upstream stopped freezing revisions (`d5cba1d97`: immutability is an ownership contract, nothing is frozen or copied), so applying a tracker's batch with the mutable applier writes into its revisions in pi as in Go; doc.go (Streams) directs in-process consumers to `ApplyImmutable` on both sides' terms.

**D72 — chord/delta.** Container identity. `===` on containers is a map's header address, or a slice's backing array plus its length; an empty slice with no capacity — every `[]` encoding/json decodes, each its own array in pi's `JSON.parse` — has none, since Go gives every one the same zero-size address. The overlay therefore finds the node of a container read from the revision by its slot (member key or entry index), not by identity, so two such empty arrays stay two drafts (TestDraftEmptyArraysAreDistinct, red with identity keys); a container a change placed is its own copy, allocated with capacity at least one, and is found by identity. `DiffRevisions` treats an identity-less array as identical to nothing, so decoded revisions align exactly as pi's do (golden "parsed empty arrays …"). Two residuals of `DiffRevisions` over values a Go caller builds by hand: one empty `[]any{}` shared by both revisions is one array in pi but identical to nothing here (give it capacity, `make([]any, 0, 1)`, as the golden's `$ref` does — "a shared empty array …"), and two headers over one backing array with one length are one array here where pi would hold two. Either can change which elements anchor an alignment, never the replica. A value with no JSON form handed to `DiffRevisions` matches nothing.

**D73 — chord/delta.** 32-bit lengths. Growing an array's length past 2^31 - 1 fails on a 32-bit build with an error naming the limit, where pi's arrays reach 2^32 - 1; indices, keys and wire addresses past 2^31 otherwise mean the same as on a 64-bit build (`Index` and `PathID` are int64). Growth inserts nulls in both, materialized in both since `d5cba1d97`.

**D74 — chord/delta.** `Draft.Sort` sorts its entries with `slices.SortStableFunc`; V8 runs TimSort. Both are stable, so a consistent comparator yields the same order and the same ops, but the comparator is called on different pairs a different number of times — measured under node, 5 descending ranks take 8 calls in V8 and 10 in Go, 5 ascending 6 and 4, 2 elements one each. A comparator with side effects agrees only as far as its calls do: a write inside an element stays, a write over an element is put back when the sort ends, and a comparator that restructures the array leaves duplicated entries as copies — each pinned by a golden comparator that does the same thing on every call (write, unshiftOnce). A comparator that prepares or aborts its own change, or adopts a competitor, is out of contract upstream (README: unspecified; pi throws an internal TypeError on the first two); Go's Sort returns `ErrDraftSettled`. The nil comparator is JavaScript's default order, each element's `String()` in UTF-16 code units, taken once before sorting, which matches V8 exactly: with two or more elements every one is compared, so an element with no string form (an object with an own "toString" member, directly or inside an array) fails the sort with pi's TypeError whichever pair V8 would have compared first, and the array is left untouched either way.

**D75 — chord/delta.** Go values in, JSON values out. Track and PrepareReplace take the root as it is, in O(1), as pi does — a nil map or slice root is null, which a change cannot draft nor a replacement be (ErrScalarRevision, pi's refusal of a null root): it must already be strict JSON in the representation encoding/json decodes (float64 numbers, []any, map[string]any, no nil map or slice — encoding/json writes one as null, which a replica cannot hold members of — and no container reachable twice), and one that is not has unspecified behavior, as in pi. `chord.CopyJSON` makes exactly that representation from any Go value; `chord.IsValue` accepts more (aliases, other numeric kinds, typed and nil containers), so it is no check for a root. Every value a draft places is copyJson's copy (D30): any Go numeric kind or json.Number becomes a float64, a typed slice, array or string-keyed map the plain one, a typed nil container an empty one, and anything else is refused with pi's copyJson text before the draft changes. The Go errors also name the offending type, and a map is walked in key order, so that of several bad members the one reported is always the same.

**D76 — chord/delta.** Not ported: `delta/revision-validator.ts`'s `JsonRevisionValidator` (`d5cba1d97`), whose one caller is `services/state.ts`'s `ReplicatedStateReplica` — the replicated-state runtime D52 leaves to its own slice (Scope queue entry 11), where it lands. `value.ts` (the `JsonRevisionStore`) and `draft.ts`'s `produce` helpers are gone upstream, and so is their Go.
...
**D77 — `ai.Model.Type`: an explicit JSON `"type": ""` reads as chat.** Go's
zero value must mean chat (every hand-built `*Model` literal is a chat model),
and a string field cannot tell `""` from absent. pi's `getModelType` is
`model.type ?? "chat"`, so an explicit `""` stays `""`: `assertChatModel` throws,
`isModelType(chat)` is false, and `hasKnownModelType` drops the model from stored
and fetched catalogs. pi-generated data never holds it (its model-data
validation rejects any type outside chat/image/classifier); only a host
`ModelsStore` or hand-written JSON can. JSON `null` is safe — both read it as chat.

**D78 — `Models.GetAllAvailable`'s fallback for a handwritten provider.** pi keeps
every model of `getAllModels()` when the provider has no `filterModels`; the Go
`Provider` interface always has `FilterModels`, so the fallback always filters
chat ids through it. A handwritten provider without `AllModelsFilterer` whose
`GetAllModels` lists a chat model its `GetModels` does not: pi keeps it, Go
drops it. `CreateProvider` providers are unaffected (`FilterAllModels`
reproduces pi's branch). Documented on `ai.AllModelsFilterer`.

**D79 — `ModelsError` with a cause whose message is empty.** pi's
`withCauseDetail` uses `formatThrownValue(cause)`, which is `message || name`, so
an `Error("")` cause appends `: Error`. A Go error has no name; nothing is
appended. The rest of pi's message rule is exact since `06b134a` (no code
prefix, cause appended only when not already contained, `jstext.Trim`).

**D80 — response-header records on the wire: pi's CLI speaks HTTP/1.1 through
undici, the port negotiates HTTP/2 and reads through net/http.** pi's
coding-agent pins HTTP/1.1 for every provider (`http-dispatcher.ts`
`allowH2:false`); the port's `sharedClient` negotiates h2 with TLS hosts, so on
real traffic `onResponse`/`sdkHttpResponse.headers` records differ in hop-by-hop
headers (`transfer-encoding: chunked`, `connection`). Over HTTP/1.1, net/http
also consumes headers before any record sees them: a `Connection` header holding
`close` in any case becomes a lone `close`, `Transfer-Encoding` comes out
lowercase `chunked`, `Trailer` names come out sorted and canonical, two equal
`Content-Length` lines are merged (undici fails `fetch failed`), and transparent
gunzip deletes `Content-Length` (restored for google, which now undoes its own
codings). `responseHeadersRecord` (anthropic, pi-messages) and
`googleSDKResponseHeaders` restore what can be restored; the rest are tagged
divergence scenarios in `ai/providers/testdata/google-stream-events` guarded by
`TestGoogleDivergencesStillDiffer`. A joined request value with an empty
contribution differs in bytes only (`Bearer k, ` on pi's HTTP/1.1 wire, `Bearer
k,` over HTTP/2). Forcing HTTP/1.1 for every adapter is the fix if this ever
matters; it is a transport change for all providers, not taken this cycle.

**D81 — google offers only `gzip, deflate`.** Since `a5a1f96` google undoes its own
content codings, as undici does, and sends undici's cleartext value
`Accept-Encoding: gzip, deflate` unless the caller names one. Over https undici
offers `br, gzip, deflate, zstd`; Go's standard library has no brotli or zstd
decoder, so the port never offers them, and a caller who asks for one gets a
hinted error. Taking them would be an E2 consult; nothing needs it.

**D82 — JavaScript values no Go field can hold (representation class).** Where
pi stores a wire value as it came and the port has a typed field: google usage
counts sent as strings (pi `"2"+1` = `"21"`), fractional or infinite counts,
a numeric `responseId`, a non-string text delta, non-string tool-call
id/name/thoughtSignature and non-object args; openai-completions tool-call
ids/names that are not strings (`String()` is stored; `[]` and `[""]` are truthy
in pi and `""` here); `parseStreamingJSON` results that are not objects (pi keeps
`12`, `"x"`, `[1]`, `null` as the arguments; the port has `{}`), non-finite
numbers (held as nil; the persisted bytes match, since JSON.stringify writes
null) and half a surrogate pair (U+FFFD); a pi-messages block event whose
`contentIndex` names no array slot (`"01"`, `1.5`) carries `ContentIndex -1`, and
an array-inherited name (`"length"`, `"__proto__"`) is an ordinary key here.
anthropic text, thinking and signature that pi holds as a number (a numeric or
boolean seed plus non-string deltas, `f19e982`) are held as their String(); after
a block's stop, `undefined + <number>` makes pi's partialJson NaN, which only
non-object arguments show.
Each measured row is a tagged scenario in the capture oracles
(`ai/providers/testdata/{google-stream-events,openai-stream,streaming-json,pi-messages,anthropic-sse}`);
the observer (`OnProviderStreamEvent`) still sees pi's value.

**D83 — port limits pi does not have.** Each fails with a hinted error: an
openai or anthropic SSE line past 16 MiB (`maxOpenAISSELine`,
`maxAnthropicSSELine`), openai stream JSON nested past
10,000 levels (`jstext.JSONSyntaxError` recurses per level), a response body
that returns no data and no error 100 reads in a row, where pi's reader would
wait forever (`progressReader` in `ai/providers/body_progress.go`,
`maxEmptyBodyReads`, on the openai, anthropic, pi-messages and google SSE
readers alike: `the <provider> response body returned no data and no error 100
reads in a row; this is a port guard against a broken body reader, report it
with the provider: multiple Read calls return no data or error`, wrapping
`io.ErrNoProgress`), and a pi-messages message holding more than 1,024 unset
content slots at once (`piMessagesMaxContentHoles`; pi's sparse array holds any
number).

**D84 — an openai error chunk whose `metadata.raw` cannot be stringified.** pi's
completions catch block throws inside itself (e.g. `{toString: 1}`) and its
stream never ends; a hang cannot be ported. The port ends the stream with the
APIError message and does not append raw (`openai_stream.go` `metadataRaw`).

**D85 — a failure later in the chunk that ended a gzip member (google).** node's
zlib works one body chunk at a time and drops a whole chunk's output on a
failure anywhere in it; Go's decoders hand output over as they go, so a complete
member's events reach the callback before a corrupt next member in the same
chunk fails the read. Both end `terminated`. Tagged in the google capture.

**D86 — undici reads one HTTP chunk at a time; Go reads everything buffered.**
A bare-JSON google error sent in the same TCP segment as an event makes pi throw
the SDK's ApiError and the port fail `Incomplete JSON segment at the end`.
Tagged in the google capture.

**D87 — node v26's TextDecoder drops a second byte-order mark at a stream's
start; the port decodes as the Encoding Standard does.** node's UTF-8 fast path
decodes the rest of a read that completes a held character with
...
each ENCODER's output is. pi encodes with Photon (Rust); the port with Go's
standard library, which produces different byte counts for the same pixels. The
header of `coding/imageresize_parity_test.go` already says the pixels are not
byte-identical, but no row said the DIMENSIONS can diverge. Measured by this
cycle's parity review against pi 0.86.1's own `resizeImageInProcess`, on a
1024x768 noise image: `maxBytes: 200000` gives **576x432 in pi and 768x576 in
Go**, and `maxBytes: 20000` gives 324x243 against 243x182 — so the dimension
note the model reads says "Multiply coordinates by 1.78" in pi and "1.33" here.
12 of 14 profile runs agreed, including the defaults and every width- or
height-only profile. **Not live at 0.86.1:** no catalog model carries
`inputLimits`, and `f5c946480`'s generator stamps the default 4.5 MiB budget,
where real images clear on the first candidate. It becomes live the day a
provider narrows `maxBytes`, which is when to measure it again. Fixing it means
matching Photon's encoder output size, which the port cannot do without
Photon — so it is debt, and pinning the port's own dimensions in a green test
would only lock in the wrong answer.
...
**K18 — google and anthropic HTTP-error messages do not match pi's; measured.**
`ai/providers/testdata/httperror/pi-ai-0.85.1.json` (captured from the published
build by `capture.mjs`) carries all four adapters' `errorMessage` for 35 bodies.
For `{"error":{"message":"blocked"}}` at 403, pi's google adapter surfaces the
body verbatim with no status — `{"error":{"message":"blocked"}}` (the
`@google/genai` error's message already carries the body, so
`formatProviderError` returns it unchanged) — and pi's anthropic adapter
surfaces the Anthropic SDK's message, `403 {"error":{"message":"blocked"}}`
(`${status} ${JSON.stringify(body)}`, the whole body, never capped: 6,028 units
on the astral row). The port's google and anthropic non-2xx branches both route
through `formatProviderError(label, status, body)` and say
`Google API error 403: blocked` / `Anthropic API error 403: blocked`, capped at
4,000. `errors.go` already holds `anthropicSDKErrorMessage`, which replicates the SDK
message (with a 4,000-unit cap pi does not apply) but is wired only into the
retry fail-fast quote — `StreamAnthropic` passes it to `retryFromOptions`, for
`validateServerRetryDelay` in `retry.go` — not the terminal non-2xx path;
google passes nil, and no google counterpart exists. The slice should decide the retry-path cap from the same oracle (extend
`capture.mjs` with a retry-after case) rather than leave it implicit. The
completions path is D20, decided; the Responses path follows pi since
`b32a55b`. **Fix as one slice:** per-adapter formatters at the two call
sites against the oracle's `google-generative-ai` and `anthropic-messages`
columns (the capture already records them), and rewrite the tests that pin the
Go shape. Not folded into the 2026-09-14 cycle under the boy-scout boundary: two
other adapters, each with its own test surface. **One transport-level gap is shared by every adapter, measured
2026-09-14 against the same build over 72 probe bodies:** pi's `fetch` decodes
the body with the WHATWG UTF-8 decoder before any client parses it, so invalid
UTF-8 becomes U+FFFD per maximal subpart; the port passes the raw bytes through
(and `encoding/json` replaces per byte on the JSON path). 9 of the 72 differed,
all of them bodies carrying raw invalid UTF-8 (latin-1 gateway HTML, truncated
multi-byte sequences). **Partly closed 2026-09-24:** the decoder now exists as
`jstext.DecodeUTF8` (checked against node's TextDecoder) and is wired into the
anthropic and pi-messages SSE streams, pi-messages' non-2xx body (pi's
`response.text()`) and both openai stream readers. The error-body path in
`errors.go` that the HTTP-error formatters share still strips one BOM and passes
the raw bytes; wire the decoder in there with this row's formatter slice, and
teach `capture.mjs` to carry byte bodies (base64) so the oracle can hold the
rows.

**K22 — request headers undici's fetch adds by default.** Measured after
`c231659`: pi's wire carries `accept: */*` when no Accept is present (always on
google, and on the SDK adapters after an accept deletion marker), plus
`accept-language: *`, `sec-fetch-mode: cors` and an `accept-encoding`. The port
sends none of them (google now sends `Accept-Encoding`, D81). Sibling of D12;
fixable by adding them where pi's fetch would.

**K23 — `strings.ToLower` where pi uses `toLowerCase`.** The anthropic tool-name
mapping (`ccToolLookup` and the Claude Code name table) and google's model-id
lowering still use `strings.ToLower`; header-name folds and the two auth gates
moved to `jstext.ToLower` (`b2e6612`). Differs only on the few code points where
Go's simple case mapping and JS's full mapping disagree.

**K24 — `Usage.CacheWrite1h` is `omitempty`.** pi writes `"cacheWrite1h": 0` on
every anthropic message; the port drops the key, and
`anthropic_response_model_test.go` normalizes it away
(`withoutZeroCacheWrite1h`). Same class as K12.

**K25 — anthropic and pi-messages parse failures use encoding/json's wording.**
pi's cause is V8's `JSON.parse` SyntaxError text. `jstext.JSONSyntaxError`
reproduces it exactly (2,244 node rows) and the openai and google loops use it
since this cycle; wire it into `iterateAnthropicSSE`'s parse failure and
pi-messages' unparseable frame, whose capture tests currently check pi's text
only around the cause.

**K26 — request URLs are parsed with net/url; pi's fetch uses the WHATWG URL
parser.** The SDK adapters join the base URL and endpoint path as their SDKs'
`buildURL`/`constructUrl` do, dropping one slash, so a base ending in two sends
an empty path segment (`sdkJoinURL`); pi-messages trims every trailing slash.
Each adapter sends what `requestURL` (`ai/providers/fetch_url.go`) makes of that
string, where pi builds it (after onPayload for the SDK adapters, before it for
pi-messages). `requestURL` applies the WHATWG parser's input handling before
net/url parses: leading and trailing C0 controls and spaces are stripped, every
tab, LF and CR is removed, and other C0 controls and DEL are percent-encoded;
for a special scheme, a run of slashes and backslashes after `scheme:` reads as
`//` and a backslash before the query or fragment as `/`. A special URL is then
held as that parser holds it: ASCII host lowercased, default port and leading
zeros dropped, an empty path as `/`, an empty userinfo dropped. It refuses with
pi's `Invalid URL` a URL net/url cannot parse, one with no scheme, a special one
with no host, or a port past 65535. A URL with credentials is refused at the
send, as undici's fetch refuses it, for the port's own client only: google and
pi-messages fail with `Request cannot be constructed from a URL that includes
credentials: <href>`, anthropic and both openai loops with their SDKs'
`Connection error.`, or `Request timed out.` when the URL passes the SDKs'
`/timed? ?out/i` test of the refusal's text (`sdkFetchError`); a custom client
is handed the URL. Measured in `ai/providers/testdata/base-url` (65 rows, 39
hrefs, 4 custom-fetch rows, 230 requests). Where the parsers still differ, rows
are tagged and guarded by `TestInvalidBaseURLDivergencesStillDiffer`,
`TestRequestURLDivergencesStillDiffer` and
`TestBaseURLRequestDivergencesStillDiffer`. Only net/url refuses a malformed
percent escape (`%zz`, a lone `%`) or a space in the userinfo; pi sends those.
Only the WHATWG parser refuses an IPv4 part past 255 (`256.0.0.1`). The WHATWG
parser also resolves dot segments (`/a/../v1`, `%2e%2e`), reads `127.1` as
`127.0.0.1`, leaves `|` in a path unescaped (the port sends `%7C`), and
percent-decodes a host (`ex%61mple.com`, which net/url refuses). The SDKs
re-serialize a query in the base URL (pi-messages does not), and @google/genai
puts `alt=sse` before a fragment in the base URL, where the port's lands inside
it. Not in the capture, measured ad hoc: pi runs a non-ASCII host through UTS
46, while the port leaves it to net/http, which punycodes the Host header
without that mapping. So `ëxample.com` goes out as pi sends it
(`xn--xample-ova.com`), but `BÜCHER.example` does not (pi
`xn--bcher-kva.example`, the port `xn--bcher-2pa.example`). Path to close: a
WHATWG host parser (percent-decoding, UTS 46 via golang.org/x/net/idna, and IPv4
number parsing) and path serializer (dot segments, the path percent-encode set).
x/net is a new root requirement, so `internal/policy/deps_test.go` must list it
with its reason.

**K27 — an empty anthropic thinking signature is omitted.** pi's anthropic
thinking block always carries `thinkingSignature` (`?? ""`), so pi writes
`"thinkingSignature":""`; the port's field is `omitempty`. Same class as K12
and K24.
...
**C7 — a golden whose upstream source is gone is frozen, and its test cannot
pass empty.** When upstream deletes the source of a golden the port still uses,
the golden keeps its section, frozen at the last sha that had the source (so it
can never be regenerated by mistake), its generator says so, and the test
`t.Fatal`s on an empty section. A table-driven test whose table can become
empty is not a test: `TestAssertCompleteFrameMatchesUpstream` passed for a cycle
iterating nothing after the `assertCases` section was dropped (frozen at
`96317e50b` since).
