...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`f5c946480`** — "feat(ai,coding-agent): add image input limits (closes #9631)" (advanced 2026-09-21). **46 changes in `590144609..origin/main`, and first-parent lies again.** The durable/pico branch is main's first-parent line for the first few commits, so `--first-parent` gives 41 and hides five REAL mainline commits that arrive as merge SECOND parents — `8bdcd4498` (the compaction fix this cycle ported), `1e39862f6`, `c1263aa11`, `59eb4c393` and `e4ce7b449` (a generator delta). Triage ran on the 46-commit union. **This is the second consecutive cycle where the first-parent log was the wrong range; stop treating it as the default.** **Verdicts: 10 port / 8 port-but-QUEUED / 1 port-but-CATALOG-ONLY / 27 n/a / 0 decide.** **Shipped (7 source commits + the regen):** `8bdcd4498` → `coding/compaction.go` (`5a7ecad`); `c596d09d9`'s ai half → `ai/types.go` (`120e5d9`); `dfbf793b7` → `coding/session_store.go` (`1c17887`); `dd01f5b24` → `LatestSession` (`9a8f43b`); `b73412a37` → `ai/envkeys.go` + `coding/resolve.go` (`7cb43ec`); `af7359b90` → `ai/providers/openai_compat.go` (`2cc2eb6`); `f5c946480` → `ai/types.go` + `ai/providers/faux.go` + `coding/{imageresize,tools,session}.go` (`c87df16`); and the two release commits → the catalog regen (`2e8f05e`). **The one to remember is `dd01f5b24`.** It reads as a perf tweak — make a header scan lazy — but reaching it meant noticing that the port had COLLAPSED two upstream functions with different sort keys into one. pi has `list()` (ordered by `modified`) and `findMostRecentSession` (ordered by file MTIME); the port answered `--continue` out of `ListSessions`, which orders by the HEADER timestamp — a value written once at creation. A session created five minutes ago therefore outranked one created last week and written to all morning, so `--continue` could reopen the wrong session outright. The subject line would never have said so. **`f5c946480` is the cycle's real work** and it is only half-armed: `Model.InputLimits` lands with all three of pi's resize call sites threaded, but its generator half is NOT in 0.86.1 (it landed after the tag), so **no model carries `inputLimits` yet** and the threading is behaviour-neutral until the next regen — at which point every vision model gets the port's own historical defaults and the provider-specific limits start to bite. **port-but-QUEUED (8):** `0e283203c` → entry 16; `4d38031fb`'s provider half → entry 2; `b73412a37`'s OAuth half → entry 1; `de2de549b` and `3c75b2747` → entry 12; `b7f788194` → entry 11; `080160162`/`a16ccd9be`/`0db565924` → entry 8, **with a path correction the detector caught**: Pico moved out of `packages/agent/src/pico` into a NEW published top-level package, `packages/durable` (`@earendil-works/pi-durable`). Two NEW rows opened: **17** (prompt cache warming — `core/cache-warmer.ts`, 437 LOC, IN scope because its consumers are mixed so E1's only-consumers clause does not fire) and **18** (a model on the tool execution context, the seam `f5c946480` wanted and the port worked around). **n/a (27):** the tui and modes commits (E1), the CHANGELOG audits, `40c256ccc` (`core/extensions/**`, E1), `6dff740fa` (clipboard/OSC 52, E1), `1e39862f6` (`src/extensions/llama/**` — E1 only-consumers: its sole non-test consumer at HEAD is `modes/interactive/interactive-mode.ts`), `60740991c` (repo-root packaging), the two merge commits, and `58541ee72` + `c1263aa11` + `36b60d2e8` in `chord/delta`. **`58541ee72` deserves its own line** — 897 changed lines in a file the port HAS. It is a `WeakRef`/`FinalizationRegistry` retention rewrite of `track()`: the exported surface is unchanged, no existing op-batch golden moved, and its new tests are a GC worker run under `--expose-gc`. n/a under the 2026-09-03 "no meaning in Go" ruling, and the collections were diffed rather than the rationale (memory: a perf-only commit can hide a port). **Reconciliation at the advance:** detector **254 paths** — 100 A / 2 D / 146 M / 6 R — and it is the pass that earned its keep this cycle: `packages/durable` is a brand-new top-level package that no pathspec written before 2026-09-20 would have printed. Non-`packages/` paths are `.gitattributes`, `README.md`, root `package.json`/`tsconfig.json`, `package-lock.json` and three `scripts/*.mjs` — all packaging. Accounting (excluding tui/evals/docs/examples) 215 files, +47,437 −1,679, the bulk being restored `packages/agent/docs` prose. None of the four split-by-HUNK tripwires escaped notice: `core/model-resolver.ts` WAS touched (`b73412a37`, one line into `defaultModelPerProvider`) and was ported. No new project-local read. **Upstream `ci.yml`, read per sha via the workflow-scoped `actions/workflows/ci.yml/runs?head_sha=` endpoint:** green at every sha this cycle ported except **`4d38031fb`, which FAILS its own build** — `build-check-test`, because its new generator test reaches the live Radius gateway; fixed one commit later by `7d5eb0ee3`, a one-line mock. Content is fine, the sha is red. `8bdcd4498` is `cancelled` (superseded push) and the release commits have no run. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.86.1** — **two releases crossed** (`ecac0a9c4` v0.86.0, `13cbf77df` v0.86.1), so the catalog was regenerated and the reference build refreshed. **Read this before trusting an empty tag check:** `git tag --contains <pin>` returned NOTHING until an explicit `git fetch origin --tags` — the sync routine fetches `origin main`, which does not bring tags. The corroborating check that cannot go stale is the version in the tree itself (`git show <sha>:packages/ai/package.json`), and it is what confirmed both tags. Catalog: 39 → **41 providers** (+`radius`, +`meta`), 1,354 → **1,443 models** (+145, −56, 767 changed), 592,846 → 667,778 B. **The catalog-only queue drained 10 → 1**: every one of the ten rows was an ancestor of `13cbf77df`, checked per row with `git merge-base --is-ancestor`, and every one landed as its row predicted — measured, not assumed (the numbers are in `2e8f05e`). The single remaining row is `f5c946480`'s `applyImageInputMetadata`, which post-dates the tag. **The method was verified before the output was trusted:** dumping `JSON.stringify(MODELS)` from the 0.85.1 build reproduced the committed catalog byte for byte, which is what licensed the new file. **Both carried re-captures came due with this build and both are done.** `0c7bb7c5c`'s Responses provider label now ships, so `ai/providers/testdata/httperror` was re-captured at 0.86.1: the `openai-responses/xai` column was the ONLY one that moved, and it is now the oracle for all 35 rows, replacing the three hand-written strings `TestResponsesHTTPErrorNamesProvider` used to assert. `4a6ed0194`'s MAJOR `@google/genai` bump (1.52.0 → 2.21.0) is in this build, and the google difftest scenarios now run against it. The anthropic response-model oracle was also re-captured — at `f5c946480`, whose `api/anthropic-messages.ts`, `api/transform-messages.ts` and `compat.ts` are identical to the v0.86.1 tag's — which discharges the src-only caveat it had carried since `1283afd0d`: **every behavioural entry came back byte for byte**, and only the catalog model moved. **Carry to the next regen:** nothing carried forward. |
| Parity proofs at the pin | **2026-09-21.** Every behaviour change was observed RED for a **behavioural** reason — by mutating the shipped code and watching a test assert a wrong value, never a compile error. Where a first-draft test compiled red on a missing field, the field was added with its use IGNORED to get a real red before being honoured. `5a7ecad`: the fallback cut came back at index 0 instead of 1. `120e5d9`: the round trip re-encoded a model without `promptCache`. `1c17887`: tied sessions came back `[a b c d e]` against pi's `[e d c b a]` — and the first version of that test could not fail, see below. `9a8f43b`: `fresh-id` (newer header) won over `resumed-id` (newer file); the parity gate then ran **13 fixtures against pi 0.86.1's real `findMostRecentSession` and got 13 identical answers**, including a broken symlink aborting the scan on both sides. `7cb43ec`: no env var, no default. `2cc2eb6`: `strict:true` reached both a catalog-shaped and a custom cerebras model. `c87df16`: **five mutants, five reds**, two of which SURVIVED the first pass — the tool-result and read-tool paths had no test until the mutants said so. **Oracles, all re-captured from pi, never hand-edited:** `ai/providers/testdata/httperror/pi-ai-0.86.1.json` (35 rows; only the `openai-responses/xai` column moved), `ai/providers/testdata/response-model/anthropic-response-model-f5c946480.json` (every behavioural entry byte-identical to the old capture; only the catalog model moved), and the review-fix oracles below, each captured by running node or the 0.86.1 build. **The catalog was independently re-derived by the parity gate** and `cmp`s equal, with the outgoing file equal to the 0.85.1 build, so both ends are pinned. **difftest re-pinned** (`PI_NPM_VERSION` 0.86.1, `PI_UPSTREAM_SHA` `f5c946480`) at **58 PASS / 0 KNOWN / 0 FAIL**, exit 0 — after a harness fix without which the new build turned 46 scenarios red (see the regen tripwires). Full gate on the shipped tree: `gofmt -l` clean, `go build`/`go vet`, `go test -race ./... -count=1` green, and `GOOS=windows` build + vet green. |
| Reviewed via | 2026-09-21 — per-commit triage from real diffs, on the UNION range. **Three independent read-only review gates, each in its own worktree: pi-go-review, pi-parity-review, and the JS-semantics lens.** All three STALLED at once, midway, on a model-stream outage (the auto-mode classifier timed out in the same window) and were RESUMED with their context rather than restarted — each had real progress in flight, and a restart would have thrown it away. All three worktrees also started at the range BASE rather than the tip and had to be reset; the briefs said to check, and they did. Findings were reproduced in the live tree before anything changed — a reproduced failure has nothing left for a refuter to argue — and every one reproduced. **9 unique findings, 3 filed by two gates**, all acted on: **`16fa8b9`** Node's `Buffer.from` STOPS at the first `=` and the port's decoder filtered it out, so separately padded chunks decoded to all of them or to bytes from neither (12 of 19 node-captured rows wrong). **`974c490`** `toFixed2` rounded exact ties to EVEN where JS rounds them UP, so a 2250px image's note said "Multiply coordinates by 1.12" where pi says 1.13 — pre-existing, and newly written into the user's own prompt text by `c87df16`. **`d17c742`** `findMostRecentSession` gates its cwd filter on truthiness, so an empty cwd means NO filter, which `list()` does not do; and the `localeCompare` comment claimed an agreement that fails under Danish and Norwegian collation. **`4280205`**, the one worth reading: **`c87df16` introduced a data race** — the resize getter read the plain `Session.Model` field from the tool goroutine while `SetModel` writes it unguarded — and had been inserted under `bashSessionEnv`'s doc comment, so godoc showed the racy function claiming it read "through a synchronized path". The getter advertised mid-session `SetModel` as its reason to exist and then broke the contract its neighbour already honoured. Same commit: a tie test that could not fail (five equal keys, which `sort.Slice` never reorders), rebuilt to 48 files so both mutants go red, and `decodeNodeBase64` made infallible in its signature. **K20** records the one finding not fixed — resize dimensions diverge under a narrowed `maxBytes` because Photon's encoders and Go's differ in size — on reachability and fixability grounds stated in the row. **Per-commit greenness: every one of the cycle's 12 commits builds, vets and tests green on its own.** |
...
the matching npm build. **Currently 1 — drained from 10 on 2026-09-20 by the
0.86.1 regen.** All ten rows were ancestors of the `v0.86.1` release commit
(`git merge-base --is-ancestor <delta> 13cbf77df`, checked per row, never by
log order), and each landed as its row predicted; the measurements are in
`2e8f05e`'s message. This cycle's own five generator deltas (`e4ce7b449`,
`4d38031fb`, `c596d09d9`, `b73412a37`, `af7359b90`) were ancestors too and
drained with them.
...
|---|---|
| `f5c946480` (2026-09-20) | `applyImageInputMetadata`: every model whose `input` includes `image` gains `inputLimits`. Two halves. (a) A **resize profile** — `{maxWidth: 2000, maxHeight: 2000, maxBytes: 4.5 MiB, jpegQuality: 80}` — stamped on ALL of them, which is byte-for-byte the port's own historical defaults, so the regen changes no behaviour by itself. (b) **Provider limits**, which do differ: anthropic `maxRequestBytes` 32 MiB and `maxPerRequest` 100 (200k-context models) or 600, amazon-bedrock `maxPerMessage` 20, openai `maxRequestBytes` 512 MiB + `maxPerRequest` 1500, google `maxRequestBytes` 20 MiB + `maxPerRequest` 3600. It landed AFTER the `v0.86.1` tag, so **0 models carry `inputLimits` today** — measured, not assumed. The src half shipped this cycle as `c87df16`, so the Go side is ready: `ai.Model.InputLimits` decodes it and the read tool, the tool-result normalizer and `Session.Run` all thread `images.resize` into the pipeline. `maxPerMessage`, `maxPerRequest` and `maxRequestBytes` have no consumer in pi either — they are provider description. **Nothing goes red when this lands**, which is the thing to watch: check the resize profile per provider after the regen rather than waiting for a test. |
