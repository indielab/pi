...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`b313731b8`** — "feat: add JSONL sidecar reclamation" (advanced 2026-09-23). **21 changes in `95fbc0499..b313731b8` on the union range, 19 first-parent** — the first-parent line now runs along upstream's `durable` branch: `25cc5c7bf` and `d192bd6dc` are reachable only through `2c2cd636f`'s second parent, and the old pin only through `d201760ff`'s. Neither merge carries anything beyond its parents outside `packages/durable`. **Release crossed: `v0.87.1` (`f07218c4d`)** — explicit tag fetch, corroborated by `packages/ai/package.json`. **Verdicts: 11 port / 10 n/a / 0 decide.** Shipped: `d192bd6dc` split-turn summary prompt (`1566679`), `3a624b82d` claude-cli 2.1.280 (`3749ec5`), `9a139c62b`'s `chord/delta` half (`0dcd440` plus its review fixes), and the 0.87.1 regen (`e953886`) draining `b4588f26a`, `db91e0331`, `27c072e98`, `b7f4b05c7` and the carried `1a584a7a5` with its xai rider. Boy scout: `72fd91135` (neutral summarization system prompt, unported since v0.79.0 — `eb8d05d`). **Queued:** `8158b0321`/`5901c9b9e`/`898ab8040`/`b313731b8` (`packages/durable`) and `9a139c62b`'s `harness/pico3` half → entry 8; `9a139c62b`'s chord services half → entry 11; `b7f4b05c7`'s image-catalog hunk → entry 3. **n/a (10):** `25cc5c7bf` (docs, examples, `packages/evals`), `81274f0e1` (repo-root scripts), `3a4c777f2`/`a8ed49771` (changelog), `f07218c4d` (version bump), `a32782520`/`967246214` (durable design docs), the merges `d201760ff`/`2c2cd636f`, and `4bc1a2fe5` — chord's `delta/{astra,cow}` prototypes: not exported (`packages/chord/package.json` maps only `.`, `./context`, `./delta`, `./bundler`, `./node`, and `delta/index.ts` re-exports neither), imported by nothing outside `test/`, and their READMEs weigh JS-runtime trade-offs (proxy traps, deep freezing) with no Go subject — not mirrored under the 2026-09-03 ruling's irrelevant-in-Go clause. Tripwire: if upstream wires either into `delta/index.ts`, it is a `port`. **Upstream `ci.yml` per sha** (workflow-scoped endpoint): `d192bd6dc`, `b4588f26a`, `a8ed49771`, `898ab8040` green; `3a624b82d` cancelled; `db91e0331` and `27c072e98` FAIL their own builds on Opus 5.5 effort levels (models.dev drift, fixed by `b7f4b05c7`, which the 0.87.1 build contains); `b313731b8` FAILS `github-copilot-anthropic.test.ts` because CI regenerates from live models.dev, which now lists `github-copilot/claude-opus-5.5` and displaces the generator's hard-coded entry — the published 0.87.1 build is unaffected and the port matches it on all 1,495 models; `b7f4b05c7`, `f07218c4d`, `9a139c62b`, `4bc1a2fe5` have no run. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.87.1** (and `pi-coding-agent` 0.87.1). Catalog regenerated by `JSON.stringify(MODELS)` after first reproducing the outgoing 0.87.0 catalog byte for byte (791,953 B): 41 providers (unchanged), 1,445 → **1,495 models** (+62, −12, 35 changed), 791,953 → 823,821 B. **The catalog-only queue drained 1 → 0**; `defaultModelPerProvider.xai` is now `grok-4.7`. Tripwires: BILLING clean (2/2 fallbacks carry `provider`); one thinking map moved (`openrouter/~anthropic/claude-opus-latest` off `"none"` → null — the alias now points at Opus 5.5); no api flips; no new catalog key or compat field. Re-captured at `b313731b8`/0.87.1 and byte-identical, so names unchanged: `completions-native-95fbc0499.json`, `tools-only-95fbc0499.json`, `anthropic-response-model-95fbc0499.json`. **New pi-captured oracles:** `coding/testdata/compaction/{requests,resume,trigger}-0.87.1.json`, `coding/testdata/defaultmodels/default-models-0.87.1.json` (the whole `defaultModelPerProvider` table — re-capture at every re-pin, `capture.mjs --src` between releases), `ai/testdata/cost/calculate-cost-0.87.1.json`, `ai/providers/testdata/service-tier/service-tier-0.87.1.json`, and `chord/delta/testdata/upstream_delta.json` (src at `9a139c62b`). |
| Parity proofs at the pin | **2026-09-23.** Every behaviour change was observed RED for a behavioural reason, by mutating the shipped code; each commit message carries its table. Compaction: the pi-captured request oracle failed on exactly the four turn-prefix requests against the old prompt, and on every request against the old system prompt. `chord/delta`: 66 porter mutations and about 90 reviewer mutations, each red against pi's bytes; two independent differential generators (125k steps against pi src) found 0 mismatches. Catalog: six targeted catalog mutations each fail their mirrored upstream assertion, and 1,495/1,495 models' supported thinking levels match pi 0.87.1's `getSupportedThinkingLevels`. **difftest re-pinned** (`PI_NPM_VERSION` 0.87.1, `PI_UPSTREAM_SHA` b313731b8) at **60 PASS / 0 KNOWN / 0 FAIL, all 60 now `dist`-backed** — `empty-text-part` (shipped in 0.87.1) and the nine transcript-model scenarios (owed since 0.86.1) left `src`; `baseten-catalog-kimi` is refreshed and guarded by a new `catalogModel` stale-copy check. Full gate on the shipped tree: `gofmt -l` clean, `go build`/`go vet`, `go test -race ./... -count=1` green, `GOOS=windows` build + vet, and `GOOS=linux GOARCH=386` build (new this cycle — `chord/delta` did not build on 32-bit). |
| Reviewed via | 2026-09-23 — per-commit triage on the union range; three port slices in parallel worktrees (compaction, ai + regen, chord), then **four review rounds, loop-until-dry**. Every finding went to an adversarial refuter that had to reproduce it by execution, and only survivors were fixed: round 1 (the ports) 23 filed / 23 reproduced; round 2 (the fixes, plus a completeness critic) 20 / 20; round 3, 13 / 13; round 4, 6 / 4 — three shipped, and the fourth, live compaction trigger timing, is entry 12's, recorded there with its measurements. Beyond the upstream diffs: **`Session.Reset` (`/new`) and `LoadHistory` kept the old compaction checkpoint**, so the next request sent the previous conversation's summary and dropped the new prompt (`22e9adf`); **resuming a pi session file ignored its compaction** — the next one re-summarized the old summary as a user turn with the INITIAL prompt — now `Session.LoadBranch` resumes from the branch projection (`edc0727`, `11ebece`) and usage recorded before a compaction or `context_edit` no longer triggers the next one (`9ad9374`); an empty `stop` summary is kept and an aborted one discarded (`224fd46`); compaction file lists sort by UTF-16 and keep every `details` value as pi's Sets do (`8ca439e`, `29c52af`, `959b2fe`); `ai.CalculateCost` and Responses service-tier pricing stopped fusing multiply-adds on arm64, which had moved the last bit of about 1 in 6 costs off pi's (`30361de`, `8eaf641`); the whole `defaultModelPerProvider` table is locked against pi (`9f8adfe`); `chord/delta` accepts scalar roots, refuses non-finite numbers of every Go kind, converts `String()`/`Number()` as JS does, and builds on 32-bit with `Index`/`PathID` widened to `int64`. |
...
the matching npm build. **Currently 0 — drained 1 → 0 on 2026-09-23 by the
0.87.1 regen (`e953886`).** Every drained row (`b4588f26a`, `db91e0331`,
...
follow the build and move the test with it.
