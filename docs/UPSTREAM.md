...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`9b791a4cc`** — "fix(coding-agent): avoid transcript scans for exact session IDs (#9601)" (advanced 2026-09-16). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all **7** first-parent changes in `f9bcd351d..9b791a4cc` — 7 commits, 7 first-parent, no merge in range. Verdicts: **1 port-shipped / 1 port-but-QUEUED / 0 port-but-CATALOG-ONLY / 5 n/a / 0 decide.** Go commits this cycle: `57a3304` (the port), `13923ed` (a boy-scout parity fix in the same area), `a5c3873` (a test the mutation pass demanded), and `12c0622` + `a005e68` (the review gates' surviving findings: a pre-existing `readSessionInfo` defect the port made observable, and JSON.parse number parity); none landed out of cycle (`git log a9e586e..HEAD` was empty when the cycle opened). The unported balance is carried by **the Scope queue rows holding queued deltas (3, 6, 7, 8, 9, 10, 12 and 14) and the rows opened as the record of a review-complete cycle's not-shipped balance (11, 13 and 15)** plus the catalog-only queue — not by the pin; rows 1, 2, 4 and 5 are `ccfe79ed2`-sized base surfaces with no post-pin change and are not drift. **One queue row moved this cycle: entry 9 (1 → 2 deltas).** Entry 12's counter stays at 8: this cycle's `session-manager.ts` hunk SHIPPED instead of queueing, and the row's status cell now says which half of that file is ported. **Read the queued-deltas column; never inherit the previous cycle's list.** The next cycle's drift is `9b791a4cc..origin/main` PLUS those rows. **port-shipped (1):** `9b791a4cc` → `coding.FindSessionByID` (`57a3304`). **The classification worth keeping:** the draft first filed it `port-but-QUEUED` against entry 12 on the premise that the port has no `SessionManager`; the adversarial pass refuted the premise — `coding/session_store.go` already homes `SessionManager`'s static discovery surface (`DefaultSessionDir`, `ListSessions` = `list()`, `LatestSession`), so `findById`, a sibling static of `list()`, is a hunk beside ported code, and parking it would have repeated the 2026-09-09 lesson recorded in row 8 ("a commit that touches a queued tree can still touch shipped Go surface inside it"). Its only in-repo caller is `main.ts` (host); that does not fire E1's only-consumers clause for a public static on a root-exported SDK class whose sibling `list()` is already ported — the same reading under which `46bde88a1` moved from a drafted `n/a` to an entry-12 delta on 2026-09-10 (a queued tree is never `n/a`), with the row-8 lesson deciding shipped-vs-queued. Porting the `list()`/`findById` shared prologue (`sessionListingDir`, `sessionCwdMatches`) exposed that `ListSessions` had never taken pi's `sessionDir` — fixed in `13923ed` under the boy-scout rule rather than recorded. **port-but-QUEUED (1):** `8a7b0c03d` → entry 9 — Bedrock's `handleMetadata` now fills `usage.cacheWrite1h` from the `cacheDetails[]` entries whose `ttl === CacheTTL.ONE_HOUR`; the 2×-input pricing that field feeds is already `ai/models.go` `CalculateCost`, so only the adapter-local extraction is queued (2026-07-21 generic-collapse check: it does not collapse — upstream's `models.ts` is untouched in range). **n/a (5):** `d7296c063` (44 files: the root `package-lock.json` plus 43 under `packages/evals/**` — E1, diffstat-dispatched; the subject says coding-agent, the diff does not), `4c2d91339` (14 `export type` re-exports of `core/extensions/**` event types from the root index, two of them — `MessageEndEventResult`, `ThinkingLevelSelectEvent` — also added to `core/extensions/index.ts`'s own barrel; E1 — root export is reachability evidence, not a test), `3349e1db1` and `60e7e76bd` (`utils/clipboard.ts`, `modes/interactive/tui-renderer.ts`, `packages/tui` — E1 by consumer: at `9b791a4cc` the file's importers are `modes/interactive/{interactive-mode,tui-renderer}.ts` and the root index, every consumer host, and `grep -rni clipboard --include='*.go'` is empty), and `b03a367a4` (`core/model-config.ts` alone: `allowedFallbackModels`, ≤3 × `{provider, model, cost}`, joins the models.json `AnthropicMessagesCompatSchema` — E1 as operator configuration, the models.json validation schema being the same class as `core/settings-manager.ts`, consistent with the 2026-07-17 model-runtime-facade ruling as re-scoped by its 2026-08-27 supersession note; grounded there rather than on the only-consumers clause, because `model-runtime.ts`, one of the file's two importers, is itself imported by `core/agent-session.ts` (entry 12) and `core/model-resolver.ts` at the pin. The port has no models.json loader, and the SDK half already SHIPPED in `ai/providers/anthropic.go:111–234,1377`, which reads `compat.allowedFallbackModels` and builds the request's `fallbacks`). **Reconciliation at the advance:** detector 62 paths (the seven per-commit stats sum to 67 with 5 overlaps), all classified; `--name-status -M` over the range is 24 A / 16 D / 21 M / 1 R — 23 of the A, all 16 D and the one R (`packages/evals/src/smoke.eval.ts → packages/evals/evals/smoke.eval.ts`) sit inside `packages/evals`, and the 24th A is `packages/ai/test/bedrock-cache-write-1h-cost.test.ts`, entry 9's oracle — so no new top-level package, nothing moved out of `packages/`, no new repo-root directory. Accounting 14 files (+313 −29), accounted for. None of the four split-by-HUNK tripwire files was in range (`model-config.ts` is not `model-resolver.ts`), so the standing `defaultModelPerProvider` rule did not fire. No new project-local read: `findById` scans the session directory under the agent dir, not `<cwd>/.pi/**`. Upstream `ci.yml` is green (push-to-main run) at all seven shas: `8a7b0c03d` (34968099493), `d7296c063` (34997917478), `4c2d91339` (35027620481), `3349e1db1` (35029858536), `60e7e76bd` (35035943939), `b03a367a4` (35076028293) and `9b791a4cc` (35078372167) — read via `actions/workflows/ci.yml/runs?head_sha=`, never the unscoped `actions/runs`, whose first page is bot workflows and which paged CI out for three of the seven in the first draft. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`git tag --contains f9bcd351d` is empty, and `packages/ai/package.json` at `9b791a4cc` still reads `0.85.1`; the one `packages/ai/CHANGELOG.md` line in range, `8a7b0c03d`'s Bedrock 1h pricing, lands under `[Unreleased]`), so there was **no catalog regen and no reference-build refresh**, and no release is cut this cycle. No `packages/ai/scripts` or `*.generated.ts` path is in range, so the catalog-only queue is **unchanged at 6** (`7d8ab31a4`, `6b94ae2ec`, `bbb61e34a`'s generator hunk, `12f59336a`, `2e6fe2f98`, `d92eb8d4b`) — see the catalog regen checklist. **For the next regen, carried from 2026-09-14:** `0c7bb7c5c`'s Responses provider label is still unreleased — the 0.85.1 build labels every Responses provider `OpenAI` — so the first build that ships it is the one to re-capture `ai/providers/testdata/httperror` against: `node capture.mjs <npm dir> pi-ai-<ver>.json`, point `loadHTTPErrorOracle` (`openai_responses_test.go`) at the new file, and then rewrite `TestResponsesHTTPErrorNamesProvider` from the new file's `openai-responses/xai` column, which today records the pre-release `OpenAI` label and after that capture records `xai`. **Carry to the next release, unchanged from 2026-09-09:** `4a6ed0194`'s MAJOR `@google/genai` bump (1.52.0 → 2.21.0) is invisible to difftest, whose google scenarios are `dist`-backed at 0.85.1, i.e. pre-bump. Re-capture the google scenarios against the first npm build that ships it. |
| Parity proofs at the pin | **2026-09-16:** `57a3304` (`coding.FindSessionByID` + the bounded header reader) and `13923ed` (`ListSessions`/`LatestSession` take pi's `sessionDir`) ship with `coding/session_find_test.go`, mirroring pi's `test/session-id-readonly.test.ts` cases plus the scan bound. **Red observed by mutation, each in a detached worktree, never in the live tree:** nine mutants, nine behavioural reds — a non-header first entry no longer deciding (`TestFindSessionByIDReadsOnlyTheFirstEntry`: "a file whose first entry is not a header is not a session"), the EOF probe removed (`TestFindSessionByIDBoundsTheHeaderScan`: "a header line still open at the scan limit must not be found"), the cwd filter disabled in `FindSessionByID` and in `ListSessions` (the two `FiltersByCwdInACustomDirectory` tests), the explicit default directory filtered, a malformed line deciding not-a-session, JSON falsy literals no longer skipped, the probe rejecting a header ending exactly at the limit, and a header without cwd treated as a match. **The first pass had two invalid mutants (compile errors — not reds) and one survivor:** removing the probe left every test green, because the over-limit fixtures were rejected only by truncation making the header unparseable; `a5c3873` adds the open-line case that kills it. **Review gates (pi-sync §3) — three independent read-only reviewers, each mutation-testing in its own detached worktree, then 3 refuters per finding:** pi-go-review `ship`; pi-parity-review and a JS-semantics lens `diverges` — 9 filed, 4 survived (all LOW, every one 3–0), 5 refuted. The survivors landed as `12c0622` (`readSessionInfo` took the LAST session-typed line where pi's `buildSessionInfo` decides by the FIRST parsed entry — pre-existing, but `FindSessionByID` made the disagreement observable and `13923ed`'s cwd filter had been reading the non-pi field) and `a005e68` (`json.Unmarshal` rejects `1e309`, which `JSON.parse` accepts as Infinity — found by running pi's helpers verbatim under node against 32 fixtures, 29 identical; plus two guards, the empty header cwd and the `.jsonl` suffix, that had survived mutation). Seven more mutants against those two commits, seven behavioural reds — sixteen in all across the five commits. The 5 refuted: `~`/`file://` expansion of `sessionDir` (pi's `normalizePath` handles user-typed input; in Go that is the shell's job — 2–1), pi's default-directory mkdir side effect on a read path (2–1), an uncommented `filepath.Abs` error discard (the file's own precedent), fixture padding arithmetic style, and a duplicate of the `normalizePath` point. Full gate re-run at the advance on the shipped tree: `gofmt -l` clean, `go build`/`vet`, `go test -race ./... -count=1` (every tested package `ok` — 14 of 17, the other three have no test files), and `GOOS=windows` build+vet, all green. **difftest re-pinned to `9b791a4cc` and re-run: **51 PASS / 0 KNOWN / 0 FAIL** (exit 0) against 0.85.1 with an empty known-divergence baseline, so no entry reports FIXED and none retires.** The re-pin is bookkeeping only — all 51 scenarios are `dist`-backed at 0.85.1 and `PI_UPSTREAM_SHA` drives only `"backend": "src"` scenarios, of which there are none; nothing this cycle touches request building. Both reference caches were present when the cycle opened. |
| Reviewed via | 2026-09-16 — per-commit triage of all seven first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Four are diffstat-dispatchable (the evals rewrite, both clipboard fixes, the extension type re-exports); the coding-agent hunks were read anyway, which is how `utils/clipboard.ts`'s consumer set was established at the sha and how `4c2d91339`'s two-line bedrock test amendment was routed to entry 9's oracle. The three in-or-near-ported-surface hunks (`bedrock-converse-stream.ts`, `model-config.ts`, `session-manager.ts`) were read in full and every Go-home claim was grepped rather than recalled. **Triage verified adversarially before any Go was written, every agent read-only:** 6 auditors (an independent from-scratch re-triage, a ledger-bookkeeping audit of every count/sha/date/line-ref/CI run, a Go-surface check of every home and absence claim, a completeness critic over pi-sync §0/§4/§5, a stale-text hunter over the carried-forward rows, and a boundary adjudicator arguing the opposite of each judgement call), then 3 refuters per filed finding instructed to REFUTE and to default to refuted when uncertain: **22 filed, 17 survived** (14 at 3–0, 3 at 2–1), 5 refuted. Every survivor was acted on. The MEDIUM that turned the cycle — `findById` has a Go home beside `ListSessions` (2–1) — became `57a3304`. Three independent auditors caught the same false "no `ci.yml` run" claim for three shas (a paging artefact of the unscoped runs endpoint). The rest: a mis-dated ruling citation (2026-06 → 2026-07-17), a mis-dated counter history in row 12 (2026-09-09 → 2026-09-10), an off-by-one file count, a mis-scoped `--name-status` sentence in the commit message, the 14-vs-16 re-export count, two stale sentences in entry 12's status cell (the port DOES have read-side V1→V2→V3 migration; the `grep -c label` claim now names the production files), the unstated membership principle of the entries list, and the `2b768ba42` precedent replaced by `46bde88a1`'s. The 5 refuted: the E1 derived list should carry the host `core` cluster (2–1 — refuted because `model-runtime.ts` has in-scope importers, so listing the cluster for diffstat dispatch would encode a wrong boundary), the queued-class rule has no permanent home (3–0 — it is pi-triage's "a queued tree is never `n/a`"), entry 9's delta date (upstream author date is the convention), the commit message asserting the difftest tally before the run (timing), and a duplicate of the re-export count. **Port review (pi-sync §3):** pi-go-review, pi-parity-review and a JS-semantics lens ran as three independent read-only reviewers over the port commits, then 3 refuters per finding: 9 filed, 4 survived (all LOW, 3–0), all fixed in `12c0622` and `a005e68` — the parity-proofs row says what they were. |
