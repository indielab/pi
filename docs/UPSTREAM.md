...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`96617628e`** — "fix(ai): use reasoning_effort for reasoning-capable mistral-medium-* models" (advanced 2026-09-08). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all 9 first-parent changes in `9767ba275..96617628e` (no merge-smuggled commits — the all-commits count equals the first-parent count): **1 port / 2 port-but-QUEUED / 1 port-but-CATALOG-ONLY / 5 n/a / 0 decide**. The unported balance is carried by **Scope queue entries 3, 6, 11, 12, 13 and 14** plus the catalog-only queue — not by the pin. The next cycle's drift is `96617628e..origin/main` PLUS those rows; a drift check that reads the sha alone will see a clean slate and be wrong. **Shipped this cycle:** `b2602be77` — pi split EventStream into TWO collections and converted BOTH to a FIFO, and the port had only one: `ai/eventstream.go` had no `waiting` collection at all, so every consumer woken by `cond.Broadcast` raced for `queue[0]`. Measured, that did not merely fail to guarantee order — it inverted it, delivering to concurrent consumers in REVERSE registration order 399/400 at GOMAXPROCS=10 and 200/200 at GOMAXPROCS=1. The port now mirrors pi's structure: `Push` hands an event directly to the longest-waiting consumer. pi's two-stack `FifoQueue` itself is NOT ported and never will be — it exists to make JS's O(n) `Array.shift()` O(1), and a Go slice reslice already is (measured flat at ~30 ns/event from 25k to 400k events, 22.93 B/op steady-state, no retention). **Also shipped, a pre-existing defect this cycle's own material surfaced:** `coding/session_tree.go`'s `BuildContext` dropped the summary of an EARLIER compaction sitting inside the latest compaction's kept range (its `appendMessage` closure had no `case "compaction"`), where pi projects *every* compaction entry in that range. Model-visible: a twice-compacted session resumed with one summary block fewer than pi sends. **Not shipped:** entries 3, 11, 13 are untouched this cycle; 6 and 12 each gained one delta. **Reconciliation at the advance:** detector 43 paths, A/D/R is 7 additions (all test files or `packages/evals` source) — **zero deletions, zero renames, no new top-level package, nothing moved out of `packages/`, no new repo-root directory**; accounting 26 files, all accounted for. None of the four split-by-HUNK tripwire files was in range, so the standing `defaultModelPerProvider` rule did not fire. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`v0.85.1` is dated 2026-09-05 and predates the pin; `git tag --contains 9767ba275` is empty), so there was **no catalog regen and no reference-build refresh** this cycle. The catalog-only queue therefore REOPENED at **0 → 1**: `7d8ab31a4` broadens Copilot's Responses routing from a `gpt-5` prefix to a `gpt-` prefix in `packages/ai/scripts/generate-models.ts`. Sized against the shipped catalog rather than guessed: of the 10 `github-copilot` `gpt-*` models, **9 are already `openai-responses`; exactly one flips** — `github-copilot/gpt-6-astra`, `openai-completions` → `openai-responses` — and no `grok-`/`oswe-`/`mai-` model is affected. The existing `gpt-6-astra` prompt-cache test (`ai/providers/openai_responses_test.go`) reads the **openai** provider, not copilot, so it does not fire. |
| Parity proofs at the pin | **2026-09-08:** both units were written red-first with a **behavioural** red, and neither red was a compile failure. The EventStream red is a transliteration of pi's own new test (`packages/ai/test/event-stream.test.ts` @ `b2602be77`, "delivers events to waiting consumers in registration order") and failed **5/5** with the exact inversion `first:2 second:1` against pi's pinned `first:1 second:2` — a deterministic wrong answer, not flakiness. The compaction red asserted 5 context messages and got 4, with `SUMMARY-ONE` absent, matching a differential run against the published `@earendil-works/pi-coding-agent` 0.85.1 `dist` (pi 5 messages, port 4). Neither defect was pinned green by an existing test: `TestEventStreamConcurrentConsumers` asserts only the **sum** across consumers (5050), which is order-independent, and the two existing compaction fixtures plus `coding/testdata/sessparity/1_compaction_custom.json` are all single-compaction — the behaviours were uncovered, not mis-pinned. Full gate green: `gofmt`/`build`/`vet`, `go test -race ./... -count=1`, `GOOS=windows` build+vet, and **difftest 51 PASS / 0 KNOWN / 0 FAIL** against 0.85.1 with an empty known-divergence baseline. The difftest re-pin to `96617628e` is bookkeeping only: all 51 scenarios are `dist`-backed, and `PI_UPSTREAM_SHA` drives only `"backend": "src"` scenarios, of which there are none. |
| Reviewed via | 2026-09-08 — per-commit triage of all 9 first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Diffstat-dispatched only for `packages/tui`, `packages/evals`, `modes/interactive`, docs/CHANGELOG and packaging; hunks read for everything touching `packages/ai/src` and `packages/coding-agent/src/core`. Then an adversarial pass: **6 independent read-only lenses** (fresh re-triage from scratch, an attack on the "already satisfied" EventStream verdict, an attack on both queue assignments, catalog blast-radius, completeness/missed-hunk, and an attack on all five `n/a` calls), with **every** filed finding handed to **3 refuters** instructed to REFUTE it and defaulting to refuted when uncertain: **11 filed, 7 refuted, 4 survived unanimously**. The pass paid for itself twice — it overturned the cycle's own "no Go change needed" verdict on `b2602be77` (which would have recorded a near-deterministic ordering inversion as an accepted divergence and permanently immunised it, since a finding against a recorded divergence is refuted, not fixed), and it caught the `BuildContext` compaction defect from the corrected upstream docs. Reviewers were read-only; no reviewer mutated the tree. |
