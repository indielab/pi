...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`8676a0dcd`** — "fix(coding-agent): require advertised X11 clipboard images" (advanced 2026-09-24). **10 changes in `b313731b8..8676a0dcd`, union = first-parent** (no merges). **No release crossed:** `git tag --contains b313731b8` is empty after an explicit tag fetch, and `packages/ai/package.json` is still 0.87.1. **Verdicts: 5 port / 5 n/a / 0 decide.** Shipped: `002fc8385` the provider stream-event observer, in four parts — `7955cff` (`StreamOptions.OnProviderStreamEvent` + `ai.DecodeOrderedValue`), `3d56313` (anthropic, pi-messages, agent, `coding.SessionOptions`), `0ed63b9` (both openai loops), `bcb1c39` (google, the @google/genai 2.21.0 chunk shape); `667fc3dd3` 1-hour cache writes in message_delta (`a7ac78b`); `a328aa89a`'s Go-homed half — the case-folding `providerHeadersToRecord` (`18e0d57`), `TYPESAFE_API_KEY` (`3160cea`), `Model.Type` + the typed/all-model `Models` accessors + the unknown-type store filter + `AssertChatModel` (`a667855`), and the session chat guard (`2d32f42`). **Queued:** `a328aa89a`'s image + classifier half → row 3, widened to "non-chat model operations" (its four image-catalog deltas subsumed, `image-models.generated.ts` deleted upstream); `002fc8385`'s azure/vertex/mistral/bedrock/codex hunks → rows 4/5/6/9/10; `b45597504` (`packages/durable` conformance suite) → row 8; `core/remote-catalog-provider.ts` → **new row 19** (the 2026-07-17 only-consumers premise re-judged false). **Catalog-only (2):** `fde38ed7c`, `a328aa89a`'s generator. **n/a (5):** `4c2dfd936` (empty commit), `36af9dc48` (tui, E1), `8d897edaa` (extension package warnings + installer args, E1 — `package-manager.ts` split by hunk, none of its five hunks touches `coding/resources.go`), `7fd564cbb` (repo-root scripts, CI, a test-only pi.dev catalog-protocol suite — row 19's oracle), `8676a0dcd` (`utils/clipboard-image.ts`, sole consumer `modes/interactive`, E1). **Upstream `ci.yml` per sha:** `4c2dfd936` FAILS `github-copilot-anthropic.test.ts` (live models.dev displaces the hard-coded copilot Opus 5.5 entry, as at the pin; `fde38ed7c` fixes it); the other nine green. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.87.1** (and `pi-coding-agent` 0.87.1), unchanged — no release in range, so `ai/models_catalog.json` is untouched. `coding/testdata/defaultmodels/default-models-8676a0dcd.json` re-captured from src at the pin (`capture.mjs --src`; 41 defaults, identical in value and order to 0.87.1's). **New src-only oracles** (upstream `packages/ai/src` at `8676a0dcd`/`002fc8385`, SDKs at the lockfile versions — openai 6.40.0, @anthropic-ai/sdk 0.124.0, @google/genai 2.21.0, partial-json 0.1.7, undici 8.10.2 — integrity-identical to the 0.87.1 cache): `ai/providers/testdata/{cache-write-1h,anthropic-sse,pi-messages,response-headers,anthropic-on-response,openai-stream,streaming-json,google-stream-events,abort-before-response}` and `internal/jstext/testdata/{json-parse-errors,utf8,lower}-node.json`. **Re-capture them against the first published build that ships these commits** and re-run each capture whenever the lockfile moves one of those SDKs. |
| Parity proofs at the pin | **2026-09-24.** Every behaviour change was observed RED for a behavioural reason by mutating the shipped code in a scratch copy; each commit message carries its table. **difftest re-pinned** (`PI_UPSTREAM_SHA` 8676a0dcd, `PI_NPM_VERSION` 0.87.1) at **61 PASS / 0 KNOWN / 0 FAIL, all `dist`-backed** (+1: `replayed-args-index-keys-openai-completions`). The harness module gained a `go.sum`: `jstext.ToLower` imports `golang.org/x/text/cases`, already a root requirement, and without it the Go side failed to build — a dark run. Full gate on the integrated tree: `gofmt -l` clean, `go build`/`go vet`, `go test -race ./... -count=1` green, `GOOS=windows` build + vet, `GOOS=linux GOARCH=386` build. |
| Reviewed via | 2026-09-24 — per-hunk triage (four maps, each challenged by a critic), four port slices in parallel worktrees, then pi-go-review + pi-parity-review per slice with a three-refuter adversarial pass on every finding, looping until dry. **Stopped by the owner after 526 agents**: 149 findings filed, 102 survived, across up to three rounds; S2's round 3 never ran and S1's round 3 was mostly unjudged — see "Open findings — 2026-09-24 sync". The slices duplicated shared fixes (`ai/ordered.go`, `internal/jstext`), reconciled at integration. Beyond the upstream diffs: `ModelsError` text is pi's message (`2384bf0`); OnResponse errors fail anthropic, faux and openai streams and fire on a 2xx only, google no longer calls it (pi never does), repeated header values join (parity-sweep-2 C10 closed); every request header value is trimmed and converted as fetch's `Headers` does, names fold with JS `toLowerCase`; both openai loops read their stream as the openai SDK's `Stream` does and fail unparseable events with V8's SyntaxError text (`jstext.JSONSyntaxError`); streamed tool-call arguments are read by a port of partial-json 0.1.7 (`aed5207`); ordered decoding lists array-index keys first and reads out-of-range numbers as ±Inf; an SSE-framed google error event reads on, as @google/genai does (parity-sweep-2 F3 was wrong about the SDK); google undoes its own content codings; an abort before the response says pi's `Request aborted` on all four adapters (`f35136b`); K5 (`021571e`) and K7 (`c7ea248`) closed. **API changes (mirror ruling):** `StreamGoogle` never calls `OnResponse`; `ai.Diagnostic.Details` is an `ai.OrderedObject`; `CreateProvider` panics with pi's message when given no implementation; `ModelsError.Error()` drops its `code: ` prefix; openai's request `Accept` is `application/json`, as the SDK sends. New Divergences D77–D86, Known parity debt K21–K25. |
...
### Open findings — 2026-09-24 sync, carried to the next cycle

The 2026-09-24 port workflow was stopped before its last review round finished
(owner's call: it had grown to 526 agents for a three-port cycle). What it left:
**S2's round-3 re-review never ran**, so the openai loops' final fixes (`24c970a`
and the commits before it) were reviewed only through round 2. **S1's round 3
was filed but mostly not judged or fixed**, and **S3 kept three survivors**.
Each is a claim, not a measurement — verify by execution before acting, and fix
or record it as a D/K row; do not carry this list past the next cycle.

- anthropic and pi-messages: an abort mid-stream ends with net/http's `context
  canceled` where pi says undici's `This operation was aborted` (google fixed in
  `e680700`; the before-response abort is fixed for all four in `3967d15`).
- anthropic: the `+=` on a non-string seed (`2111900`) always concatenates; JS
  `+` on two non-string primitives adds numerically.
- anthropic: a CRLF split across two reads is two line breaks in pi's reader
  (the second flushes an empty event); the port waits for the LF. The replay
  test's comment claims read-boundary independence for pi, which is false.
- pi-messages: the error message and `details.statusText` use Go's
  `http.StatusText`, where pi uses the server's reason phrase (`resp.Status`
  carries it).
- pi-messages: the hole-bound error counts unset slots as blocks.
- anthropic: the 16 MiB SSE line cap fails with the bare `bufio.Scanner: token
  too long` (openai's is hinted, D83).
- google: the per-read TextDecoder step of `processStreamResponse` (BOM strip,
  WHATWG decode) is not applied before the bare-JSON check (the fixer was
  stopped mid-change); an unparseable base URL keeps Go's `url.Parse` text where
  pi says `Invalid URL`; no test covers `googleFetchError`'s `Op == "Post"` guard.
- tests: `responseHeadersRecord` is exercised after the body is read (trailers
  merge in), and `sseDoer` duplicates `cannedDoer`.
...
the matching npm build. **Currently 2 (2026-09-24: `fde38ed7c`, `a328aa89a`);
...
headers timeout instead (`bb33378`). This is
...
### chord/delta immutable tracking (ported 2026-09-23, upstream `9a139c62b`)

Upstream deleted the mutable operation-log tracker (its rows, D64–D67, went with it) for an immutable one: `track(initial)` holds committed revisions, `beginChange()` hands out a revocable copy-on-write draft, `prepare()` diffs the draft's result against the committed revision (`diffRevisions`), `adopt()` commits. The oracle is `chord/delta/testdata/upstream_delta.json`, captured by `testdata/capture.mts` from pi's own `packages/chord/src/delta` at `9a139c62b` (no npm build carries it): 284 diffRevisions pairs (upstream's state-diff suite, 120 random identity-sharing arrays, the op cap, the anchor and semantic-table budgets, and the snapshot threshold and each cost term at pi's flip point), 109 tracker/draft scenarios, 13 probes of the undefined slots pi's splice fallback leaves past 10,000 inserted items, state-fuzz's 100 seeds x 100 steps, and 20 random scripts over every draft operation. Every batch is compared byte for byte, modulo D69's member order.

**D69 — chord/delta.** Object member order. JavaScript enumerates an object's keys integer-like first (ascending), then in insertion order; a Go map has no insertion order, so `DiffRevisions` visits an object's members — and `Draft.Keys` lists them — integer-like first, then by UTF-16 code unit: the order of an object whose keys were inserted sorted. encoding/json marshals a payload's members in a third order, byte order (`{"10","9","a","｡","😀"}` in that order, where `Draft.Keys` gives `9 10 a 😀 ｡`), so a pi replica decoding a Go payload inserts them in byte order, and JavaScript then enumerates its integer-like keys first. Where pi's insertion order differs, a batch lists the same member ops in a different order (sets, then deletes, on both sides; nothing else moves). Measured against pi: 1,387 of state-fuzz's 10,000 batches, 54 in the differential scripts, and upstream's own state-diff case "emits sets and deletes" (`change` was inserted before `add`). The replica is the same value either way — member ops on distinct keys commute — but a pi replica's key insertion order follows the order of Go's ops. Not fixable without an ordered object in the port's value model, which the port declines for the whole wire (Rulings, "Do NOT make the CBOR decoder order-preserving"); a Go producer's own map literal has no order to preserve. The golden records pi's own order as `raw` beside Go's.

**D70 — chord/delta.** The draft is methods, not a Proxy; doc.go maps each trap to its method. Consequences: a write through a settled draft returns `ErrDraftRevoked` and a read panics with it (pi throws a TypeError from both); `Get`/`Has` see own members only (`"toString" in draft` is true in pi); `Pop`/`Shift` return `(value, present, err)` where pi returns undefined; the traps with no Go spelling (defineProperty, setPrototypeOf, preventExtensions, symbol keys, a mutator called on a foreign receiver) have no port. `Get("length")` and `Set("length", v)` on an array are kept, since they are the property they spell; `Set` converts `v` as ArraySetLength does (ToNumber: null 0, booleans 0/1, strings by StringToNumber, arrays through their string form, an object with a "toString" member a TypeError), pinned by 64 assignments captured from pi.

**D71 — chord/delta.** Immutability is by contract. Upstream freezes every revision and deep-freezes each prepared change's ops ("deeply freezes prepared metadata" is not ported); Go cannot freeze a map or slice, so `Tracker.Value`, `Prepared.Base`/`Value`/`Ops` and every op payload are documented read-only and shared. The failure mode differs, and it is the port's own `Apply` that reaches it: a tracker's batches carry its revisions (a base batch `Replace{tracker.Value()}` is the committed revision; `Prepared.Ops`' payloads are `Prepared.Value`'s containers), so applying them in-process with the mutable applier writes into the revisions. pi throws — measured under node at `9a139c62b`, both ways the review traced: apply of a prepared batch to a replica built from the base batch ("Cannot redefine property: x", the tracker's value intact) and codec recovery from a base batch ("Cannot redefine property: deep") — where Go changes the revision silently (a never-adopted change becomes visible in `Tracker.Value`, and a later change can prepare `[]` while replicas differ). doc.go (Streams) and `Apply`'s doc therefore direct in-process consumers of a tracker's batches to `ApplyImmutable`, as pi's replicas do (services/state.ts:295,315), or to detach them first; TestCodecSurvivesRecoveryFromTheLastBaseBatch replays that way and asserts the retained revision unchanged. The store's ownership bookkeeping (value.ts's `#owned` WeakSet and `commit`'s repeated-placement expansion) has nothing to do in Go: every container in a revision was allocated by the import or by the draft, and the draft clones on every insertion, so no container is placed twice (pinned by TestDraftCopiesAssignedValues). One part of it is observable and ported: the draft context's `owned` set, as the arrays a change created (`transaction.fresh`). pi writes those in place, so finalize skips their dense check and the store's commit makes it, after the whole tree, parent first, as "Replicated state arrays …"; an array of the revision fails in finalize as "Draft arrays …" (golden "dense check …" scenarios).

**D72 — chord/delta.** Container identity. `===` on containers is a map's header address, or a slice's backing array plus its length. Every array the tracker owns gets a backing array of its own — capacity at least one, because two `make([]any, 0)` share Go's zero-size address and would be ONE array to the draft: writes to one empty array landed in the other (pinned by TestDraftEmptyArraysAreDistinct, red with the extra slot removed). For `DiffRevisions` over values it did not build, an empty slice with no capacity — every `[]` encoding/json decodes, each its own array in pi's `JSON.parse` — is identical to nothing, so decoded revisions align exactly as pi's do (golden "parsed empty arrays …"; with every such slice one array, `[1,[],2]` → `[2,[],1]` was a permutation where pi sets two elements). Two residuals, both of values a Go caller builds by hand: one empty `[]any{}` shared by both revisions is one array in pi but identical to nothing here (give it capacity, `make([]any, 0, 1)`, as the golden's `$ref` does — "a shared empty array …"), and two headers over one backing array with one length are one array here where pi would hold two. Either can change which elements anchor an alignment, never the replica. A value with no JSON form handed to `DiffRevisions` matches nothing.

**D73 — chord/delta.** Holes cost memory. `SetLen(n)` and an index written past the end materialize one slot per hole, where a sparse JavaScript array costs nothing until it is filled; a change that never fills them fails `Prepare` on both sides with pi's text, but `SetLen(1e9)` allocates in Go first. On a 32-bit build a slice holds at most 2^31 - 1 elements, so a length or index past that (up to pi's 2^32 - 1) fails the write with an error instead; indices, keys and wire addresses past 2^31 otherwise mean the same as on a 64-bit build (`Index` and `PathID` are int64).

**D74 — chord/delta.** `Draft.Sort` is `slices.SortStableFunc`; V8 runs TimSort. Both are stable, so a consistent comparator yields the same order and the same ops, but the comparator is called on different pairs a different number of times — measured under node, 5 descending ranks take 8 calls in V8 and 10 in Go, 5 ascending 6 and 4, 2 elements one each. A comparator with side effects agrees only as far as its calls do; upstream's own "keeps comparator edits when sorting object drafts" uses two elements. The nil comparator is JavaScript's default order: each element's `String()` in UTF-16 code units, then undefined slots, then holes. It takes each string once, before sorting, which matches V8 exactly: with two or more elements every one is compared, so an element with no string form (an object with an own "toString" member, directly or inside an array) fails the sort with pi's TypeError whichever pair V8 would have compared first, and the array is left untouched either way.

**D75 — chord/delta.** Go values in, JSON values out. Track, PrepareReplace and every draft write accept nil, bool, any Go string or numeric kind, json.Number, `map[string]any` and `[]any` (typed nils as empty), store every number as float64, and refuse everything else with pi's text: a struct, pointer, or typed map or slice as "…must be plain objects or arrays" (pi's class instance), a func, chan or complex as "…must be strict JSON". The Go errors also name the offending type, and a map is copied in key order so that of several bad members the one reported is always the same.

**D76 — chord/delta.** Not ported: value.ts's `JsonRevisionStore.adopt`, whose one caller is services/state.ts's `ReplicatedStateReplica` (state.ts:295,315) — the replicated-state runtime D52 leaves to its own slice (Scope queue entry 11); and draft.ts's `produce`/`produceWithMetadata`, `@internal` helpers that nothing calls at `9a139c62b` (`MutableReplicatedStateImpl` moved to `track`/`beginChange`/`prepareReplace`/`adopt`, which Go ships) and that `delta/index.ts`, the package's `./delta` export, does not re-export. Neither is reachable through `@earendil-works/chord/delta`; a Go port of the helpers would be an unexported function with no caller.
...
(`ai/providers/testdata/{google-stream-events,openai-stream,streaming-json,pi-messages}`);
...
openai SSE line past 16 MiB (`maxOpenAISSELine`), openai stream JSON nested past
10,000 levels (`jstext.JSONSyntaxError` recurses per level), an openai body that
returns no data and no error 100 reads in a row (`io.ErrNoProgress`, where pi's
SDK would wait forever), and a pi-messages message holding more than 1,024 unset
content slots at once (`piMessagesMaxContentHoles`; pi's sparse array holds any
number). anthropic's SSE line cap (16 MiB) still fails with the bare
`bufio.Scanner: token too long` — carried finding.
...
**K21 — transport-error text on anthropic, openai and pi-messages.** When a
request gets no response, pi's SDK adapters say `Connection error.` and
pi-messages says `fetch failed` (so do header values carrying a control
character other than NUL/CR/LF, which undici's client refuses); the port
surfaces net/http's `Post "<url>": …`. google is fixed (`0c126e6`, `bb33378`:
every transport failure, timeouts included, is `fetch failed`). Unmeasured for
anthropic and openai: measure first, then a per-adapter mapper like
`googleFetchError`.
