...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **PORTED to `96317e50b`; REVIEWED to `64eeb82a4`** (2026-09-03). The pin sha is held on purpose: upstream fast-forwarded `main` onto its long-lived `dev` branch — **338** first-parent changes (314 non-merge + 24 sync merges), **+90,866/−24,426** across 568 files, plus **17** main-line commits that rode in as *second* parents of the dev merges and would have been missed by a first-parent sweep. Full triage (331 units): **79 port / 3 port-but-CATALOG-ONLY / 167 port-but-QUEUED / 82 n/a / 0 decide** after the 2026-09-03 mirror ruling settled both escalations. Advancing the sha now would erase the unported balance from every future drift check; it advances when Scope queue entry 11 drains. **Shipped this cycle (Phase 0):** skills-with-bash-only (`1d6dbf9e3`), write-tool message (`e583b290a`), EXIF non-Exif APP1 scan (`c6b00676b`), UUIDv7 follower timestamps (`ef3786544`), `vllmPriority` (`256f63024`), custom tool-call `arguments:{}` + cumulative delta and the `streamSimple` key-assert removal (`5c6655e76`), absent-vs-empty pins (`86bac52f9`/`1a7bc80e7`), `Models.streamDeferred` (`b37834b69`), the Anthropic beta-namespace migration + mid-conversation effort (`4e69b0c28`). **Not shipped, no Go home:** `2631b25c3` and `2b768ba42` (the port has no `SessionManager`, no label entries, no in-memory session) → queue entry 12; the architecture migration → entry 11. Reconciliation: detector 615 paths (312 A / 67 D / 236 M, no renames); accounting sweep 568 files. None of the four split-by-HUNK files appears in the range; `core/model-resolver.ts` is byte-identical at both ends. |
