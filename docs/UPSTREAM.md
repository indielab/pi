...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`f9bcd351d`** — "docs(ai): document Codex WebSocket cleanup" (advanced 2026-09-15). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all **2** first-parent changes in `ceea48f5d..f9bcd351d` — 2 commits, 2 first-parent, no merge in range. Verdicts: **0 port-shipped / 0 port-but-QUEUED / 0 port-but-CATALOG-ONLY / 2 n/a / 0 decide** — a triage-only advance with **no Go commits**, and none landed out of cycle either (`git log 09e0370..HEAD` was empty when the cycle opened). The unported balance is carried by **Scope queue entries 3, 6, 7, 8, 9, 10, 11, 12, 13, 14 and 15** plus the catalog-only queue — not by the pin — and **no queue row moved this cycle**. **Read the queued-deltas column; never inherit the previous cycle's list.** The next cycle's drift is `f9bcd351d..origin/main` PLUS those rows. **n/a (2):** `53816d7dc`, a `.github/APPROVED_CONTRIBUTORS` approval (+1 name, bot-authored), and `f9bcd351d`, one sentence appended to `packages/ai/README.md`'s Codex provider note telling callers to run `cleanupSessionResources(sessionId)` so the pooled WebSocket does not keep the process alive — prose, zero `src` lines; both diffstat-dispatched. **The classification worth keeping:** the README sentence documents behaviour that is NOT new — `cleanupSessionResources` is `packages/ai/src/session-resources.ts:12` at the old pin, authored in `23420012a` and on the main line via the merge `c5340670b` (both 2026-05-04, before the port began) — and the connection pool it drains belongs to the Codex adapter, Scope queue entry 10, whose WebSocket transport the 2026-08-31 consult answer already defers to the WS slice: at both pins the registry's only registrant is `openai-codex-responses.ts`'s `closeOpenAICodexWebSocketSessions`. `session-resources.ts` itself (24 LOC, root-exported at `index.ts:37`) has no Go home and no ledger row; with a single Codex registrant it lands with entry 10. Nothing to queue: a doc of unchanged upstream behaviour is not a delta. **Reconciliation at the advance:** detector 2 paths (`.github/APPROVED_CONTRIBUTORS`, `packages/ai/README.md`), A/D/R is **0 additions, 0 deletions and 0 renames** — no new top-level package, nothing moved out of `packages/`, no new repo-root directory. Accounting 1 file (`packages/ai/README.md`), accounted for. None of the four split-by-HUNK tripwire files was in range, so the standing `defaultModelPerProvider` rule did not fire. Upstream CI is green at `f9bcd351d` (run 34901833857); the bot commit `53816d7dc` has no run of its own. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`git tag --contains ceea48f5d` is empty, and `packages/ai/package.json` at `f9bcd351d` still reads `0.85.1` under an `[Unreleased]` changelog that this range does not touch), so there was **no catalog regen and no reference-build refresh**, and no release is cut this cycle. No `packages/ai/scripts` or `*.generated.ts` path is in range, so the catalog-only queue is **unchanged at 6** (`7d8ab31a4`, `6b94ae2ec`, `bbb61e34a`'s generator hunk, `12f59336a`, `2e6fe2f98`, `d92eb8d4b`) — see the catalog regen checklist. **For the next regen, carried from 2026-09-14:** `0c7bb7c5c`'s Responses provider label is still unreleased — the 0.85.1 build labels every Responses provider `OpenAI` — so the first build that ships it is the one to re-capture `ai/providers/testdata/httperror` against: `node capture.mjs <npm dir> pi-ai-<ver>.json`, point `loadHTTPErrorOracle` (`openai_responses_test.go`) at the new file, and then rewrite `TestResponsesHTTPErrorNamesProvider` from the new file's `openai-responses/xai` column, which today records the pre-release `OpenAI` label and after that capture records `xai`. **Carry to the next release, unchanged from 2026-09-09:** `4a6ed0194`'s MAJOR `@google/genai` bump (1.52.0 → 2.21.0) is invisible to difftest, whose google scenarios are `dist`-backed at 0.85.1, i.e. pre-bump. Re-capture the google scenarios against the first npm build that ships it. |
| Parity proofs at the pin | **2026-09-15:** no Go code changed this cycle, so there is no new red to record. The proofs of record for shipped behaviour stay with the commits that shipped it — most recently `ac474d2`, `66d2fdd`, `256d036` and `b32a55b` (the google `TOO_MANY_TOOL_CALLS` bounce and the Responses HTTP-error label and tail), whose red-first record is this row as committed in `09e0370`. Full gate re-run on the unchanged tree at the advance: `gofmt -l` clean, `go build`/`vet`, `go test -race ./... -count=1` (every tested package `ok` — 14 of 17, the other three have no test files), and `GOOS=windows` build+vet, all green. **difftest re-pinned to `f9bcd351d` and re-run: **51 PASS / 0 KNOWN / 0 FAIL** (exit 0) against 0.85.1 with an empty known-divergence baseline, so no entry reports FIXED and none retires.** The re-pin is bookkeeping only — all 51 scenarios are `dist`-backed at 0.85.1 and `PI_UPSTREAM_SHA` drives only `"backend": "src"` scenarios, of which there are none. Worth a line: both reference caches were absent on this machine when the cycle opened (`~/.cache/pi-upstream` and `~/.cache/pi-npm/0.85.1`), so the upstream mirror was re-cloned and `run.sh` re-installed `@earendil-works/pi-ai@0.85.1` from npm before capturing — the tally is against a fresh install of the published build, not a cached tree. |
| Reviewed via | 2026-09-15 — per-commit triage of both first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Both are diffstat-dispatchable (a contributor approval and a README sentence); the README hunk was read anyway, which is how `cleanupSessionResources` was traced to `session-resources.ts:12` at the old pin and to its 2026-05-04 origin (`git log -S`), establishing that the sentence documents pre-existing entry-10 behaviour rather than a new delta. Adversarially verified before the ledger commit, every agent read-only: **5 auditors** — an independent from-scratch re-triage that reproduced both verdicts, both reconciliation sweeps, the tag/version check and the CI read; a ledger-bookkeeping audit of every count, sha, date and cross-reference in the four rows and the commit message; a Go-surface check on `cleanupSessionResources`, its registry and its registrants; a completeness critic over pi-sync §0/§4/§5 for an n/a-only cycle; and a stale-text hunter over every sentence carried forward or dropped — then **3 refuters** per filed finding, instructed to REFUTE and to default to refuted when uncertain: **5 filed, 0 survived** (three refuted 3–0, two 2–1). The one MEDIUM — that the 2026-09-14 cell's upstream-CI-read narrative leaves the ledger with no permanent home — was refuted on pi-sync §5: it is cycle narrative, which lives in git, not a ruling. **Two refuted findings were acted on anyway** because the underlying fact was re-verified independently of the classification argument the refuters won: the draft named only the merge `c5340670b` as `cleanupSessionResources`'s origin, and the authoring commit `23420012a` reached the main line through that merge's second parent — the rows now name both. The re-triage also surfaced, without filing it, that `session-resources.ts` is a root-exported generic registry with no Go home; it is recorded above against entry 10 rather than opened as a row, since nothing in range touches it and its only registrant is the Codex adapter. |
