...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`ceea48f5d`** — "Revert \"fix(ci): Fix a broken test\"" (advanced 2026-09-14). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all **8** first-parent changes in `62129190d..ceea48f5d` — 8 commits, 8 first-parent, no merge in range. Verdicts: **3 port-shipped / 0 port-but-QUEUED / 0 port-but-CATALOG-ONLY / 5 n/a / 0 decide.** The three ports, one Go commit each: `ac474d2` (upstream `71dca871b`: Google `TOO_MANY_TOOL_CALLS` leaves the error-mapped set), `66d2fdd` (upstream `ceea48f5d`: the revert of that, restoring it — `google.go` ends the cycle byte-identical to where `81d296c` left it), and `256d036` (upstream `0c7bb7c5c`: Responses HTTP errors are labelled with the model's provider — `OpenAI` for `openai`, the id verbatim otherwise — in `ai/providers/errors.go`'s `formatResponsesHTTPError` and its one caller in `openai_responses.go`). **Plus one boy-scout commit with no upstream sha, `b32a55b`:** the Responses HTTP-error TAIL now matches pi byte for byte — pi lets the stringified `error` object replace the SDK's `${status} ${msg}` (error-body.ts `messageCarriesBody`) and never caps the SDK message, where the port had pinned the SDK message under a 4,000-unit cap of its own. Surfaced by the parity gate measuring the 0.85.1 build, fixed in the function `256d036` had just touched (2026-09-02 rule: local and provable), oracle committed under `ai/providers/testdata/httperror`; the same capture is what K18 records for google and anthropic. No Go commit landed out of cycle (`git log e930306..HEAD` was empty when the cycle opened). The unported balance is carried by **Scope queue entries 3, 6, 7, 8, 9, 10, 11, 12, 13, 14 and 15** plus the catalog-only queue — not by the pin — and **no queue row moved this cycle**. **Read the queued-deltas column; never inherit the previous cycle's list.** The next cycle's drift is `ceea48f5d..origin/main` PLUS those rows. **n/a (5):** two `packages/agent/docs/pico/*.md` prose commits (`7b4cfd6eb`, `f3c672245`); the evals commit `b21588402` (E1 — every hunk under `packages/evals/**` plus repo-root `vitest.base.ts`, despite its `coding-agent` subject); and the DeepSeek test pair `e8413008c`/`713bdf38d`, an exact revert (`git diff e8413008c^1..713bdf38d -- packages/ai/test` is empty) — the "fix" pointed at the pre-rename id, `12f59336a` (already queued catalog-only, an ancestor of the pin) having renamed `deepseek-v4-flash` to `deepseek-flash`, so the revert was right and the Go tests still on the old id stay where the regen checklist records them. **The classification worth keeping:** `71dca871b` was mirrored under the 2026-09-03 ruling (upstream deletes, we delete) even though upstream's own build rejected it — `@google/genai` 2.21.0 declares the member, so the 17-case switch failed the `never` exhaustiveness check and upstream CI was red at `npm run build` — and upstream reverted it 63 hours later (Friday evening to Monday morning), by which point the drop had already been ported here — the port commits were rebuilt afterwards with corrected messages, so their dates postdate the revert; do not read the ordering off `git log`. Both commits are in range and the port mirrors both; a re-judge row drafted for the bounce was dropped unrecorded because the bounce landed inside the cycle (`git log` holds the story). **Reconciliation at the advance:** detector 18 paths, A/D/R is **2 additions** (`packages/evals/src/models.eval.ts`, `packages/evals/src/providers.eval.ts`), **0 deletions and 0 renames** — no new top-level package, nothing moved out of `packages/`, no new repo-root directory (`vitest.base.ts` is a modified repo-root file). Accounting 6 files, all accounted for — **and note what the sweeps cannot see:** `google-shared.ts` nets to zero across the range, so neither sweep prints it; only the per-commit triage saw the two commits that touched it. None of the four split-by-HUNK tripwire files was in range, so the standing `defaultModelPerProvider` rule did not fire. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`git tag --contains 62129190d` is empty, and `packages/ai/package.json` at `ceea48f5d` still reads `0.85.1` under an `[Unreleased]` changelog, which `0c7bb7c5c` extends by one line), so there was **no catalog regen and no reference-build refresh**, and no release is cut this cycle. No `packages/ai/scripts` or `*.generated.ts` path is in range, so the catalog-only queue is **unchanged at 6** (`7d8ab31a4`, `6b94ae2ec`, `bbb61e34a`'s generator hunk, `12f59336a`, `2e6fe2f98`, `d92eb8d4b`) — see the catalog regen checklist. Upstream CI is green at `ceea48f5d` (it was red at `71dca871b` and `0c7bb7c5c`, both on the same build error), so a release can be cut from HEAD again. **For the next regen:** `0c7bb7c5c`'s provider label is unreleased — the 0.85.1 build labels every Responses provider `OpenAI` — so the first build that ships it is the one to re-capture `ai/providers/testdata/httperror` against: `node capture.mjs <npm dir> pi-ai-<ver>.json`, point `loadHTTPErrorOracle` (`openai_responses_test.go`) at the new file, and then rewrite `TestResponsesHTTPErrorNamesProvider` from the new file's `openai-responses/xai` column, which today records the pre-release `OpenAI` label and after that capture records `xai`. **Carry to the next release, unchanged from 2026-09-09:** `4a6ed0194`'s MAJOR `@google/genai` bump (1.52.0 → 2.21.0) is invisible to difftest, whose google scenarios are `dist`-backed at 0.85.1, i.e. pre-bump. Re-capture the google scenarios against the first npm build that ships it. |
| Parity proofs at the pin | **2026-09-14:** four Go changes this cycle — three ports and one boy-scout fix — each red-first and behavioural: observed failing with the code untouched, then green on the code change alone. `ac474d2`: `TOO_MANY_TOOL_CALLS` moved from the `TestGoogleFinishReasonSafety` table into a table-driven `TestGoogleFinishReasonUnknownFails` and its row failed on the message (got `Provider stopped with: TOO_MANY_TOOL_CALLS`, want `Unhandled stop reason: …`). `66d2fdd`: the row moved back and failed the other way (got `Unhandled stop reason: TOO_MANY_TOOL_CALLS`, want `Provider stopped with: …`); the same commit tightens `TestGoogleFinishReasonUnknownFails` to byte equality under the boy-scout rule, the bytes being provably verbatim on both sides (pi's catch stores `error.message` unchanged for a plain Error, because `normalizeProviderError` finds no status or body and `formatProviderError` then returns the message as-is; Go's fail closure copies `err.Error()`). `256d036`: `TestResponsesHTTPErrorNamesProvider` drove a 403 through the adapter for `xai`, `opencode` and `openai`; the first two failed on the label and `openai` passed, before the formatter changed. `b32a55b`: `TestResponsesHTTPErrorMatchesPi` replays all 35 rows of the captured oracle (`ai/providers/testdata/httperror/pi-ai-0.85.1.json`, produced by `capture.mjs` streaming pi's four adapters in the published 0.85.1 build against a loopback server) through the Go adapter and compares byte for byte; **23 of 35 rows failed** against the previous commit's formatter — tail replaced by the `error` object, JS key order, JS escaping (`<` where encoding/json writes `\u003c`), U+2028, the astral-character cap, the SDK message capped where pi never caps it, and three classes the parity gate's own probing added to the oracle: falsy JSON-scalar bodies (`null`, `0`, `false`, `""`) surfaced as raw text, a number JSON.parse overflows to Infinity, and a leading UTF-8 BOM — then all 35 went green. Full gate at the advance: `gofmt -l` clean, `go build`/`vet`, `go test -race ./... -count=1` (every package `ok`), and `GOOS=windows` build+vet, all green. **difftest re-pinned to `ceea48f5d` and re-run: **51 PASS / 0 KNOWN / 0 FAIL** (exit 0)** against 0.85.1 with an empty known-divergence baseline, so no entry reports FIXED and none retires — bookkeeping, since all four changes are response-side (a stop-reason mapping and an error message) and touch no request body, and all 51 scenarios are `dist`-backed at 0.85.1. difftest covers request bodies only; the oracle above is the response-side proof. Where the build cannot reach — the provider label from `0c7bb7c5c` is unreleased — the label is asserted from the source rule (`openai-responses.ts:198-201`) with the oracle's tail. The Google strings are pi's source strings (`google-shared.ts`'s `Unhandled stop reason: ${_exhaustive}`; the `Provider stopped with: ` prefix from `d7b02636`). |
| Reviewed via | 2026-09-14 — per-commit triage of all 8 first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Diffstat-dispatched for the two pico prose commits and the evals commit; hunks read for the five `packages/ai` commits, tests included — which is how the DeepSeek pair was established as an exact revert rather than assumed, how `71dca871b`'s one deletion was traced to its Go home, and how `0c7bb7c5c`'s prefix was shown to reach only the HTTP-status path (a plain Error carries no status, so `formatProviderError` returns the message untouched). **The build-broken commit was measured, not trusted:** `@google/genai` 2.21.0's `FinishReason` was read from the published package (`genai.d.ts`, `node/node.d.ts` and `web/web.d.ts` all declare `TOO_MANY_TOOL_CALLS`), both versions of `mapStopReason` were typechecked against it under pi's `NodeNext` resolution (the 18-case switch passes, the 17-case one fails `TS2322`), and upstream's `ci.yml` run was read for every sha in range that has one (`e8413008c` has none — it was pushed together with `7b4cfd6eb`). The cycle re-fetched upstream after its first verification round and found `0c7bb7c5c` and `ceea48f5d` had landed since preflight; both were folded in rather than left as next-cycle drift. Adversarially verified before the ledger commit, every agent read-only: four rounds, every agent read-only. **Round 1** (5 auditors — independent re-triage, go-review, parity, ledger bookkeeping, completeness critic — and 15 refuters): 5 filed, 5 refuted, 0 survived; the critic's refuted HIGH (upstream had moved past the range) was acted on regardless, which is how `0c7bb7c5c` and `ceea48f5d` entered the cycle. **Round 2** (5 auditors, 36 refuters): 12 filed, 8 survived — a wrong "sixteen hours" in three places, the CI-run wording, and the Responses HTTP-error tail, which the parity gate had measured against the 0.85.1 build; that finding became the boy-scout commit's oracle and fix. **Round 3** (4 auditors, 57 refuters): 19 filed, 14 survived — falsy JSON-scalar bodies, an Infinity-valued number, the BOM, a stale sha and a wrong red count in the rebuilt messages, and K18's claim that `anthropicSDKErrorMessage` was unused (it feeds the retry fail-fast quote); all folded in. **Round 4** (4 auditors, 21 refuters): 7 filed, 5 survived — the BOM strip moved to the SDK level so the retry quote matches pi, a discriminating nested-message row, the `openai-responses/xai` capture column, the capture header's citation, and the invalid-UTF-8 decode gap, recorded in K18 rather than fixed because every adapter shares it. Ledger bookkeeping and completeness both verdict `ship` in round 4; parity's remaining `diverges` is exactly K18's decode gap. |
