...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`be26e3270`** — "docs: extract interactive testing and release guidance into skills (#9370)" (advanced 2026-09-09). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all 7 first-parent changes in `96617628e..be26e3270`, and unlike last cycle the two counts DIVERGE — **19 commits, 7 first-parent** — because `3e4bc2680` merges a 12-commit branch whose entire content is `packages/agent/src/harness/**`. Verdicts: **3 port (one of them part-queued) / 4 n/a / 0 port-but-CATALOG-ONLY / 0 decide**. The unported balance is carried by **Scope queue entries 3, 6, 7, 8, 9, 10, 11, 12, 13, 14 and 15** plus the catalog-only queue — not by the pin. That list is longer than last cycle's and none of the growth is new debt: the previous cell named only "3, 6, 11, 12, 13 and 14" while entries 7, 8, 9 and 10 all held queued deltas at the time. **Read the queued-deltas column; never inherit the previous cycle's list.** The next cycle's drift is `be26e3270..origin/main` PLUS those rows. **Shipped:** (1) `4a6ed0194` — one live hunk hidden inside a 16-file dependency bump (`@google/genai` 1.52.0 → 2.21.0), which is exactly why hunks are read for `packages/ai/src` however much the diffstat looks like packaging: `TOO_MANY_TOOL_CALLS` joined pi's error-mapped Gemini finish reasons, and without it the port fell through `mapGoogleStopReason`'s default and FAILED the stream with "Unhandled stop reason: …" where pi ends the turn with "Provider stopped with: …". The set now matches upstream's 16 members exactly. (2) `561a2e066` — an **EMPTY** delta, and the reason is structural: pi wrapped the opencode/opencode-go api tables so a bare `pi-ai` consumer would finally send `x-opencode-session`, a gap the port never had, because it homes pi's coding-agent attribution in `ai/providers/attribution.go`. Verified cell by cell across all four adapters, both entry points, and `cacheRetention: none`, then locked with a transliteration of pi's own new test. (3) `3e4bc2680` — **split by hunk, not queued whole.** Most of it (fork policy, JSONL v4, the streaming legacy-v3 source) has no Go base and is queued against entry 8; but `harness/types.ts`'s `FileSystem` is a tree the port SHIPS as `coding/execenv.go` (slice 8b-i), and upstream added a member to it (`openTextLineReader` + `TextLine`/`TextLineReader`) and rebuilt `readTextLines` on it. Porting that half surfaced a defect in shipped Go code: `ReadTextLines` split the whole file on `\n` and returned a **phantom trailing empty line** for every newline-terminated file, which pi has never produced. The helper it borrowed, `splitLines`, counts lines the way pi's READ TOOL does — its comment said "as in pi", true of a different function — and `maxLines` could hand the phantom back as a real line. Fixed in place under the boy-scout rule, red-first. **Opened:** Scope queue entry **15** — deliberately a queue row and not a Divergences row, because a recorded divergence makes every future finding against it refuted-by-default. **Queued:** entry 8 gained `3e4bc2680`'s unported half (11 → 12). No `packages/ai/scripts` delta appeared, so the catalog-only queue stands at 1. **One classification was challenged and stands:** `faa9863cb` edits `core/agent-session.ts`, for which entry 12 exists holding six deltas — a reviewer argued it should be queued there rather than `n/a`. It stays `n/a` on E1's only-consumers clause and on a structural check: the whole new path is gated on `_extensionRunner.hasHandlers("input")`, the port has no extension runtime at all, and `Session.Steer`/`FollowUp` are thin passthroughs, so the change is a provable no-op here. Recorded so the next cycle can reopen it rather than rediscover it. **Tripwire FIRED** — see the harness tripwire below, which was checked because this cycle touched the harness tree, and which had to be re-pointed before it could be read at all. **Reconciliation at the advance:** detector 53 paths, A/D/R is 9 additions (2 `.pi/` skills, 5 harness src/test, 2 `packages/ai` opencode src/test) — **zero deletions, zero renames, no new top-level package, nothing moved out of `packages/`, no new repo-root directory**; accounting 44 files, all accounted for. None of the four split-by-HUNK tripwire files was in range, so the standing `defaultModelPerProvider` rule did not fire. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`git tag --contains 96617628e` is empty) and every `package.json` in the monorepo still reads `0.85.1` at `be26e3270`, so there was **no catalog regen and no reference-build refresh**, and no release is cut this cycle. The catalog-only queue stands at **1**, unchanged: `7d8ab31a4` (Copilot's Responses routing broadens from a `gpt-5` prefix to a `gpt-` prefix in `packages/ai/scripts/generate-models.ts`; sized against the shipped catalog, exactly one model flips — `github-copilot/gpt-6-astra`, `openai-completions` → `openai-responses`). Nothing under `packages/ai/scripts` was touched in this cycle's drift, so the queue neither grew nor drained. **Carry to the next release:** `4a6ed0194` is a MAJOR `@google/genai` bump (1.52.0 → 2.21.0). The port hand-rolls the Google request rather than linking that SDK, so any wire-shape change the bump brought is invisible in the TS diff AND invisible to difftest, whose google scenarios are `dist`-backed at 0.85.1 — i.e. pre-bump. Re-capture the google scenarios against the first npm build that ships it. |
| Parity proofs at the pin | **2026-09-09:** two of the three units were written red-first with a **behavioural** red, and neither red was a compile failure. The google red is the new table entry in `TestGoogleFinishReasonSafety` failing on the asserted message (`error message = "Unhandled stop reason: TOO_MANY_TOOL_CALLS", want "Provider stopped with: TOO_MANY_TOOL_CALLS"`) while the two existing rows stay green, so the red isolates the new behaviour. The `ReadTextLines` red is `[one two ""]` against pi's `[one two]`, with an unterminated-final-line control that was green before and after. The third unit has **no red, by design**, and says so rather than hiding it: `ai/providers/opencode_provider_headers_test.go` is a **characterization** test whose assertions were green before they were written — which IS the finding for `561a2e066`. It is not vacuous, and that was measured rather than argued: deleting the session attribution from `google.go`, `openai_responses.go` or `anthropic.go` fails it, as does dropping the headers entirely or fabricating one without a session id. Its first draft covered one of the four api tables pi's single wrapper wraps, and review caught that the other three could lose the header with the whole repo still green; it now runs all four. Every assertion is pi's own from `packages/ai/test/opencode-provider-headers.test.ts` @ `561a2e066`, and the one cell where pi's two paths disagree (a case-variant deletion marker on google) is deliberately left unasserted, because a green test carrying the port's own value would immunise entry 15. The `TextLineReader` unit is likewise transliterated from pi's own `packages/agent/test/harness/text-line-reader.test.ts` @ `3e4bc2680`, chunk-boundary and malformed-UTF-8 cases included; both of its new behaviours were mutation-checked. Full gate green: `gofmt`/`build`/`vet`, `go test -race ./... -count=1`, `GOOS=windows` build+vet, and **difftest 51 PASS / 0 KNOWN / 0 FAIL** against 0.85.1 with an empty known-divergence baseline. The difftest re-pin to `be26e3270` is bookkeeping only: every scenario is `dist`-backed and `PI_UPSTREAM_SHA` drives only `"backend": "src"` scenarios, of which there are none. **None of this cycle's changes was difftest-reachable in any case** — the harness captures request BODIES, while `561a2e066` is a header, `4a6ed0194` is response-side stop-reason mapping, and `3e4bc2680` is filesystem I/O. That is a limit of the instrument, not a gap in the proof, and it is why all three units carry unit-level oracles taken from pi. |
| Reviewed via | 2026-09-09 — per-commit triage of all 7 first-parent changes PLUS the 12 commits `3e4bc2680` smuggles in, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Diffstat-dispatched only for `modes/interactive`, `.pi/`, docs/CHANGELOG and packaging; hunks read for everything touching `packages/ai/src` and `packages/coding-agent/src/core` — the only reason the one live hunk inside `4a6ed0194` was found. Two adversarial passes, both read-only: **6 investigators** before porting (upstream header semantics across both pi paths, the port's own behaviour matrix cell by cell, the google enum member-for-member, an attack on the empty-delta verdict, the harness delta, and ledger bookkeeping), then **5 review lenses** after (`pi-go-review`, `pi-parity-review`, a from-scratch re-triage running both reconciliation sweeps, a second independent attack on the empty delta, and a test-quality critic hunting vacuous or self-serving assertions). Every filed finding went to **3 refuters** instructed to REFUTE and to default to refuted when uncertain: **17 filed, 17 refuted, 0 survived**. **A clean sweep is a result to be suspicious of, and it was not treated as a verdict.** Three refuted findings were acted on anyway, each because the porter re-verified the underlying FACT independently of the classification argument the refuters won: the one-of-four-adapters coverage hole (demonstrated by mutation — three adapters could lose the header with the repo green), the possibility of a negative assertion passing without a request being made (`Header.Get` on a nil header returns `""`), and the "no Go base" rationale for `3e4bc2680` (`coding/execenv.go` is the shipped base, and porting the half that had one exposed the `ReadTextLines` phantom-line defect). A refuter panel is a filter on plausible-but-wrong findings, not a licence to discard a measured one. No reviewer mutated the tree; every mutation was the porter's, serial, and reverted. |
...
the matching npm build. **Currently 1 — REOPENED 2026-09-08.** It was drained
0 on 2026-09-07 by the 0.85.1 regen (7 queued deltas plus `17de82d7b`'s
`gpt-6-astra` generator half, all ancestors of the `v0.85.1` release commit);
`7d8ab31a4` reopened it.
