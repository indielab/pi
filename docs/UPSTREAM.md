...
**D73 — chord/delta.** Holes cost memory. `SetLen(n)` and an index written past the end materialize one slot per hole, where a sparse JavaScript array costs nothing until it is filled; a change that never fills them fails `Prepare` on both sides with pi's text, but `SetLen(1e9)` allocates in Go first.
