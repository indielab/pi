...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`744a94d7d`** — "fix(coding-agent): use current Copilot model in registry test" (advanced 2026-09-04). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all 17 first-parent changes in `64eeb82a4..744a94d7d` (no merge-smuggled commits — the all-commits count equals the first-parent count): **2 port / 2 port-but-CATALOG-ONLY / 2 port-but-QUEUED / 11 n/a / 0 decide**. The unported balance is carried by **Scope queue entries 11, 12, 13 and 14** — not by the pin. The next cycle's drift is `744a94d7d..origin/main` PLUS those four rows; a drift check that reads the sha alone will see a clean slate and be wrong. **Shipped this cycle:** `8b5899dce` (the reversal of `5c6655e76` — custom/grammar tool-call arguments are seeded at `toolcall_start` again and the seed-replay delta is gone, in `ai/providers/openai.go` + `openai_responses.go`; the anthropic auth gate widened to pi's `assertRequestAuth` header set; the `AssistantMessageEvent` contract docblock in `ai/events.go`) and `c2d3dc55b` (signal-killed processes map to `128+signum` in `coding/execenv.go`, with a new `execenv_unix.go`/`execenv_windows.go` split). **Not shipped:** the eleven upstream deletions and the server/client/harness/experimental rewrite (entry 11); `SessionManager`/`AgentSession` (entry 12); edit-tool result details (entry 13); `assistant-message-frame.ts` (entry 14, opened this cycle). Reconciliation at the advance: detector 72 paths (72 M, 0 A / 0 D, no renames — no new top-level package); accounting sweep 48 files, all accounted for; **none of the four split-by-HUNK files in range**, `core/model-resolver.ts` byte-identical at both ends. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.84.4** — **still held**. `git tag --contains 3316c4e35` is empty, the newest tag is still `v0.84.4`, and every package's `package.json` reads `0.84.4` at both ends of the range. No release tag was crossed: **no catalog regen, no port tag, no release tweet this cycle.** The catalog-only queue therefore grew rather than drained — now **7** (see "Catalog regen checklist"). The Anthropic caveat from the previous pin still stands: `4e69b0c28` is UNRELEASED in 0.84.4, so Anthropic difftest scenarios captured from the published build report a `betas`/URL mismatch until re-captured `backend:"src"`. |
| Parity proofs at the pin | **2026-09-04:** both ports were written red-first with a **behavioural** red (a compiling test asserting the wrong value), not a compile red. Independent adversarial review then filed 15 findings; each was handed to a skeptic instructed to REFUTE it, and **6 were refuted, 6 survived and were fixed, 3 were left unadjudicated by the refutation cap** (all three re-examined by hand: two duplicated survivors already fixed, the third — that the openai key check's precedence over Cloudflare base-URL resolution is unpinned — was **declined with reason**: the key check is `openai.go:118`, Cloudflare `:134`, grammar `:145`, so any mutation moving the check past Cloudflare also moves it past grammar, which the existing control already catches; it adds no distinct mutation coverage). The survivors included a **real regression this port would have introduced**: widening the anthropic gate to accept header-owned auth made the `cloudflare-ai-gateway` branch reachable with an empty key, and it synthesized `cf-aig-authorization: "Bearer "` — a credential pi never sends (fixed at `anthropic.go`, pinned by `TestAnthropicStreamSimpleGatewayHeaderOwnedAuthSendsNoEmptyGatewayBearer`). Every new precedence assertion carries a **control** — the error the same request reports when a key IS supplied — so it cannot pass vacuously, and the anthropic one was mutation-verified by moving the gate below `buildAnthropicParams` and observing it fail. **Correction to the previous pin's record:** it said `TestOpenAIStreamSimpleMissingKeySurfacesOnTheStream` pins that "the port's StreamSimple adapters never carried pi's pre-`5c6655e76` eager API-key assertion". `8b5899dce` RESTORED that assertion, so that sentence is now backwards; the test is retargeted at `8b5899dce` + G3 and asserts **precedence**, not the absence of a check. For the same reason `5c6655e76`, listed as shipped at the previous pin, now carries a partial reversal on top of it. |
| Reviewed via | 2026-09-04 — per-commit triage of all 17 first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines or from the clone's working tree. Diffstat-dispatched only for `packages/tui` / docs / host `modes/interactive` + `cli/`; hunks read for everything touching `packages/ai/src`, `packages/coding-agent/src/{core,utils}`, `packages/agent/src` and `src/index.ts`. Then a 23-agent pass: 4 serialized porters on file-disjoint units (serialized because they share one working tree), 6 independent read-only reviewers (per-unit parity, cross-cutting Go idiom, cross-cutting test quality), 1 serialized mutation-tester that reverted each shipped behaviour and re-ran its covering test, and 12 refuters. |
...
the matching npm build. **Currently 7** (5 → 7 on 2026-09-04; no tag crossed):
...
- `265a33393` — qwen-token-plan. Bigger than its subject: it adds `qwen3.8-flash`
  to `QWEN_TOKEN_PLAN_INDIVIDUAL_MODEL_IDS` (our `qwen-token-plan-individual` has
  8 models today and does not carry it), and it **replaces the hand-maintained
  thinking maps with `getEffortThinkingLevelMap(m.reasoning_options)`**, keeping a
  literal fallback for `glm-5`/`glm-5.1` only. So `thinkingLevelMap` becomes
  models.dev-derived for most of the tree — including `qwen3.8-max`, whose bespoke
  `{low, medium, xhigh}` map in our catalog has no counterpart in the new
  derivation — and `supportsReasoningEffort` flips from a 9-model denylist to
  "false whenever no map could be derived". It also drops the
  `recordModelsDevReasoningOptions` call. **Regen tripwire: thinkingLevelMap is
  request-body-affecting**, so diff it per model after the regen, not just the
  model list.
- `0dbf2b76f` — `grok-build-0.1` added to `XAI_BUILTIN_EXCLUDED_MODEL_IDS`. It is
  live in our catalog today, so the regen removes it.
