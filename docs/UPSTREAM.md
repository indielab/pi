...
**D72 — chord/delta.** Container identity. `===` on containers is a map's header address, or a slice's backing array plus its length. Every array the tracker owns gets a backing array of its own — capacity at least one, because two `make([]any, 0)` share Go's zero-size address and would be ONE array to the draft: writes to one empty array landed in the other (pinned by TestDraftEmptyArraysAreDistinct, red with the extra slot removed). `DiffRevisions` on caller-built values inherits Go's notion: two empty slices, or two headers over one backing array with one length, are one array — which can change which elements anchor an alignment, never the replica. A value with no JSON form handed to `DiffRevisions` matches nothing.
