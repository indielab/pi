...
## Drift at last sync check (2026-09-02) — pin advanced to 96317e50b

Delta `3fc3ef532..96317e50b`, **5** first-parent changes, **no merges**: the
first-parent count and the total commit count are both 5, so nothing rode in
under a merge. **No release crossed** — zero `package.json` files appear
anywhere in the range, `git tag --contains 3fc3ef532` returns nothing, and the
newest upstream tag `v0.84.4` (2026-08-28) is an ancestor of the **old** pin, so
no tag falls inside the delta. No catalog regen is due, the npm reference build
stays **0.84.4**, and **no port tag is cut and no release tweet is drafted**.
Nothing under `packages/ai/scripts` changed, so the `port-but-CATALOG-ONLY`
queue stays at **0**. Verdicts: **2 port (→ 2 Go commits + review fixes); 3 n/a;
0 decide**. The **Scope queue** takes no delta and opens no row.

Both reconciliation passes ran. The **detector** (`--name-only`, no pathspec)
printed **11** paths, every one classified: 10 status M and 1 A — the addition
is upstream's own new tui regression test — with **no D and no R even under
`--find-renames`**. The top-level tree listing and the `packages/` directory set
are identical at both ends: no new top-level package, no file moved out of
`packages/`, no new repo-root directory. The **accounting** sweep over
`packages` minus the four structural exclusions reported **8 files,
146(+)/3(−)**, each mapped to a verdict below; the three-file gap against the
detector is exactly `.github/APPROVED_CONTRIBUTORS`,
`packages/tui/src/terminal.ts` and
`packages/tui/test/regression-sigwinch-kill-eacces.test.ts`. Of the four
never-diffstat-dispatch files **none appears in the range**, so
`defaultModelPerProvider` was untouched this cycle.

### Port worklist (2 → 2 Go commits + review fixes)

| upstream | subject | Go | notes |
|---|---|---|---|
| `b8b873b98` | fix(ai): add `supportsMaxOutputTokens` compat flag for openai-responses (#8941) | `f57c288` + `bdb60cf` | **Request-body golden surface, but the default path must not move a byte.** Three upstream hunks collapse into ONE Go file: `types.ts` (the interface field) and `api/openai-responses.ts` (`getCompat` + the `buildParams` gate) land in `ai/providers/openai_responses.go`, and the third — `coding-agent/src/core/model-config.ts` registering the key in the TypeBox `OpenAIResponsesCompatSchema` — needs **no Go home**, because the port has no compat allowlist at all: `ai.Model.Compat` is a bare `json.RawMessage` and the anonymous `raw` struct inside `getResponsesCompat` is the only place any Responses compat key is named in Go. **The trap is the default.** Unlike every other Responses opt-in this flag defaults to **true** (`model.compat?.supportsMaxOutputTokens ?? true`), and Go's zero value is false, so it is seeded in the `getResponsesCompat` defaults literal beside `SupportsDeveloperRole` and `SupportsLongCacheRetention` rather than left implicit; getting that wrong would silently strip `max_output_tokens` from **every** openai-responses request. Conjunct order matches upstream (maxTokens first, compat second). No catalog model sets the flag (0 occurrences in `ai/models_catalog.json`), so the default wire body is byte-identical to `65ffeec` |
| `96317e50b` | fix(ai): bump claude code user agent version | `2bff692` | **Request-header golden surface.** `claudeCodeVersion` 2.1.75 → 2.1.251 at `ai/providers/anthropic.go:24`, consumed at `:1306`. **Scope correction worth recording:** this is *not* every Anthropic request. Upstream names the literal exactly once, at `anthropic-messages.ts:937`, inside the `isOAuthToken` client's `defaultHeaders`; the api-key, `ANTHROPIC_AUTH_TOKEN`-bearer and github-copilot branches send pi's own runtime user agent and are untouched. The port mirrors that — the header is set under `case oauth:`. Collateral swept: `anthropic_test.go:275` hardcoded the old literal and now spells the header through the constant. That created a fresh hazard the porter caught and closed — with **every** assertion reading the constant, a stale constant would keep the whole suite green while each OAuth request impersonated a retired Claude Code build — so `TestAnthropicClaudeCodeUserAgentPinnedToUpstream` pins the literal `claude-cli/2.1.251` once, against the upstream sha. Two node-probe comments in `pi_user_agent_wire_test.go` that embedded the old version were genericized to `claude-cli/<v>`; they document header-slot ordering, not the version |

### n/a (3)

| upstream | subject | why |
|---|---|---|
| `3205678b3` | chore: approve contributors from issue #8885 | `.github/APPROVED_CONTRIBUTORS`, +2 lines. Always-noise set; diffstat-dispatched |
| `605a1b038` | fix(tui): wrap SIGWINCH self-signal so restricted seccomp policies do not crash startup (#8898) | **E1.** The commit's two files in full: `packages/tui/src/terminal.ts` and its new regression test. Diffstat-dispatchable — `packages/tui` is the cheapest exclusion the port has. The port carries no terminal driver and raises no signals at startup |
| `23842b1e6` | fix(coding-agent): tunnel proxied HTTP requests (#8134) | **E1, only-consumers clause — ruled on substance, not on the filename.** See below |

### The `23842b1e6` ruling, made structurally rather than inherited

`core/http-dispatcher.ts` is **not** on the E1 derived list, and the four prior
`n/a` rulings on that file (`91050859`, `a93f0666`, `2117b61c`, `14551e769`) all
predate the 2026-08-27 rewrite and rest on "Go uses `net/http`" — a statement
about the *mechanism*, which does not by itself reach this change's substance.
So it was re-derived. The src hunk is two lines adding `proxyTunnel: true` to
the `undici.EnvHttpProxyAgent`. A whole-repo sweep at `96317e50b` finds every
non-test importer is host surface — `cli.ts`, `main.ts`, `rpc-entry.ts`,
`core/settings-manager.ts` and two `modes/interactive/**` components — and it is
re-exported from **neither** `coding-agent/src/index.ts` nor `ai/src/index.ts`.
Every consumer is host, so **E1's only-consumers clause fires**. The module is
also process-global Node/undici runtime configuration (`setGlobalDispatcher`,
`undici.install()`), which is packaging.

**Watch item (not a `decide`, no action).** The substance does leave a real
non-equivalence, and it runs *opposite* to last cycle's `NO_PROXY` table.
Upstream is **pinning** CONNECT tunnelling for plain-HTTP origins, reverting
undici ≥8.7's default. Go's transport does the undici-8.7 thing:
`net/http/transport.go:1941` issues CONNECT only when `targetScheme == "https"`,
and `:2058` routes an `http://` target through a proxy by absolute-URI
forwarding. So on a plain-HTTP `baseUrl` behind an `HTTP_PROXY` the port sits on
the behaviour upstream deemed broken for #8134. Dormant — it needs both a proxy
and an `http://` model baseUrl — and out of scope regardless, since the whole
...
**Eight independent agents, none of them the porter**, and every finding either
review filed was handed to three further agents instructed to REFUTE it.

- **`pi-go-review` — SHIP** on both changes, no findings.
- **`pi-parity-review` — FAITHFUL** on both changes. One finding, **killed
  3/3**: that a wrong-typed compat value discards all overrides. Correctly
  refuted *as a finding against these commits* — it is pre-existing, not
  introduced — but its substance was upheld separately and became the recorded
  divergence above. The finding was right about the world and wrong about whose
  fault it was.
- **Completeness sweep (3 lenses: coverage / test-strength / collateral)** —
  **9 gaps**, one **major**. The major one was real and is why `bdb60cf` exists:
  `TestResponsesSupportsMaxOutputTokens` only read `body["max_output_tokens"]`,
  and the full-body golden covered only the flag=true default with a minimal
  options set, so **two mutations of the shipped gate survived the entire repo
  suite** — nesting the neighbouring `temperature` block inside the gate (M11),
  and having the gate `delete(params, "store")` (M8). Both change the body
  without changing whether `max_output_tokens` is present, which a single-key
  lookup cannot see. `TestResponsesFullBodyUnderMaxOutputTokensFlag` now asserts
  the FULL marshalled body under both flag values with `Temperature` and
  `ServiceTier` populated; both mutations go red.
- **Final adversarial verifier** — re-derived every load-bearing mutation itself
  on a quiet tree (M11, M8, the decode-path mutation, and the difftest gate
  mutation, each with a pre-hardening control run), confirmed the shipped `.go`
  files were byte-identical to their committed state after all mutation
  testing, and independently checked the `backend:"src"` justification by
  structure (`grep -rl supportsMaxOutputTokens ~/.cache/pi-npm/0.84.4` → 0 files,
  while `max_output_tokens` → 64). It also produced the cycle's most
  consequential finding, the `want: 1024` rejection above.

**Process note — two agents mutation-tested the same file concurrently.** The
hardening agents ran in parallel on the assumption their files were disjoint;
they were not, because both mutated `ai/providers/openai_responses.go` to test
it. Each observed the other's mutations as "unauthored edits" and one escalated
three of them as possible parity defects. The verifier correctly identified all
three as the sibling agent's own M11/M8/MDEC, one-for-one. No third party
touched the tree and there was nothing to chase — but every "survived"/"caught"
datum in both reports was taken on an unstable tree, so the load-bearing ones
were re-derived serially before being believed. **Mutation testing must be
serialized within a working tree**, or given a worktree per agent.

### Base gate

`gofmt -l` clean, `CGO_ENABLED=0 go build ./...` clean, `go vet ./...` clean,
`go test -race ./... -count=1` green across all **11** test-carrying packages
(14 listed, 3 with no test files). Package count unchanged; the port gains
`TestResponsesSupportsMaxOutputTokens`,
`TestResponsesDefaultBodyUnchangedByMaxOutputTokensFlag`,
`TestResponsesFullBodyUnderMaxOutputTokensFlag` and
`TestAnthropicClaudeCodeUserAgentPinnedToUpstream`.
...
- `responses-max-output-tokens-unsupported` (**PASS**) — the flag's false arm,
  which no dist scenario could reach. Both sides emit a body with no
  `max_output_tokens` despite `options.maxTokens: 1024`. **Proven to have
...
once a release ships the flag. `ai/providers/openai*.go` changed this cycle, so
the request diff was triggered and run — the first time since the suite went
fully dist-backed that `PI_UPSTREAM_SHA` selects anything.

## Drift at last sync check (2026-09-01) — pin advanced to 3fc3ef532

Delta `853a80d26..3fc3ef532`, **7** first-parent changes, **no merges**: the
first-parent count and the total commit count are both 7, so nothing rode in
under a merge. **No release crossed** — zero `package.json` files appear
anywhere in the range, all nine versioned packages read `0.84.4` at both ends,
and the newest upstream tag `v0.84.4` (2026-08-28) is an ancestor of the **old**
pin by `merge-base --is-ancestor`, so no tag falls inside the delta. No catalog
regen is due, the npm reference build stays **0.84.4**, and **no port tag is cut
and no release tweet is drafted**. Nothing under `packages/ai/scripts` changed,
so the `port-but-CATALOG-ONLY` queue stays at **0**. Verdicts: **2 port (one → 1
Go commit + review fixes, one a proven no-op); 1 port-but-QUEUED; 4 n/a; 0
decide**.

Both reconciliation passes ran, and both were re-derived independently by a
second agent instructed to break them. The **detector** (`--name-only`, no
pathspec) printed **31** paths, every one classified: 27 status M and 4 A — the
four additions are all new upstream test files — with **no D and no R even under
`--find-renames`**. The top-level tree listing and the `packages/` directory set
are identical at both ends: no new top-level package, no file moved out of
`packages/`, no new repo-root directory. The **accounting** sweep over `packages`
minus the four structural exclusions reported **28 files, 758(+)/92(−)**, each
mapped to a verdict below; the three-file gap against the detector is exactly
`packages/tui/src/terminal-image.ts`, `packages/tui/test/terminal-image.test.ts`
and `packages/coding-agent/docs/terminal-setup.md`. Of the four
never-diffstat-dispatch files **none appears in the range**, so
`defaultModelPerProvider` was untouched this cycle.

### Port worklist (2 → 1 Go commit + review fixes, 1 no-op)

| upstream | subject | Go | notes |
|---|---|---|---|
| `afda4d620` | fix(agent): stop prepared tools after preflight abort (#8936) | `c454063` + `24d4071` | **Model-visible tool-result text; no request-body or system-prompt bytes move.** In parallel mode preparation is sequential but execution is deferred, so a call can be fully prepared before a LATER call's preflight aborts the run — and before the fix the prepared call still executed, which for a tool with an external side effect means the write happened after the user aborted. pi re-checks the signal at the head of every deferred entry; an aborted call yields `"Operation aborted"` and skips execute **and** finalize, so `AfterToolCall` never runs for it, while still getting its `tool_execution_end` and its tool result. Parity-confirmed byte for byte: `errorToolResult` ≡ pi's `createErrorToolResult` (`content:[{type:"text",text}]`, `details:{}` — and Go's `Details any` with `omitempty` still marshals `{}`, since a non-nil interface is not omitempty-empty). The **sequential** path is correctly left alone, as upstream left it: there execution immediately follows preparation and the loop already breaks on abort, so no prepared-but-unstarted window exists. Batch termination is unaffected in both (an aborted outcome sets no `Terminate`, so `shouldTerminateBatch` stays false). **Review moved the check** — see "Review gates" |
| `62835ea81` | fix(coding-agent): use `ctx.cwd` for cwd-sensitive tools when available (#8627) | — (**no-op**) | **In scope, and provably empty on the half the port has.** It threads `ctx?.cwd \|\| cwd` into the cwd-resolution call site of each of the seven cwd-sensitive tools (12 src hunks across `core/tools/{read,write,edit,grep,find,ls,bash}.ts`, many of them just the `ExtensionContext` type annotation; `read.ts` routes through `resolveReadPathAsync` and `bash.ts` through `resolveSpawnContext` rather than calling `resolveToCwd` directly), all of which ARE ported (`coding/tools.go`). But for every tool an `AgentSession` builds, the two operands are the same value: `agent-session.ts:392` assigns `this._cwd` exactly once, `:2772` builds the built-ins with `createAllToolDefinitions(this._cwd, …)`, `:2791` constructs the `ExtensionRunner` with that same `this._cwd`, and `runner.cwd` is likewise written once (`runner.ts:312`) and returned verbatim by `createContext()`'s getter. So `ctx?.cwd \|\| cwd` ≡ `cwd` on the session path. Upstream's own seven new tests never construct an `AgentSession` — they build a synthetic `fakeCtx(testDir)` and pass an explicit 5th argument, which is what you write when the real path cannot produce a differing `ctx.cwd`. The one path that DOES change is a `customTools` entry constructed at a cwd other than the session's, and that lands exactly on the ctx-passthrough seam this ledger already recorded as a deliberate divergence at the `bb3d7d39` port ("Custom/`Tools`-override bash tools get no PI_* metadata … Go's `CustomTools` are opaque `AgentTool`s built before the session exists"). This commit adds one more ctx-fed value (cwd) to that same already-ruled seam; it is a pre-existing gap, which the standing rulings resolve with the formula rather than re-escalate. **The port's `Session.Cwd` is written once in `NewSession` and has no setter**, so even a live-cwd supplier would be a constant equal to the cwd already captured |

### Port-but-QUEUED — entries 9 and 10 take their first delta

| upstream | subject | entry | what it will add |
|---|---|---|---|
| `a63fb12c1` | fix(ai): match subdomains and root domains in NO_PROXY (#8737) | **9** (Bedrock) and **10** (Codex) | `packages/ai/src/utils/node-http-proxy.ts` is in scope — no exclusion test fires — but a whole-repo sweep at `3fc3ef532` finds exactly **two non-test importers** — `api/bedrock-converse-stream.ts` (imported at :56, called at :207) and `api/openai-codex-responses.ts` (:33 / :972); the third hit is the module's own test — and it is not re-exported from `packages/ai/src/index.ts`. Both are queued with no Go base, so the delta is recorded on both rows and **must not be summed**: it is one upstream change that lands with whichever adapter ships first |

### The `NO_PROXY` non-equivalence — measured, because the obvious assumption is wrong

The tempting shortcut is that Go's `http.ProxyFromEnvironment` already does what
`a63fb12c1` added, so the delta is free. **It is not**, and the entries 9/10 note
says so because this was measured rather than assumed: the real upstream file at
the sha was executed under node against a Go probe calling
`http.ProxyFromEnvironment`, one process per case since the env is cached. The
commit's own title has two halves and Go implements only the first.

| `NO_PROXY` entry | target | pi (post-`a63fb12c1`) | Go stdlib |
|---|---|---|---|
| `example.com` | `api.example.com` | DIRECT | DIRECT — *the fix moved pi TOWARD Go here, which is what makes equivalence look plausible* |
| `.example.com` | `example.com` | DIRECT | **PROXY** |
| `*.example.com` | `example.com` | DIRECT | **PROXY** |
| `*example.com` | `api.example.com` | DIRECT | **PROXY** (inert entry) |
| `[2001:db8::1]` | `http://[2001:db8::1]:9000` | DIRECT | **PROXY** (portless bracketed entries inert; `[…]:port` does parse) |
| `[::1]:9001` | `http://[::1]:9000` | PROXY | **DIRECT** |
| *(empty)* | `http://127.0.0.1:9000` | PROXY | **DIRECT** |
| `10.0.0.0/8` | `http://10.1.2.3:9000` | PROXY | **DIRECT** — divergence in the OPPOSITE direction |

Mechanism, read from `GOROOT/src/vendor/golang.org/x/net/http/httpproxy/proxy.go`
(go1.26.4): `matchHost` is set false for any entry already starting with `.`
(:264-268, and `*.` is normalized to `.` at :261-262), so `domainMatch` only ever
suffix-matches and never exact-matches the root — whereas upstream now strips the
prefix to a bare domain and tests exact-equality (`node-http-proxy.ts:106`)
*before* the dot-suffix test (:110). Separately, Go returns "no proxy" for
`localhost` and any loopback IP (:178-188) **ahead of** consulting `NO_PROXY` at
all, and supports CIDR entries (:227-231, matcher at :338-343) that upstream's
`parseNoProxyEntry` has no concept of. All eight rows survived an adversarial
verifier instructed to refute them, each re-derived by independent execution —
two verifiers attached corrections to rows they upheld, both folded in above —
and the whole table was then re-measured a third time, from scratch, by the
ledger audit.

**Scope of the divergence.** These rows are not confined to unbuilt code: the
port applies Go's stdlib `NO_PROXY` semantics on **every** provider request, via
`http.DefaultTransport`'s `Proxy` (inherited through `base.Clone()` on the
primary path, and named literally at `ai/providers/retry.go:113` on the
fallback). pi has no counterpart there at all — `packages/ai/src` sets no fetch
dispatcher or proxy agent anywhere, and `node-http-proxy.ts` plus its two
importers are the only proxy machinery in the package — so on the generic
request path this is a divergence in the port's FAVOUR on capability, not a
gap. The table bites where the two adapters land.

**Consequence for entries 9 and 10:** `http.ProxyFromEnvironment` is not a
drop-in for `resolveHttpProxyUrlForTarget`. The Bedrock/Codex adapters need
their own resolver mirroring `node-http-proxy.ts`. Note also that pi reads
`no_proxy` from the **provider env overlay first** (`getProxyEnv` checks
`env?.[key]` before `process.env`), which the stdlib cannot do at all — so
`StreamOptions.Env` supplying proxy settings is part of the same work.

### n/a (4)

| upstream | subject | why |
|---|---|---|
| `56c6fb33c` | fix(coding-agent): settle active turn before in-memory fork (#8937) | **E1.** `core/agent-session-runtime.ts` is named twice in the boundary — the E1 derived list and the "genuinely out" table, "+ the session reload / `/new` lifecycle" — so the diffstat dispatch is allowed. The hunk was read anyway: it only moves `await this.teardownCurrent("fork", …)` above `sessionManager.newSession`/`createBranchedSession` and switches to the pre-existing `previousSessionFile`. Structural: zero port hits for `AgentSessionRuntime`, `teardownCurrent` or `createBranchedSession`; the port's only fork is `SessionRecorder.ForkFrom` (`coding/session_store.go:350`), a mutex-guarded `lastID` reassignment with no runtime teardown/rebuild, and `Session.Reset()` already refuses while a run is active |
| `f2a622789` | feat(coding-agent): adjust TUI selections in thinking-mode, models and scoped models (#8900) | **E1.** Every `src` hunk is under `modes/interactive/components/**`; the rest is CHANGELOG and coding-agent tests of those TUI selectors. The one non-cosmetic hunk (`toggle`/`normalizeEnabled` in `scoped-models-selector.ts`) is selector-local — `git grep -l enabledIds 3fc3ef532 -- packages/` returns only that component, `interactive-mode.ts` and one coding-agent regression test. Structural: zero port hits for `scopedmodel`, `enabledIds`, `fuzzyFilter` or `strikethrough` |
| `649214477` | fix(tui): detect Zed terminal capabilities (#8828) | **E1.** The commit's three files in full: `packages/tui/src/terminal-image.ts` (adds `"zed"` to the alacritty/vscode capability branch), its test `packages/tui/test/terminal-image.test.ts`, and `coding-agent/docs/terminal-setup.md`. The port carries no terminal-capability detection at all |
| `3fc3ef532` | fix(coding-agent): keep theme markers visible (#8950) | **E1.** `modes/interactive/components/settings-selector.ts` + CHANGELOG + tests; a follow-up to `f2a622789` |

### RETRACTED — the first `62835ea81` ruling, and why it was wrong

This is recorded because the ledger's value is that its *reasons* are auditable,
not just its outcomes. `62835ea81` was first ruled **`n/a` under E1's
only-consumers clause**, on the premise that the sole supplier of a non-undefined
`ExtensionContext` is `core/extensions/wrapper.ts` (host), so `ctx` is always
undefined on the SDK path. **That premise is false**, and it came from a grep
that stopped one hop short: `wrapToolDefinition` is indeed only called with a
factory there, but the caller of *that* is `core/agent-session.ts:2715-2716`,
which wraps **both** `customTools` and the **built-in** definitions with
`wrapRegisteredTools(…, runner)`. `agent-session.ts` is ported SDK surface
(`coding/session.go`), not host, so the consumer set is mixed and E1's
only-consumers clause — which fires only when EVERY consumer is host — cannot
fire.

**The finding that broke it was itself REFUTED**, and that has to be on the
record too, because this section acts on it. The triage auditor's filed remedy
was to re-triage `62835ea81` to a plain `port` with a Go home in
`coding/tools.go`'s seven cwd-capturing factories. The adversarial verifier
rejected that remedy on three legs, two of which hold: the change is a **provable
no-op** for every `AgentSession`-built tool (the worklist row's argument, which
originates with the verifier, not the porter), and the one live path — a
`customTools` entry built at a foreign cwd — is the ctx-passthrough seam already
recorded as a deliberate divergence at `bb3d7d39`, which the standing rulings say
to resolve with the formula rather than re-open. On its third leg the verifier
also defended the ORIGINAL E1 citation, arguing that `agent-session.ts` merely
*calls* a host helper and so does not make the consumer set mixed. **That third
leg is not accepted here**, and the disagreement is left visible rather than
resolved by fiat: the decision to install a context for built-in tools is taken
in ported SDK code even though the mechanism lives in host code, so leaning on
"EVERY consumer is host" was shaky at best.

What that leaves is a verdict of `port` with an empty Go delta rather than `n/a`,
chosen deliberately: the no-op grounding is settled by three single-assignment
reads that any future reader can re-run, whereas the `n/a` grounding requires
adjudicating whose surface `agent-session.ts` is — and the two classifications
produce identical work (none). **An owner who prefers `n/a` here should say so;
the tally would move to 1 port / 5 n/a and nothing else would change.**

So the original reason does not survive even though the outcome does, and "the
answer was right" is not a defence of a false premise. Two process notes worth
carrying:

- **Follow the supplier chain to its caller, not to the first factory site.** The
  question is never "who installs the context" but "who decides that a context is
  installed", and those were two different files here.
- **A no-op is a better n/a than a category.** The argument that settled this did
  not need E1 at all; it showed by reading three single-assignment sites that the
  two operands of `\|\|` are the same value. Prefer that shape — it survives a
  boundary rewrite, and a category does not.
- **A refuted finding can still be right about something.** This one's remedy was
  wrong and its supplier-chain correction was disputed, yet it destroyed the
  premise the ruling rested on. Verdicts are not the unit of value; premises are.

### Pre-existing divergence surfaced this cycle (recorded, NOT fixed) — parallel `tool_execution_end` ordering

Parity review filed the port's `tool_execution_end` ordering for a parallel batch
as a divergence from pi, and the finding was **refuted as a finding against this
commit** — but the underlying fact is real, was measured, and had never been
written down, so it is recorded here.

pi invokes the deferred entries inside one synchronous `finalizedCalls.map(...)`
turn (`agent-loop.ts:548` at the sha) and `emit` reaches the first listener
synchronously, so pi's end events are strictly slot-ordered. The port spawns one goroutine per slot and gates emits
behind `serialMu`, so ordering follows the scheduler. Measured on a throwaway
copy, 300 iterations each:

- batch `a,b` prepared + `c`'s preflight aborting — port at **`b6e2c14`** (the
  tree before this cycle's port): `296 × c,b,a`, `4 × c,a,b`; pi model:
  `300 × c,a,b`.
- plain 3-call parallel batch with **no abort anywhere** — port: four distinct
  orderings (`229 × c,a,b`, `64 × c,b,a`, `4 × a,b,c`, `3 × a,c,b`); pi model:
  `300 × a,b,c`.

So it is neither new nor abort-specific: parallel end ordering had never matched
pi and had always been nondeterministic, as a property of the pre-existing spawn
design.

**What the shipped tree changes, and what it does not.** Scope this carefully,
because the two halves differ. For a batch where every prepared call is aborted,
the ends are now emitted by the loop goroutine in slot order — the fix decided
the abort once and moved those emits off the tool goroutines entirely — so that
case is deterministic by construction and no longer races. What remains
divergent, and is **not repaired here**, is the ordinary path: a batch that
actually executes still emits its ends from racing goroutines, which is the
second measurement above and has nothing to do with abort. Repairing that means
changing how every parallel batch emits, a separate change with its own parity
surface. Tool **result messages** were never affected — both pi and the port emit
those in slot order, which is what pi's own `#8935` test asserts and what the
port now asserts too.

### Review gates

Five independent agents ran over the committed port and over the cycle's triage,
none of them the porter, and **every finding either agent filed was handed to a
further agent instructed to REFUTE it** — 15 findings, **8 upheld, 7 refuted on
reproduction**. Five of the eight upheld are `NO_PROXY` non-equivalence findings
— between them they produced the eight measured rows tabulated above — and they
change no Go code this cycle, becoming the entries 9/10 queue note instead. The
other three are the abort evaluation point and two test-assertion gaps, and
together they produced `24d4071`.

- **`pi-go-review` — SHIP**, two LOW. It verified the four named risks by
  execution rather than by reading the diff's comments: the aborted branch's
  `serialMu` discipline was proved with a negative control (the same probe with
  `safeEmit`'s `defer` removed hangs and dies at `wg.Wait`, so the probe genuinely
  detects a leaked mutex, and the shipped code does not leak one); an 8-call probe
  showed exactly one start, one end and one result per call with no double-emit;
  and it noted that emitting the end is load-bearing beyond parity, since it is
  what drives the copy-on-write removal from `AgentState.PendingToolCalls`.
- **`pi-parity-review` — FAITHFUL**, three LOW, but one of those LOWs was the
  most consequential finding of the cycle and is the reason `24d4071` exists (the
  per-goroutine abort check; see "Applied" below). It confirmed the byte-level
  result equivalence, the skipped finalize, the untouched sequential path and the
  batch-terminate behaviour, and filed the ordering and evaluation-point
  divergences against the concurrency structure rather than the semantics.
- **Test-strength (mutation) — SHIP.** Five independent mutations, every one red
  **for the right assertion**; and under the load-bearing mutation the new test is
  the **only** failing test in the package, so it is the sole guard for the
  behaviour rather than redundant with an existing one. **Provenance:** this ran
  at `c454063`, before `24d4071` moved the fix and rewrote the assertions, so it
  is a result about the first cut. Only the load-bearing red was re-observed on
  the shipped tree (forcing `batchAborted := false` reds exactly one test,
  `TestAgentParallelPreflightAbortStopsPreparedTools`, with `ran [first]`) — first
  by the porter and then independently by the ledger audit. The other four were
  not re-run after the fix.
- **Two adversarial triage auditors.** One attacked the five `n/a` rulings: four
  survived structurally, and the fifth (`62835ea81`) had its PREMISE destroyed
  even though the finding itself was then refuted on its proposed remedy — see the
  retraction above, which records both halves. The other confirmed the queued
  ruling, the no-release finding and the catalog-queue finding, reproduced both
  reconciliation sweeps exactly, and produced the measured `NO_PROXY` table.

**Applied (upheld):** the abort decision moved out of the per-call goroutine and
onto the loop goroutine (`24d4071`). Parity review filed the evaluation point
using a self-aborting tool; the decisive measurement is the **verifier's own**
additional probe, built while trying to refute the finding — an EXTERNAL abort
(the live `Session.Abort` path) fired just after the batch starts executed only
ONE of three prepared calls in **97 of 100** runs, where pi runs all three, with
the control at `b6e2c14` **100/100** pi-matching. That control is what shows the
divergence was introduced by `c454063` rather than pre-existing. pi's
all-or-nothing behaviour for an external abort is established by READING
`agent-loop.ts:520-548` at the sha — every deferred entry's check runs inside one
synchronous map turn, so the event loop cannot deliver an abort between two of
them — and corroborated by a JS model extracted from that source; a single
`batchAborted` snapshot taken before any tool body starts is the closest
structural analogue Go affords.
Also applied: the test now asserts tool-result ids and texts as ORDERED slices,
matching pi's own `#8935` assertions.

**Declined (refuted on reproduction):** extracting the three `"Operation aborted"`
literals into a Go constant — pi holds the same three inline copies at the same
three structural sites and has no such constant, the 2026-07-13 ruling already
declined this class, and under mutation two of the three sites are already
test-pinned; the parallel end-ordering "fix", proved pre-existing at `b6e2c14`
(recorded above instead); two findings against `difftest/config.env`, both
vacuous — **all 49 scenarios are `dist`-backed and none is `src`-backed**, so the
sha re-pin cannot change any capture today; a phrasing nit about which
`ai/providers/retry.go` branch names `ProxyFromEnvironment` (factually right, but
not a finding against this commit — folded into the divergence-scope note above
instead); an observation that the cycle's docs were unwritten at review time,
which this section is; and the `62835ea81` re-triage, whose disposition is set out
in the retraction above. That is all seven.

One honest note on the applied fix: the snapshot reproduces pi exactly for an
externally-raised abort, which is the realistic path and the one that was broken.
It does **not** reproduce pi's behaviour when a tool aborts *synchronously from
inside its own `Execute`* and an earlier slot does so before a later slot's check
— pi would skip the later call, the port runs it. That sub-case was
nondeterministic before and is now deterministic; no in-tree tool aborts mid-batch.

### Base gate

`gofmt -l` clean, `CGO_ENABLED=0 go build ./...` clean, `go vet ./...` clean,
`go test -race ./... -count=1` green across all **11** test-carrying packages (14
listed, 3 with no test files). Package and test counts unchanged. The new test is
stable at `-count=20`, `-count=100` and under `-shuffle=on`.

### Harness

Differential harness re-pinned `853a80d26` → `3fc3ef532` in `difftest/config.env`
and re-run twice — once after the port and once after the review fixes — **49
PASS / 0 KNOWN / 0 FAIL, exit 0** both times. The npm reference build is
unchanged at **0.84.4**, and the re-pin is bookkeeping rather than a capture
change: all **49 scenarios are `dist`-backed and 0 are `src`-backed**, so
`PI_UPSTREAM_SHA` selects nothing today — it is kept in lockstep so it is already
right the moment a `src`-backed scenario is added. Nothing under
`ai/providers/openai*.go` changed, so the 6-scenario request diff was not
triggered.
