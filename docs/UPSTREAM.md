...
**D38 — chord/delta.** `chord/delta` — ApplyImmutable validates each op before walking to it, as Apply does. Upstream `9a139c62b` made applyImmutable run assertValidOp first as well, which closed the reserved-segment half of this row (pinned by TestApplyImmutableValidatesBeforeWalking: `applyImmutable({}, [["s",["missing","__proto__"],1]])` is UnsafePathError on both sides). What remains is a string-spelled array index ON THE PATH past the array's end: pi's copyContainers checks hasOwn before the array rule and reports PathError, Go's walk reports *UnsafePathError. Verified under node at 9a139c62b: applyImmutable({xs:[{a:1}]}, [["s",["xs","5","a"],2]]) → pi PathError ["xs","5"], Go *UnsafePathError; with "0" instead of "5" both report UnsafePathError, and pi's mutable apply agrees with Go throughout. Both terminate the stream; validate-first is kept deliberately.
