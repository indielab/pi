...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`95fbc0499`** — "docs(coding-agent): update changelog with --mode validation fix (#9877)" (advanced 2026-09-22). **17 changes in `8bfef4de8..origin/main`; first-parent and the union agree** (no merges in range). **Release crossed: `v0.87.0` (`16787ad5b`)** — tag fetched explicitly and corroborated by `packages/ai/package.json`. **Verdicts: 7 port / 10 n/a / 0 decide.** Shipped: `47a18e37b` GIF signature (`4249476`), `1b6ddca87` empty text parts (`5cd0943`), the v0.87.0 regen with `890f92088`'s runtime half (`91042e1`), `10d1ad621`'s `m` permute op (`4fd7875`), and `466db0fec`'s agent hooks (`5c6af5a`) and static session-context half (`fb1f9df`). **Queued:** the rest of `466db0fec` → entries 12 and 16; the rest of `10d1ad621` → entries 8 and 11; `47a18e37b`'s harness copy → entry 8; `1a584a7a5` whole → the catalog checklist (its `defaultModelPerProvider.xai` rider waits for `grok-4.7` to reach the catalog — absent from 0.87.0). **n/a (10):** `c7cdb460a`, `1e0fe2049` (`modes/interactive/bug-report.ts`), `b6419322e` (`prompt-templates.ts`; its `resource-loader.ts` hunk feeds only prompt templates — E1 only-consumers), `aef5fc429` (`core/extensions/**`), `8c7279378` (tui), `e40126f57` (`cli/args.ts`), `eaf72ed4d` (tests), `9ac95c715` / `7f06f9cf1` / `4c8eb393c` / `95fbc0499` (changelog). **Reconciliation:** the no-pathspec detector printed only `packages/*` paths plus the root `package-lock.json`; `packages/durable` moved by docs/changelog/version only; no new package. **Upstream `ci.yml` per sha:** `466db0fec` FAILS its own build (stale expectations, fixed by `eaf72ed4d`); `16787ad5b` has no run; every other checked sha green. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.87.0** (and `pi-coding-agent` 0.87.0). Catalog regenerated by `JSON.stringify(MODELS)` after first reproducing the outgoing 0.86.1 catalog byte for byte: 41 providers (unchanged), 1,443 → **1,445 models** (+4, −2, 1,270 changed), 667,778 → 791,953 B. **The catalog-only queue drained 2 → 0**, then `1a584a7a5` joined (1). Measured, not assumed: 960/960 image models gained `inputLimits` at exactly the pipeline-default resize profile (now pinned catalog-wide by `TestCatalogResizeProfilesMatchPipelineDefaults`), and with `890f92088`'s runtime flip in the same commit **0 of 689** openai-completions catalog models changed their effective `strict`. Tripwires: BILLING clean (2/2 fallbacks carry `provider`), 7 thinking maps moved (cloudflare-workers-ai, data), no api flips. **Re-captured from pi, never hand-edited:** `completions-native-95fbc0499.json` and `tools-only-95fbc0499.json` (only `"strict":false` left the openai-completions tools), `anthropic-response-model-95fbc0499.json` (behavioural entries byte-identical; only the catalog model moved), `agent-95fbc0499.json` (captured at src 95fbc0499 AND from the 0.87.0 build, byte-identical — which discharges the agent golden's src-only caveat), the sessparity goldens and `estimate-0.87.0.json` (from the 0.87.0 build). **Carry to the next regen:** `1a584a7a5` and its xai default rider. |
| Parity proofs at the pin | **2026-09-22.** Every behaviour change was observed RED for a behavioural reason by mutating the shipped code. GIF: `GIF`/`GIFs are fun` came back image/gif and `notes.txt` read as an image. Empty text parts: a leading `{"text":""}` part, and an empty-only message sent. Strict flip: restoring the old default turned 12 tests red, the re-captured oracles among them. Agent hooks: 12 mutations, each red (the table is in `5c6af5a`); the four `continue()` characterization tests each fail under at least one queue mutation (`e7e2e78`). Session context: every rule's mutation fails a pi-captured sessparity scenario (`fb1f9df`), and the malformed-edit fix was red against scenario 21 first. `m` op: 10 mutations, each red (`4fd7875`). **Independent differential checks by the review gates:** 36 `m`-op cases against pi src; 16 agent-loop scenarios traced event-by-event against the 0.87.0 loop (all identical); 12 extra adversarial session scenarios captured from pi (all pass); every re-captured oracle reproduced byte for byte by the reviewer. **difftest re-pinned** (`PI_NPM_VERSION` 0.87.0, `PI_UPSTREAM_SHA` 95fbc0499) at **60 PASS / 0 KNOWN / 0 FAIL**, two new scenarios (`empty-text-part-openai-completions` src, `replayed-args-html-chars-openai-completions` dist). Full gate on the shipped tree: `gofmt -l` clean, `go build`/`go vet`, `go test -race ./... -count=1` green, `GOOS=windows` build + vet green. |
| Reviewed via | 2026-09-22 — per-commit triage from real diffs on the union range; ports in parallel worktrees (agent, session, chord), small ports and the regen in the main tree. **Two independent review passes, each applying the pi-parity-review, pi-go-review and JS-semantics lenses, each in its own detached worktree.** Every MED finding was reproduced with executed pi-vs-Go evidence before being fixed. Batch A (GIF, empty text, regen, chord): no correctness defect; the difftest re-pin was owed (done here); a per-key test row pinned a D3-class strict value and was dropped from the blob, with D3 extended to name it; the resize check was labelled a regen tripwire. Batch B (agent, session, estimates): **`ba7f2e5`** a malformed `context_edit` was skipped at load, orphaning every later entry and silently dropping the history before it — pi keeps the entry; **`fd8c8cf`** replayed tool-call `arguments` (openai-completions, Responses), the compaction summary's tool lines and the google/faux deltas carried json.Marshal's `&`/`<`/`>` escapes into model-visible text — pre-existing, on every OpenAI-compatible provider; **`084d2ea`** `jstext.Stringify` wrote `-0`; the agent golden still read the removed hook's scenario and was re-captured (`b5ce0dc`); four upstream `continue()` scenarios had no Go test (`e7e2e78`). **`681fb65`** (boy scout, surfaced by the porter): `ai`'s tool-token estimate measured json.Marshal's text — 32 `<` estimated 61 tokens where pi says 21 — now one JSON.stringify-faithful `internal/jstext.Stringify` serves both estimators. Two estimate gaps that only the queued cut-point rework can close are recorded on entry 12. |
...
the matching npm build. **Currently 1 — drained 2 → 0 on 2026-09-22 by the
0.87.0 regen, then `1a584a7a5` joined.** Both drained rows were ancestors of the
`v0.87.0` release commit (`git merge-base --is-ancestor <delta> 16787ad5b`,
checked per row, never by log order), and each landed as predicted; the
measurements are in `91042e1`'s message — 960/960 image models gained
`inputLimits` at the pipeline-default resize profile, 620 of 689
openai-completions models gained an explicit `supportsStrictMode: true`, and
with `890f92088`'s runtime half landed in the same commit, **0 of 689** catalog
models changed their effective `strict`.
...
|---|---|
| `1a584a7a5` (2026-09-22) | **A runtime rider travels with this row — land them in ONE commit.** Generator: xai models.dev entries take `cost: getModelsDevCost(m.cost)` instead of four flat fields, so long-context price `tiers` reach the catalog (pi's test pins `grok-4.7` at 2/6/0.5 with a `tiers[0]` of `inputTokensAbove: 200000` at 4/12/1). Rider: `defaultModelPerProvider.xai` `grok-4.6` → `grok-4.7` (`core/model-resolver.ts` → `coding/resolve.go:37`) — **held, not ported**, because `grok-4.7` is absent from the 0.87.0 catalog (checked in the tarball; the commit post-dates `16787ad5b`) and a default naming a model the catalog lacks fails every xai session that relies on it. **Check at the regen:** `xai/grok-4.7` exists, then move the default, then confirm the every-default-resolves guard is green; verify `ai.ModelCost` decodes `tiers` (grep the Go type before assuming). |
