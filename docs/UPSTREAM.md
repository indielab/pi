...
| What | Value |
|---|---|
| TS source fully reviewed/ported | `ccfe79ed2` — "feat(extensions): ui prompt events (#8355)" (2026-08-27; **4 port, 0 port-but-catalog-only, 9 n/a, 0 decide**). Delta `a79b37334..ccfe79ed2`, **13** first-parent changes, **no merges** — first-parent count and total commit count are both 13, so nothing rode in under a merge. **NO RELEASE CROSSED**: no tag contains the pin and no `packages/*/package.json` moved, so `pi-ai` stays **0.84.3**, the catalog is untouched, the npm reference build does not move, **no port tag is cut and there is no release tweet**. Four Go commits plus a harness re-pin and a review-fix commit: `tool_choice` un-suppressed and the compaction override dropped (`351c0f4`), the streamed thinking signature serialized once (`c728a26`), unterminated session files repaired on resume (`46116a4`), Windows `taskkill` resolved by absolute path (`d5d7b4a`), the harness re-pinned (`5075436`) and review fixes (`b789eca`). **Two byte-golden surfaces moved**, both test-locked: the openai-completions request body now carries `tool_choice` with no `tools` (upstream reverted its own two-day-old guard), and the `thinkingSignature` is published only at block end rather than per delta — session format and the streamed partials, not the replayed request body, which is byte-unchanged. The DEFAULT system prompt did **not** move and `systemprompt_golden_test.go` is in no commit's file list. The `port-but-CATALOG-ONLY` queue is **unchanged at 3** — `packages/ai/scripts` is tree-identical across the range — and the **harness backlog goes 11 → 12**, the `packages/agent/src/harness/env/nodejs.ts` half of `7af2d27dc`. Both reconciliation passes ran: the unrestricted `--name-only` **detector** printed **44** paths, all classified, no new top-level package, no file moved out of `packages/`, no new repo-root directory; the **accounting** sweep reported **28 files, 523(+)/59(−)**. Base gate green before and after: `gofmt -l` clean, `go build ./...` and `go vet ./...` clean (plus `GOOS=windows` build and vet, new this cycle), `go test -race ./... -count=1` green across all **10** test-carrying packages, with **8 new test functions** (3 of them inversions of assertions the previous cycle had pinned the other way). Differential harness re-pinned `a79b37334` → `ccfe79ed2` and **fully dist-backed at 49 dist / 0 src** — the last two `src` scenarios retired, the suite's first cycle needing no upstream clone at all — at **49 PASS / 0 KNOWN / 0 FAIL**, exit 0. The pin still means *"everything in scope is reviewed and ported **except the harness tree**"* — see the 2026-08-07 ruling. Prior pin: `a79b37334` (2026-08-25, 6 ports, release v0.84.3). |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.84.3**, **unchanged this cycle** — the delta crosses no release, so the reference build stays where the 2026-08-25 regen put it. Installed at `~/.cache/pi-npm/0.84.3`; package-lock integrity == registry `dist.integrity` for BOTH `pi-ai` (`sha512-M0YUV8vN…`) and `pi-coding-agent` (`sha512-Yr2p9Pub…`), re-verified independently by the parity reviewer this cycle rather than taken from the porter. `ai/models_catalog.json` is **byte-untouched**: `packages/ai/scripts` and `models.generated.ts` are tree-identical across `a79b37334..ccfe79ed2`, so there is nothing to regen and the 2026-07-30 "decide by EXECUTION" rule has no work to do. The catalog-only queue therefore stays at **3** — `e8c632ef6` (Workers AI mirrored under `workers-ai/`), `650e7a612` (OpenRouter reasoning controls) and `587be985a` (deepseek-v4-flash-vision-exp) — all still parked for the next release regen. Vendor SDKs unchanged, so every proof standing on their pinned versions still stands. Scenario backends are now **49 `dist` / 0 `src`**, the first fully dist-backed suite with no `src` scenarios at all; the published build became the right reference for the tool-choice pair again the moment upstream reverted `fe37e9f9b`, because 0.84.3 never contained that guard. |
