...
**D76 — chord/delta.** Not ported: value.ts's `JsonRevisionStore.adopt` and draft.ts's `produce`/`produceWithMetadata`, whose only callers are services/state.ts's `ReplicatedStateReplica` and `MutableReplicatedStateImpl` — the replicated-state runtime D52 leaves to its own slice (Scope queue entry 11). None is exported by `@earendil-works/chord/delta`.
