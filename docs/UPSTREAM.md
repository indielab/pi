...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`590144609`** — "fix(tui): reduce fuzzy search latency" (advanced 2026-09-18). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** **AND READ THIS SECOND: first-parent lies in this range.** The `pico` branch was merged so that it BECAME main's first-parent line, so `--first-parent 9b791a4cc..origin/main` is 30 units that are mostly pico/chord design docs, while the real upstream mainline is a **25-commit chain (`9b791a4cc..fe219d7f8`) reachable only through merge SECOND parents**. A first-parent diffstat showed ~86 changed `packages/ai/src` lines; the union range shows **1,287 insertions / 785 deletions across 28 `packages/ai/src` files**. Triage ran on the union. This is the 2026-07-17 "merges smuggle in-scope hunks" lesson at full scale — if a future cycle's first-parent log reads as docs-only, check the union before believing it. Note also that the four `Merge origin/main into pico` commits REPLAY work already inside the pin, so a per-merge diff double-counts; only the 25-chain and the branch's own commits are new. **Verdicts over the judged units: 10 port-shipped / 4 port-but-QUEUED / 5 port-but-CATALOG-ONLY / 11 n/a in the chain (plus the branch-side docs, tui and experimental commits) / 0 decide.** **port-shipped, this cycle (5):** `e98f287ee` → `ai/retry_classify.go` (`df5e7d4`); `a8b3dd199` → `coding/tools.go` + `coding/tooloperations.go` (`b8be9ef`); `16292398a` → `ai/{types,transcript}.go` + `coding/session.go` (`d9e6d21`); `2c995acf4` + `c4289b20e` → `chord/delta/**` (`da559ed`, `8b6ac52`). **port-shipped, OUT of cycle (5) — the reconciliation that a pin-only drift check would have missed:** `git log e88db74..` held 12 commits when this cycle opened, porting `9e05370b2` in six slices (`e6d507d`…`54d04a7`), then `e4c75a732` (`f12caf2`), `16235fd93` (`8b5f489`), `1283afd0d` (`b16a0e7`) and `e5d18382a` (`fa4f2cc`), plus two JS-semantics fixes (`9aeed84`, `16dda13`, retiring D16). Verified complete against the union: `NormalizeContext` is called at all three of the port's stream entries (`ai/stream.go` ×2, `ai/models_runtime.go`), matching `ModelRuntime.stream`/`streamSimple`/`streamApi`, so `9e05370b2`'s `provider-composer.ts` and `model-runtime.ts` threading hunks are covered. **The commit worth remembering is `16292398a`:** it WITHDRAWS `e4c75a732`'s `SystemMessage.replace` three days after upstream shipped it, and the port had shipped it as `f12caf2` one cycle earlier. Under the mirror ruling (2026-09-03) upstream deletes and we delete — no shim, no deprecation window — so `Replace` is gone from `ai/types.go` with its key slot, decoder and marshal arm, the two clears are out of `ai/transcript.go`, `hasLateReplacement` is deleted, and `ai/{,providers/}transcript_replace_test.go` and both `replace-e4c75a732.json` fixtures went with the feature. A forced prompt is now PROJECTED onto the request by `withForcedPromptProjection`/`projectForcedPrompt` and never recorded, so the transcript keeps its structured sections. **port-but-QUEUED (4):** `661619e87` → **new entry 16** (`ai/utils/overflow.ts`: the port has no `isContextOverflow` at all, so it cannot react to a provider refusing a request as too long when its own estimate said otherwise); `bdee230f1` → entry 3; `46b66c59a` and `cf8d5fac3` → entry 8 (the Pico3 kernel, ~7,500 src lines, **which SURVIVES at upstream HEAD** — `56cd5989e` deleted only the earlier `harness/pico/**` prototype and `729d5cb74` only the design docs, so reading either subject as "Pico was reverted" is wrong; check the tree at the sha). `cf8d5fac3`'s `ai/` and `agent/harness/` hunks are `n/a` under **E3**: `JsonRepresentation`, `IsJsonCompatible` and the conditional `ToolResultMessage<TDetails>` are type-level only. **n/a in the chain (11):** `1247476e6`, `42cd371ba`, `5a3a03a7f` (evals — E1); `509ee2bd0`, `46c9de402` (`core/extensions/**` — E1, and structurally confirmed: the port has no extensions runner, only prose); `465853498`, `13784598d`, `fde6d778f`, `590144609` (interactive-mode / TUI components / tui fuzzy — E1); `fe219d7f8` (`core/tools/renderers/bash.ts` — a `Container` subclass, E1); `7140838fd`, `781139411`, `ea9e09351` (contributors / changelog / tests-only). Branch-side: `7e1950768` (the experimental `micro` agent — its own `main.ts` + `tui.ts`, zero importers at HEAD, E1) and the `docs(agent)`/`docs(chord)` commits. **Reconciliation at the advance:** detector 332 paths, top-level `packages/{agent,ai,coding-agent,evals,chord,tui}` + `tsconfig.json` + `package-lock.json` + `.github` — **no new top-level package, nothing moved out of `packages/`, no new repo-root directory**. `--name-status -M` is 71 A / 62 D / 198 M / 1 R (the R is `packages/agent/docs/telemetry-schema.md` → `packages/agent/telemetry-schema.md`). Accounting (excluding tui/evals/docs/examples) 311 files, +26,473 −34,521 — the large deletion side is the pico prototype and design-doc churn, classified. None of the four split-by-HUNK tripwires (`model-resolver.ts`, `package-manager.ts`, `trust-manager.ts`, `telemetry/src/index.ts`) is in range. No new project-local read. **Upstream `ci.yml`, read per sha via `actions/workflows/ci.yml/runs?head_sha=` (never the unscoped endpoint):** `e98f287ee`, `a8b3dd199`, `ea9e09351`, `13784598d`, `fe219d7f8` and the pin `590144609` are green — but **`781139411`, `16292398a` and `46c9de402` FAIL**, and the failure is upstream's own catalog drift, not the content: `tsgo --noEmit` rejects test files naming retired model ids (`glm-5.2`, `glm-5.1`, `glm-5v-turbo`), red from a CHANGELOG-ONLY commit (`781139411`, which cannot break a type check by its content) and fixed by `ea9e09351` ("update tests for current model catalogs"). `generate-models.ts` fetches models.dev live, so the window is time-dependent rather than commit-dependent. That window is why `ea9e09351` is `n/a`-but-worth-naming: it is a tests-only commit that is also the fix for a three-commit red streak crossing a sha we ported. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`git tag --contains 9b791a4cc` is empty, and `packages/ai/package.json` at `590144609` still reads `0.85.1`; every `packages/ai/CHANGELOG.md` line in range sits under `[Unreleased]`), so there was **no catalog regen and no reference-build refresh**, and **no release is cut this cycle** — hence no tag and no release tweet. The catalog-only queue is **10**: it grew by five (`9e05370b2`'s generator half, `6671c6047`, `aa50fe778`, `3955b27a1`, `bb0f4aa60`) and **lost one**. **The retirement is the thing to carry forward:** `d92eb8d4b` queued `supportsToolReferences: true` onto the Fireworks compat, and `9e05370b2` then DELETED `supportsToolReferences` from `AnthropicMessagesCompat`, `deferredToolsMode` from `OpenAICompletionsCompat`, and the row's own oracle (`test/fireworks-deferred-tools.test.ts`) along with `src/utils/deferred-tools.ts` and the `kimi-deferred-tools` example. Replaying it at the next regen would have emitted a key the type no longer has. **So: verify every queued row against the generator at HEAD before replaying it** (`git -C <clone> grep -n "<the key>" origin/main -- packages/ai`) — a row is a description of code that may no longer be there. The five additions also form a SECOND ordered chain that crosses the first; the checklist header now carries the full replay order for all ten rows. **For the next regen, carried from 2026-09-14:** `0c7bb7c5c`'s Responses provider label is still unreleased — the 0.85.1 build labels every Responses provider `OpenAI` — so the first build that ships it is the one to re-capture `ai/providers/testdata/httperror` against: `node capture.mjs <npm dir> pi-ai-<ver>.json`, point `loadHTTPErrorOracle` (`openai_responses_test.go`) at the new file, then rewrite `TestResponsesHTTPErrorNamesProvider` from the new file's `openai-responses/xai` column. **Carry to the next release, unchanged from 2026-09-09:** `4a6ed0194`'s MAJOR `@google/genai` bump (1.52.0 → 2.21.0) is invisible to difftest, whose google scenarios are `dist`-backed at 0.85.1. Re-capture the google scenarios against the first npm build that ships it.
| Parity proofs at the pin | **2026-09-18.** Every behaviour change was observed RED for a **behavioural** reason, by mutating the shipped code and watching a test assert a wrong value — never a compile error. **`df5e7d4`:** the Azure case returned false without the pattern. **`b8be9ef`:** two mutants, two reds — the local ops returning nil on a signal ("exit code = <nil>, want 137"), and the tool treating a nil code as success. **`d9e6d21`:** six mutants, six reds — projection disabled ("system messages per request = [1 1 2 3], want [1 1 1 3]"), head taking a fresh timestamp, head dropping the replayed `toolsAdded`, head prepended without dropping the other system messages, an opaque replayed prompt declaring nothing, and `ResolveTranscript` always collapsing. **One mutant was discarded rather than counted:** making an opaque transcript emit the full sections directly is what `DiffSystemPromptSections(nil, …)` already returns, so it is a no-op refactor and its survival is not a coverage gap — worth recording, because a surviving mutant that proves nothing is easy to file as a hole. **`b5e3e4d`:** three mutants, three reds. **`30f93a4`:** the unnamed-signal case pinned. **`da559ed` + `8b6ac52`:** six existing goldens went red on their own, and the 25 new tests were each killed by mutating the shipped code (11 + 13 mutations plus the registry-pruning one); all 36 `held/<mutator>/<index>` op batches diff IDENTICAL to pi's, captured by running upstream's `delta/index.ts` at `c4289b20e` under node. **Goldens all re-captured FROM pi at the ported sha, never hand-edited** — `coding/testdata/sessionprompt/sessionprompt-16292398a.json` (its `capture.mts` now extracts `_installAgentForcedPromptProjection` verbatim from upstream source and installs it as the constructor does), `agent/testdata/transcript/agent-16292398a.json`, `coding/testdata/systemprompt/systemprompt-16292398a.json` and `ai/testdata/retry-classify/classify-e98f287ee.json` (121 → 123 rows). **The parity gate re-derived all four independently and got `cmp`-identical files**, which is the check that matters against an oracle bent toward the Go. Exactly the four sessionprompt scenarios `e4c75a732` had changed moved back, and two were renamed for what they now prove. **difftest re-pinned to `16292398a`** (`PI_UPSTREAM_SHA`; `PI_NPM_VERSION` unchanged at 0.85.1) and re-run on the final tree: **58 PASS / 0 KNOWN / 0 FAIL**, exit 0. Full gate on the shipped tree: `gofmt -l` clean, `go build`/`go vet`, `go test -race ./... -count=1` all green, and `GOOS=windows` build + vet green.
| Reviewed via | 2026-09-18 — per-commit triage judged from real diffs (`git diff <sha>^1..<sha>`), on the UNION range rather than first-parent, for the reason the pin cell gives. **Three independent read-only review gates over the port commits, each mutation-testing in its own detached worktree: pi-go-review, pi-parity-review, and a JS-semantics lens.** Findings were acted on rather than routed to refuters, because each arrived with a concrete failure scenario and **every one reproduced verbatim in the live tree** — a reproduced failure has nothing to refute, and the refuter pass exists to kill findings that rest on a premise, not ones that rest on a measurement. **4 survived, 3 filed by more than one gate.** The two MEDIUMs that mattered, both against `d9e6d21` and both fixed in `b5e3e4d`: the forced-prompt projection filtered system messages by CONCRETE TYPE where pi filters by role — while `GetCurrentSystemMessage` two lines above reads pointer form — so a `*ai.SystemMessage` was replayed into the head AND left beside it, yielding `"Exact forced prompt.\n\nORIGINAL RECORDED PROMPT"`, the exact failure `16292398a` exists to prevent; and `EnableCompaction` ASSIGNED `Agent.TransformContext` while its comment claimed it re-wrapped, silently dropping an embedder's wrapper. The fix inverted the ownership: one chain installed at `NewSession` (previous → compaction → projection last) with compaction in a slot. Boy-scout in the same function: a re-enable was discarding the compaction checkpoint, against the "compaction is PERMANENT" invariant. **The finding only the JS-semantics lens caught is the one worth reading:** `b8be9ef` did not merely leave a gap, it INTRODUCED one — for a signal Node cannot name, pi reports exit code 0 and calls a killed command successful, where the port now reports 128+N. Verified independently under node before acting (`kill -EMT $$` → `close(code=0, signal=null)`), kept deliberately as **D68**, and the false premise it exposed in `coding/execenv.go`'s doc comment ("Node reports that case as a null code plus a signal name") corrected — an equivalence claim that was true of the OS and not of what Node hands pi. A LOW from the same gate became **K19** rather than a fix, on reachability grounds stated in the row. Two cheap findings closed with them: a doc comment `d9e6d21` had taken out with the deleted `replace` test, and the two red commits noted below. **Per-commit greenness, closed at the owner's go-ahead:** the oracle re-capture first landed as its own commit AFTER the Azure pattern, which left the first two commits of the cycle red on their own (`TestProviderErrorPatternSourcesMatchPi` compares the Go pattern source against pi's byte for byte, so a Go-side edit the capture has not seen fails by construction). It is now folded into `df5e7d4`, and **every one of the cycle's ten commits was verified to `go build ./...` and `go test ./... -count=1` green on its own** before `main` moved. The rewrite was checked by tree identity against the pre-rewrite tip (`pre-squash-backup`, `904e155`) and cross-references inside the rewritten commit messages were remapped with it. Only this cycle's commits were rewritten — the base is `16dda13`, so the twelve out-of-cycle commits the previous pin advance published keep their shas.
...
| 3 | **Image generation** | 1,125 LOC in `packages/ai/src` (of which **684 is `image-models.generated.ts`** → catalog surface, not hand-ported) + 228 LOC of `openrouter-images` api/provider | ~2 | Root-export surface (2026-08-27 ruling). Auth, catalog machinery and helpers already exist port-side. `scripts/generate-image-models.ts` is `port-but-CATALOG-ONLY`. | **3** — `5ce4afbd9` (2026-08-29: `image-models.generated.ts` +75 lines, three openrouter image models — `meta/muse-image`, `recraft/recraft-v4-styles` and one sibling); `fbc87d25a` (2026-09-05: `image-models.generated.ts` +30 lines); `bdee230f1` (2026-09-16: `image-models.generated.ts` +34 −4). Catalog surface with no port-side file: `ai/` embeds `models_catalog.json` only, so both land when the entry's base port ships. **Note these did NOT drain with the 0.85.1 regen** even though both are ancestors of the release tag — the text-model queue drained, the image queue cannot until entry 3 has a Go home |
...
already shipped ahead of it. Entry 16 is off that spine and depends on nothing
in it — take it in any cycle with room.
...
the matching npm build. **Currently 10 — grew by five and lost one on
2026-09-18.** It was drained to 0 on 2026-09-07 by the 0.85.1 regen (7 queued
deltas plus `17de82d7b`'s `gpt-6-astra` generator half, all ancestors of the
`v0.85.1` release commit); `7d8ab31a4` reopened it. Every row touches
`generate-models.ts`, and two of them form an ORDERED chain — `6b94ae2ec`
rewrites `applyThinkingLevelMetadata`'s fireworks branch and
`processFireworksModels`, and `bbb61e34a` then rewrites line 729 of
`detectOpenAICompletionsCompat` (`sendSessionAffinityHeaders`) — so replay the
queue in commit order, and check each row against the release sha with
`git merge-base --is-ancestor` rather than by log order. (`d92eb8d4b` was the
third link in this chain until 2026-09-18 retired it — see below.)

**A queued catalog delta can be killed by a later upstream change, and this
queue is where that gets noticed.** `d92eb8d4b` (2026-09-10) queued
`supportsToolReferences: true` onto the Fireworks `anthropicCompat`. `9e05370b2`
(2026-09-16) then DELETED `supportsToolReferences` from
`AnthropicMessagesCompat` and `deferredToolsMode` from
`OpenAICompletionsCompat` outright — the whole deferred-tools mechanism is
replaced by the transcript model — and deleted the row's own oracle,
`packages/ai/test/fireworks-deferred-tools.test.ts`, along with
`test/deferred-tools.test.ts`, `src/utils/deferred-tools.ts` and the
`kimi-deferred-tools` example. Replaying that row at the next regen would have
emitted a key the type no longer has. **It is RETIRED, not drained.** The
feature survives in a new shape (`supportsToolSearch`,
`supportsMidConvoToolAdditions`), which `9e05370b2`'s own row below carries. The
lesson, which is the reason this paragraph exists rather than a silent
deletion: **verify each queued row against the generator at HEAD before
replaying it** (`git -C <clone> grep -n "<the key>" origin/main -- packages/ai`),
because a row is a description of code that may no longer be there.

**The 2026-09-18 additions make a SECOND ordered chain, and it crosses the
first.** `9e05370b2` rewrites `detectOpenAICompletionsCompat`'s default-compat
object and adds `applyOpenAICompletionsTranscriptMetadata` /
`applyOpenAIResponsesTranscriptMetadata` beside it; `bbb61e34a` edits a line in
the same function, and `6671c6047` adds a key to the same Baseten compat object
`9e05370b2` reshaped. `aa50fe778` then MOVES google out of `loadModelsDevData`
into a new `processGoogleModels`, and `bb0f4aa60` edits the
`loadModelsDevData` hunk `aa50fe778` left behind. Commit order for the whole
queue is therefore: `7d8ab31a4`, `6b94ae2ec`, `bbb61e34a`, `12f59336a`,
`2e6fe2f98`, `9e05370b2`, `6671c6047`, `aa50fe778`, `3955b27a1`, `bb0f4aa60`
(ten rows — `d92eb8d4b` is retired) — replay it that way, and still check each
row against the release sha with `git merge-base --is-ancestor`.

| delta | what regenerates |
|---|---|
| `bb0f4aa60` (2026-09-18) | DeepSeek V4 effort metadata. Two hunks. (a) `applyThinkingLevelMetadata`'s `deepseek-v4` branch gains `&& model.thinkingLevelMap === undefined`, so it stops overwriting a map the catalog already carries — measured against the shipped catalog, **all 34** `openai-completions` `*deepseek-v4*` models already have a `thinkingLevelMap`, so after the regen the branch fires on none of them and each keeps whatever models.dev gives it. (b) `loadModelsDevData` gains an `opencode-go`/`deepseek-v4.1-flash` arm taking `getEffortThinkingLevelMap`. That id is **not in the shipped catalog** (opencode-go carries `deepseek-v4-flash`, `deepseek-v4-flash-vision-exp` and `deepseek-v4-pro`), so its effect depends on what models.dev serves at regen time — measure it then rather than predicting it. Do not confuse this with `12f59336a`, which renames the **deepseek** provider's Flash ids; different provider. No src-side Go change: `ThinkingLevelMap` is catalog data. |
| `3955b27a1` (2026-09-17) | Vercel AI Gateway stops replaying unsigned thinking as assistant text (#9676). `fetchAiGatewayModels` adds `compat: { allowEmptySignature: true }` to every model it builds, so all **237** `vercel-ai-gateway` models in the shipped catalog gain it (0 carry it today). **Request-body-affecting**, and no src-side Go change: `ai/providers/anthropic.go` already reads `allowEmptySignature` through `applyCompat` — the same key `6b94ae2ec` turns on for Fireworks. |
| `aa50fe778` (2026-09-16) | Google thinking levels derive from models.dev instead of hardcoded families — the catalog half of `16235fd93`, whose src half SHIPPED this cycle as `8b5f489`. The four id-regex helpers (`isGoogleThinkingApi`, `isGemini3ProModel`, `isGemini3FlashModel`) and their three `applyThinkingLevelMetadata` arms are DELETED for one `getGoogleThinkingLevelMap(modelId, reasoningOptions)` that prefers `getEffortThinkingLevelMap` and falls back to the Gemma 4 map; google and google-vertex move out of the generic `loadModelsDevData` path into a new `processGoogleModels`, which also resolves `gemini-flash-latest` → `gemini-3.5-flash` and `gemini-flash-lite-latest` → `gemini-3.1-flash-lite` for their metadata, skips `gemini-3.1-flash-lite-preview` on vertex, and pins vertex `gemini-2.5-flash` `cacheRead` to 0.03. **Diff thinkingLevelMap PER MODEL after this regen** (see the tripwire below): 23 of the 29 shipped `google-generative-ai` models carry one today, and this row rewrites how every one of them is derived. No src-side Go change. |
| `6671c6047` (2026-09-16) | Baseten automatic prompt caching needs session affinity. `processBasetenModels`' shared compat gains `sendSessionAffinityHeaders: true`, so all **20** baseten models in the shipped catalog gain it (0 carry it today). No src-side Go change — `ai/providers/` already sends the headers off this key; this is the same key `bbb61e34a` flips for openrouter, in a different provider's object. |
| `9e05370b2` (2026-09-16) | The generator half of the mid-conversation system-message work, whose src half SHIPPED this cycle across six commits (`e6d507d`…`54d04a7`). Adds `supportsMidConvoSystemMessages` / `supportsMidConvoToolAdditions` / `supportsMidConvoToolChanges` to the compat the catalog emits: `applyOpenAICompletionsTranscriptMetadata` turns the first on for the verified completions models and adds tool additions for Kimi K3; `applyOpenAIResponsesTranscriptMetadata` does the Responses side, with `OPENAI_RESPONSES_PROXY_PROVIDERS` held back from tool search; and the Anthropic path turns on both keys for verified first-party models and for opencode/github-copilot relays, minus `MID_CONVO_EFFORT_UNSUPPORTED_ANTHROPIC_MODELS`. **All three keys are absent from all 1,354 models in the shipped catalog**, so until this regen lands every model falls back to the `false` default and the port collapses every transcript — which is what 0.85.1's pi does too, so the port and the build agree today and will still agree after the regen. **Request-body-affecting on every provider that gains a key.** No src-side Go change: `ai/providers/` reads all three through `applyCompat`. |
| `2e6fe2f98` (2026-09-10) | Removes `openai-codex/gpt-5.4` and `openai-codex/gpt-5.4-mini`, both in the shipped catalog. No Go default points at either — `coding/resolve.go`'s `openai-codex` default is `gpt-5.5`, matching upstream's `model-resolver.ts` — and the Go tests that say `"gpt-5.4"` build synthetic models or name other providers, so no fixture breaks. The Codex adapter itself is queue entry 10; these are catalog rows only. |
| `12f59336a` (2026-09-10) | DeepSeek's Flash models collapse into one: `deepseek/deepseek-v4-flash` and `deepseek/deepseek-v4-flash-vision-exp` are REMOVED and `deepseek/deepseek-flash` ("DeepSeek V4.1 Flash", `text`+`image`, 1M context, 384k max tokens) is added. Its thinking map is set explicitly to `DEEPSEEK_V4_FLASH_THINKING_LEVEL_MAP`, because the new id no longer matches `applyThinkingLevelMetadata`'s `includes("deepseek-v4")` branch — so it resolves to the same map the removed model carried (`low`/`high`/`max`, with `minimal`/`medium` null). Pricing moves (input/output/cacheRead): Flash 0.14/0.28/0.0028 → **0.30/1.20/0.006**, `deepseek-v4-pro` 0.435/0.87/0.003625 → **1.32/3.96/0.044**. **Breaks two Go tests at the regen** — see the model-id tripwire below. |
| `6b94ae2ec` (2026-09-10) | Fireworks stops being an allowlist and reads catalog effort metadata. `processFireworksModels` adds `allowEmptySignature: true` to the shared `anthropicCompat` — so all **14** fireworks `anthropic-messages` models in the shipped catalog gain it — and sets `forceAdaptiveThinking` on any model advertising an `effort` reasoning option, with a five-id verified fallback where models.dev is incomplete. `applyThinkingLevelMetadata` then rewrites the thinking maps: `glm-5p2` loses its `low`/`medium` aliases (both → `null`, since Fireworks maps them to `high`), `kimi-k3` loses `medium`, `qwen3p8-max` gets a synthesized low/medium/xhigh map, `qwen3p8-2p4t-a95b` and any toggle-advertising model get `off: "none"`, and `deepseek-v4-pro-0813` gains `low`. No src-side Go change is required: `ai/providers/anthropic.go` already reads `allowEmptySignature` and `forceAdaptiveThinking` through `applyCompat`, and `ThinkingLevelMap` is catalog data. `519184eb6` is this delta's test half and rides with it. |
| `bbb61e34a` (2026-09-10) | `detectOpenAICompletionsCompat`'s `sendSessionAffinityHeaders` flips `false` → `isOpenRouter`, and the generator emits any key differing from `OPENAI_COMPLETIONS_DEFAULT_COMPAT` (still `false`), so a regen ADDS `sendSessionAffinityHeaders: true` to every openrouter `openai-completions` model — **351** in the shipped catalog, of which 0 carry the key today. The src half shipped this cycle, and it makes the regen a no-op in behaviour rather than a change: the port's own default already resolves these models to `true`. Land it anyway so the catalog keeps matching what pi publishes. |
| `7d8ab31a4` (2026-09-07) | Copilot's `needsResponsesApi` widens from a `gpt-5` prefix to a `gpt-` prefix. Measured against the shipped catalog, exactly ONE model flips: `github-copilot/gpt-6-astra`, `openai-completions` → `openai-responses`. The other 9 copilot `gpt-*` models are already `openai-responses`, and no `grok-`/`oswe-`/`mai-` model is touched. |
...
assert on the decoded compat. Checked and passing at 0.85.1.
...
list. At 0.85.1 that moved 21 surviving models (mostly a new `off: null`;
`qwen3.8-flash` changed shape outright).
...
branches (the reasoning-level and `thinkingLevelMap.off` handling), so a copilot
model whose `api` flips from `openai-completions` to `openai-responses` starts
executing code it never reached before. `7d8ab31a4` will do exactly that to
`github-copilot/gpt-6-astra` at the next regen. No Go test pins any copilot
model's `api` today (the copilot references in `ai/` are headers, `filterModels`
and beta-effort, all on synthetic models), so nothing will go red to announce
it — check the flipped model's request body by hand after that regen.
...
silently stops testing anything. 0.85.1 removed 28 models and moved
`anthropic/claude-opus-4.8-fast`'s sole copy from `openrouter` to
`vercel-ai-gateway`, breaking a fixture that had already been re-pointed once
before. Prefer fixtures that DERIVE the expectation from the catalog and assert
the precondition that makes the behaviour deterministic — see
`TestResolveModelProviderPrefixFallsBackToFullID`. **Loaded for the next regen:**
`12f59336a` removes `deepseek/deepseek-v4-flash`, which two tests in
`ai/providers/openai_compat_test.go` look up by id —
`TestOpenAICompatDeepSeekUsesMaxTokens` (a loop over `deepseek-v4-flash` and
`deepseek-v4-pro`) and `TestOpenAICompatDeepSeekSendsMaxTokensInBody`. Both
`t.Fatal` on a missing model, so they go red rather than vacuous; re-point them
at `deepseek-flash`.
...
`ai/providers/testdata/httperror`); google and anthropic are K18.
...
on the astral row). The port's `google.go:450` and `anthropic.go:660` both route
...
retry fail-fast quote — `anthropic.go:648` passes it to `retryFromOptions`, for
`validateServerRetryDelay` in `retry.go` — not the terminal non-2xx path at
`anthropic.go:660`; `google.go:431` passes nil, and no google counterpart
exists. The slice should decide the retry-path cap from the same oracle (extend
...
multi-byte sequences); one leading BOM is stripped (`stripBOM` in `errors.go`)
and that is the only piece of the decode step ported. Fix with a ~30-line
decoder at the read site shared by all four adapters, and teach `capture.mjs`
to carry byte bodies (base64) so the oracle can hold the rows.
