...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`9767ba275`** — "fix(coding-agent): select Radius models after catalog discovery" (advanced 2026-09-07). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all 19 first-parent changes in `744a94d7d..9767ba275` (no merge-smuggled commits — the all-commits count equals the first-parent count): **3 port / 2 port-but-CATALOG (the two release commits) / 1 port-but-QUEUED / 13 n/a / 0 decide**. The unported balance is carried by **Scope queue entries 3, 11, 12, 13 and 14** — not by the pin. The next cycle's drift is `9767ba275..origin/main` PLUS those five rows; a drift check that reads the sha alone will see a clean slate and be wrong. **Shipped this cycle:** `17de82d7b` (pi split the two prompt-cache params — a `supportsExplicitPromptCacheMode` model now spells long retention `prompt_cache_options: {ttl:"30m"}` and must NOT also send `prompt_cache_retention: "24h"`, in `ai/providers/openai_responses.go`); `fcff255b0` (strict sampling is the DEFAULT for the built-in read/bash/powershell/edit/write tools — `PREFER_STRICT_TOOL_SAMPLING` and the `PI_EXPERIMENTAL` gate are gone, in `coding/tools.go` + `coding/experimental.go`); `9767ba275` (`defaultModelPerProvider["radius"]` `auto` → `balanced` in `coding/resolve.go`); the **0.85.1 catalog regen**; and the difftest re-pin that returned the whole suite to `dist`. **Not shipped:** entries 11–14 are untouched this cycle; entry 3 gained one delta. **Reconciliation at the advance:** detector 136 paths (59 `coding-agent`, 40 `tui`, 9 `ai`, the rest packaging/CHANGELOG noise), A/D/R confined to clipboard, the experimental CLI split and TUI native — **no new top-level package, nothing moved out of `packages/`, no new repo-root directory**; accounting sweep 66 files, all accounted for. **The accounting sweep earned its keep this cycle:** it surfaced `src/utils/clipboard*.ts` (4 files, one added, one deleted) that a per-commit diffstat had hidden by truncating `caf6dfe73`'s 54-file stat — judged `n/a` on E1's only-consumers clause, since every consumer is `modes/interactive/**` plus a root re-export and the port has no clipboard code. `core/model-resolver.ts` **was** in range (`9767ba275`) and was diffed per the standing rule — the first time that tripwire has fired and been caught rather than missed. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **advanced from 0.84.4**. Two release tags crossed in range (`107d79f11` v0.85.0, `d981de122` v0.85.1), so the catalog was regenerated: **563,988 → 592,846 B, 1290 → 1354 models across an unchanged 39 providers** (+92 / −28 / 86 changed). Endpoint-pinned per the 2026-07-30 ruling by executing `JSON.stringify(MODELS)` against `dist/models.generated.js` on both builds — the OUTGOING catalog is byte-identical to a fresh 0.84.4 derivation and the incoming one to a fresh 0.85.1 derivation, which makes the committed diff provably equal to the upstream release diff. **The catalog-only queue is DRAINED, 7 → 0**: all seven queued deltas plus `17de82d7b`'s generator half are ancestors of `d981de122` (checked with `git merge-base --is-ancestor`, never log order). All four recorded regen tripwires were checked and passed — BILLING (every `allowedFallbackModels` entry still carries `provider`), `4e69b0c28`'s OpenRouter reroute (15 `anthropic/*` models now on `anthropic-messages`, `:batch` correctly excluded, no Go change needed because the registry dispatches on `model.Api` and pi has no openrouter branch either), `265a33393`'s derived `thinkingLevelMap` (21 surviving models changed), and `0dbf2b76f` (`grok-build-0.1` gone). **The Anthropic caveat carried since 2026-09-04 is RESOLVED:** `4e69b0c28` shipped in 0.85.1 (the 0.84.4 dist still writes `"anthropic-beta": …join(",")`, 0.85.1 writes `betas: […]`), so all 14 `src`-backed difftest scenarios flipped to `dist`. **Port release `v0.85.21` cut from this cycle.** |
| Parity proofs at the pin | **2026-09-07:** all four units were written red-first with a **behavioural** red — the `:max` fix reproduced the exact reported failure (id `"claude-opus-4-8:max"`, empty level, spurious "not found" warning) before the one-line change, and the strict-sampling red showed `nil` sampling on all five tools under `PI_EXPERIMENTAL` unset and `0` while the `=1` arm already passed, isolating exactly the new behaviour. Review ran **6 independent read-only lenses** (parity per unit, a dedicated completeness/missed-hunk lens, cross-cutting Go idiom, cross-cutting test quality), and **every** finding was handed to **3 separate refuters instructed to REFUTE it**, defaulting to refuted when uncertain: **3 filed, 2 refuted, 1 survived unanimously (0/3 refuted)**. The survivor was a **pre-existing** parity divergence the cycle then fixed rather than deferred: `validThinkingLevels` carried six of pi's seven `VALID_THINKING_LEVELS`, so every `:max` spec fell through to the custom-id fallback with the suffix glued on. Its covering test looped over six levels where upstream loops over seven, so it **pinned the omission instead of catching it** — and because that test only walks the custom-id path, where a missing level is indistinguishable from a genuine custom id, it could never have caught this at all; a second test now pins the real-model path. Independently mutation-verified: removing the `!SupportsExplicitPromptCacheMode` guard turns the catalog-driven prompt-cache test red on `gpt-6-astra`, so it is not vacuous. Full gate green — `gofmt`/`build`/`vet`, `go test -race ./... -count=1`, `GOOS=windows` build+vet, and **difftest 51 PASS / 0 KNOWN / 0 FAIL, now 51/51 `dist`-backed** against the published 0.85.1 build with an empty known-divergence baseline. |
| Reviewed via | 2026-09-07 — per-commit triage of all 19 first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Diffstat-dispatched only for `packages/tui`, docs/CHANGELOG, packaging and `modes/interactive` + `cli/`; hunks read for everything touching `packages/ai/src`, `packages/coding-agent/src/core` and the split-by-HUNK files. Then a 15-agent review pass: 6 independent read-only reviewers on file-disjoint lenses plus 9 refuters (3 per filed finding). Porting itself stayed serialized in one working tree — parallel porters would corrupt each other — and reviewers were forbidden to mutate the tree. |
...
the matching npm build. **Currently 0 — DRAINED 2026-09-07** by the 0.85.1
regen (7 queued deltas plus `17de82d7b`'s `gpt-6-astra` generator half, all
ancestors of the `v0.85.1` release commit). The next generator delta opens the
queue again.
