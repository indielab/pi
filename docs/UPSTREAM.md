...
| What | Value |
|---|---|
| TS source fully reviewed/ported | **`62129190d`** — "docs(agent): status lives in task state" (advanced 2026-09-11). **READ THIS BEFORE COMPUTING DRIFT: the pin is REVIEW-complete, not PORT-complete.** This cycle triaged all **15** first-parent changes in `bbb61e34a..62129190d`, and the two counts agree again — 15 commits, 15 first-parent — because no merge is in range. Verdicts: **0 port-shipped / 1 port-but-QUEUED / 3 port-but-CATALOG-ONLY / 11 n/a / 0 decide** — a triage-only advance with **no Go commits**, and none landed out of cycle either (`git log c3ffe74..HEAD` was empty when the cycle opened). The unported balance is carried by **Scope queue entries 3, 6, 7, 8, 9, 10, 11, 12, 13, 14 and 15** plus the catalog-only queue — not by the pin. **Read the queued-deltas column; never inherit the previous cycle's list.** The next cycle's drift is `62129190d..origin/main` PLUS those rows. **Queued:** entry 6 → **3** (`4bd3f48df`: Mistral-hosted GLM-5.2 joins `usesReasoningEffort`), catalog-only → **6** (`12f59336a`, `2e6fe2f98`, `d92eb8d4b`). **n/a (11):** ten `packages/agent/docs/pico/*.md` prose commits and one `.github/APPROVED_CONTRIBUTORS` approval, all diffstat-dispatched; `d92eb8d4b`'s `packages/coding-agent/docs/extensions.md` hunk is prose too. **The classification worth keeping:** `d92eb8d4b` is a `feat(ai)` carrying a 210-line new test and **zero** `src` lines. Its one behavioural hunk is a generator key — `supportsToolReferences: true` on Fireworks' shared `anthropicCompat` — and the code that key switches on already ships in Go (`ai/providers/anthropic.go` reads it through `applyCompat` and already emits `defer_loading` and `tool_reference` for first-party Anthropic). It is catalog-only for that reason, not because it is small, and it is request-body-affecting at the regen. **Reconciliation at the advance:** detector 16 paths, A/D/R is **2 additions** (`packages/agent/docs/pico/pico-handoff.md`, `packages/ai/test/fireworks-deferred-tools.test.ts`), **0 deletions and 0 renames** — no new top-level package, nothing moved out of `packages/`, no new repo-root directory. Accounting 14 files, all accounted for. None of the four split-by-HUNK tripwire files was in range, so the standing `defaultModelPerProvider` rule did not fire. |
| npm build the byte-goldens were captured from | `@earendil-works/pi-ai` **0.85.1** — **unchanged**. No release tag is in range (`git tag --contains bbb61e34a` is empty, and `packages/ai/package.json` at `62129190d` still reads `0.85.1` under an `[Unreleased]` changelog), so there was **no catalog regen and no reference-build refresh**, and no release is cut this cycle. The catalog-only queue GREW from 3 to **6**: `7d8ab31a4`, `6b94ae2ec` and `bbb61e34a`'s generator hunk (unchanged), plus `12f59336a`, `2e6fe2f98` and `d92eb8d4b` — see the catalog regen checklist, which now records a longer ordered chain (`6b94ae2ec` → `bbb61e34a` → `d92eb8d4b`) and a Go fixture the regen will break. **Carry to the next release, unchanged from 2026-09-09:** `4a6ed0194`'s MAJOR `@google/genai` bump (1.52.0 → 2.21.0) is invisible to difftest, whose google scenarios are `dist`-backed at 0.85.1, i.e. pre-bump. Re-capture the google scenarios against the first npm build that ships it. |
| Parity proofs at the pin | **2026-09-11:** no Go code changed this cycle, so there is no new red to record. The proofs of record for shipped behaviour stay with the commits that shipped it — most recently `b9ae800` (retry cap) and `4beb4da` (OpenRouter affinity), whose red-first record is this row as committed in `c3ffe74`. Full gate re-run on the unchanged tree at the advance: `gofmt -l` clean, `go build`/`vet`, `go test -race ./... -count=1` (every package `ok`), and `GOOS=windows` build+vet, all green. **difftest re-pinned to `62129190d` and re-run: **51 PASS / 0 KNOWN / 0 FAIL** (exit 0) against 0.85.1 with an empty known-divergence baseline, so no entry reports FIXED and none retires.** The re-pin is bookkeeping only — all 51 scenarios are `dist`-backed at 0.85.1 and `PI_UPSTREAM_SHA` drives only `"backend": "src"` scenarios, of which there are none. |
| Reviewed via | 2026-09-11 — per-commit triage of all 15 first-parent changes, judged from real diffs (`git diff <sha>^1..<sha>`), never from subject lines and never from the clone's working tree. Diffstat-dispatched only for the ten `packages/agent/docs/pico*` prose commits and the contributor approval; hunks read for all four `packages/ai` commits, tests included — which is how `d92eb8d4b`'s zero-`src` shape and `12f59336a`'s explicit thinking map were established rather than assumed. Catalog effects were measured by decoding the shipped `ai/models_catalog.json`, not estimated. Adversarially verified before commit, every agent read-only: **5 auditors** — an independent from-scratch re-triage that reproduced all 15 verdicts and both reconciliation sweeps; a catalog measurer that re-derived every count, price and thinking map in the three new rows from the shipped catalog and the generator at each named sha; a Go-surface check; a ledger-bookkeeping audit of cell counts, counters and stale text; and a completeness critic — then **3 refuters** per filed finding, instructed to REFUTE and to default to refuted when uncertain: **3 filed, 2 refuted 3–0, 1 survived 3–0.** The survivor: the checklist intro cited `bbb61e34a`'s edit as "line 726", a hunk-header start carried forward from the previous cycle; the edited line is 729, now corrected. **The strongest evidence is behavioural:** two agents independently transliterated every scenario of pi's `fireworks-deferred-tools.test.ts` (3 models × 3 loader names, the dedupe case, and all five keep-normal-schemas cases) and ran them against the shipped Go adapter with the regen's compat simulated (`allowEmptySignature` + `supportsToolReferences`), one via `go test -overlay` and one as an out-of-tree program, so no repo file was written. Every body matched pi's exactly, and with today's compat no deferral is sent. That is what licenses "no src-side Go change" for `d92eb8d4b`. |
